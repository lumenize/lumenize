...
| `prose-voice.md` | ADRs, `docs/vision/**`, `tasks/**`, rules, skills — ⚠️ **read before DRAFTING, not only when editing** | how Larry writes: concrete over abstract, examples, lead the reader; coinage needs his approval first. Budgets calibrated on the three hand-written references, checked by `npm run audit:prose` |
...
| `ui-routing.md` | `apps/nebula-studio-ui/**` | the URL is read in one place (`viewState`) and written by one function (`navigate` / `leaveTo`), in `src/view-state.ts` only; `npm run audit:urls` is the proof. ADR-017 says what rides the URL; this says how |
| `studio-guidance.md` | `apps/nebula/platform/**`, `codegen-loop.ts`, `galaxy.ts`, `container/app/**`, `website/docs/nebula/**` | the guidance tree the model reads every turn — one home per kind of guidance (the placement table), the platform layer generated into the bundle, the Galaxy seed on the scaffold, `preview` not `publish` |
