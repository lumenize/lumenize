...
Promise pipelining allows using the result of an unawaited call as a 
parameter to another call. This creates dependent operations that can still be 
done with a single round trip because the server resolves the promise values.
...
round trip by substituting promise values on the server side.
*/
it('compares promise pipelining performance', async () => {
  ...
  // All 3 dependent operations sent in exactly 1 round trip for both!
  expect(lmMetrics.wsReceivedMessages).toBe(1);
  expect(cwMetrics.wsReceivedMessages).toBe(1);
});

/*
## Promise Pipelining (nested syntax)

Promise pipelining also works with nested call syntax. Both calls are sent 
in a single round trip with the server resolving dependencies.
