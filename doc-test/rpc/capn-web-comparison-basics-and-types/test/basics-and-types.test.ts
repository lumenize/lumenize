...
(SQLite engine) supports except `Blob`, plus a few additional types. The only 
types that Workers RPC supports that Lumenize does not are native `Request` 
and `Response` (Lumenize carries `RequestSync` and `ResponseSync` instead), 
`Blob`, and Readable/Writable Streams. 

**Cap'n Web** has added most of the types it lacked at launch. It still 
throws on `Map`, `Set`, and cycles, and it sends an alias as separate copies. 
`RegExp` support is merged and awaiting a release.
...
it('demonstrates error as value', async () => {
  ...
  expect(capnwebResult.stack).not.toContain('basics-and-types.test.ts'); // ❌
});
...
**Cap'n Web**: ❌ Throws "Serialization exceeded maximum allowed depth" on a 
cycle, and sends an alias as separate copies
...
it('demonstrates circular references', async () => {
  ...
  await expect(async () => capnwebClient.echo(circular))
    .rejects.toThrow('maximum allowed depth'); // ❌
});
...
**Both**: ✅ All standard types preserved correctly
*/
it('demonstrates standard type support', async () => {
  ...
  expect(Number.isNaN(await capnwebClient.echo(NaN))).toBe(true); // ✅
  expect(await capnwebClient.echo(Infinity)).toBe(Infinity); // ✅
  expect(await capnwebClient.echo(-Infinity)).toBe(-Infinity); // ✅
