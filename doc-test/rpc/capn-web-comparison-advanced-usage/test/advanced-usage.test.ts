...
/*
# vs Cap'n Web (advanced usage)

This living documentation compares advanced usage patterns between Lumenize RPC 
and Cap'n Web (Cloudflare's official "last-mile" RPC solution).

Using this doc-test approach, when either package adds capability, we'll
know immediately because the tests will start failing.
*/
...
it('demonstrates error as value', async () => {
  ...
  expect(lumenizeResult.stack).toContain('advanced-usage.test.ts');
