...
By using this doc-test approach, when either package adds capability, we'll
be able to instantly tell because the tests in here will start to fail
as soon as we upgrade to the latest version.
...
Cap'n Web, but we couldn't find any documentation or examples of doing this and 
trying the obvious didn't work. If anyone knows if/how this can be done with 
Cap'n Web, please let us know and we will immediately update this document. 

That said, if this holds, it is the biggest usage difference between Cap'n Web 
and Lumenize RPC.
...
Most disappointing to us at Lumenize on the supported types front is that Cap'n 
Web does not AND WILL NOT EVER support cyclic values or aliases. Our core 
product makes use of directed acyclic graphs (DAG) with heavy use of 
aliases. Refactoring to a nodes + edges data structure is unworkable.

We have often thought that we would move our inside-Cloudflare transport to 
Workers RPC but those discussions have stopped because of this:
