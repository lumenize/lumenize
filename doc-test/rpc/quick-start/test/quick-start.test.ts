...
it('detects package version', () => {
  expect(lumenizeRpcPackage.version).toBe('0.24.0');
});
...
You can even access the remote DO's globalThis as a parameter, but we
don't provide a way to access it directly.
*/
it('shows accessing globalThis as a parameter (but not directly)', async () => {
  using client = getLumenizeClient('global-this');
  
  client.ctx.storage.kv.put('id', (globalThis as any).crypto.randomUUID());
  const result = await client.ctx.storage.kv.get('id');
