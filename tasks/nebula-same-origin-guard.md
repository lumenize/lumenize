...
`apps/nebula-studio-ui/vite.config.ts` proxies `/auth`, `/gateway` and `/app` to the Worker with `changeOrigin: false` (flipped from `true` on 2026-09-02 so the Worker sees the browser's real Host — magic-link URLs follow the request origin, and the rewrite had every hand-driven login link landing on `:8787`; the config comment carries it) — and either way **no `Origin` rewrite**. That is the shared standalone dev server (5174 → 8787) and it has **three** consumers: `npm run dev:studio`, the `ui-smoke` lane (whose setup loads that very config and whose header says it relies on there being no origin check), and the `/live` harness (`harness/lib/browser.ts` `bootStudioVite`; captured artifacts show `localhost:5174/auth/.../refresh-token`).
