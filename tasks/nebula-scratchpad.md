...
- **Propagate wire `callId` into `CallContext` as a top-level field**: Today `callId` exists only on wire-level message types in `packages/mesh/src/gateway-messages.ts` (CallMessage, CallResponseMessage, IncomingCallMessage, IncomingCallResponseMessage); it never reaches `CallContext`, so downstream DOs can't include it in logs. Add `callId?: string` directly on `CallContext` (NOT in `state` — `state` is the mutable handler scratchpad; `callId` is immutable for the life of a call, matching `callChain` and `originAuth`). Optional because server-originated `lmz.call()` (no client at the head of `callChain`) doesn't transit a Gateway so has no wire callId — the ~99% client-originated calls get it; the rest log without. Motivated by the Gateway-timestamp collision concern: WS invocation boundaries are fuzzy (`feedback_cf_clock_traps.md`), so multiple frames handled back-to-back can share `Date.now()`, making timestamp unreliable for per-call correlation. Once propagated, Gateway logs at `lumenize-client-gateway.ts:322/330/340/354` (currently timestamp-only frame-handling paths) AND every downstream DO's logs can carry the same `callId`, letting an operator stitch a single failing call end-to-end. Gateway already includes `callId` in call-result paths (`:605, :628`), so the field is already trusted there — the work is propagating it into the outbound `CallContext` the Gateway builds for `__executeOperation` and every hop's `runWithCallContext` preserving it. Implementation surface: `packages/mesh/src/types.ts` (CallContext shape), `lumenize-client-gateway.ts` (Gateway-side packing), `lmz-api.ts` (context-building + propagation).

- **Watch for browser ALS polyfills and swap one in when ready**: Today, `packages/mesh/src/lmz-api-context.browser.ts` is a module-scoped-variable shim that preserves context synchronously but not across `await` boundaries — `LumenizeClient` framework code works around this by threading `CallContext` explicitly through closures (refactored 2026-06-03; see `tasks/playwright-test-template.md` § Known blockers #2 for the journey). User code reading `this.lmz.callContext` AFTER an `await` inside a browser-side `@mesh()` handler hits a silent cliff — corrupted with whatever context the most recent concurrent handler set. No current handler does this, so it doesn't bite. **The clean fix is a real ALS polyfill**, but as of 2026-06-03 the available options are weak: `@b9g/async-context` uses `node:async_hooks` internally (defeats our purpose), `@webfill/async-context` is 3+ years stale, `simple-async-context` is unvetted, and `unctx` has a different API model. Userland Promise-then patching can't intercept V8's `await` fast-path; the working alternative requires zone.js-style global Promise replacement, too heavy for a library. The TC39 [AsyncContext proposal](https://github.com/tc39/proposal-async-context) is at Stage 2 — when it advances to Stage 3+ and ships natively in a browser engine, OR when a battle-tested polyfill (TC39-shaped, light-weight, not Node-ALS-internal) appears in the npm ecosystem, swap it in. The mesh-side change at that point is small: rewrite `lmz-api-context.browser.ts` to wrap the polyfill, keep the same `runWithCallContext`/`getCurrentCallContext` exports. The framework-internal explicit threading we added stays as a defense-in-depth — works correctly regardless of whether the polyfill is present. Re-evaluate every ~6 months or when someone needs cross-await callContext reads in a browser handler.
