...
- [ ] Measure Gateway hop latency overhead (requires live deployment)
  ...
  - **What to measure**: Both *latency* (round-trip per call) and *throughput* (peak per-DO sustained txn/s) for Client→Gateway→DO→Gateway→Client vs hypothetical Client→DO→Client direct.
  - **Throughput direction not obvious**: More hops add queues, and queues can *improve* throughput when an upstream stage offloads serial work from the bottleneck (Gateway doing OCAN checks + routing on its own DO, sparing the parent DO's ~2.5 ms serial CPU floor) or absorbs bursts. With 1:1 Gateway architecture, auth work for N clients runs across N concurrent Gateway DOs instead of serially on the parent DO. The Gateway could plausibly cost latency per call but win on peak per-DO throughput; could also break even or lose. Direction is empirical.
  ...
  - **Methodology — see [Piercing the temporal haze](/blog/benchmarking-cloudflare-durable-objects-from-outside)**: This task is the "next rung up" the post's Ping baseline section explicitly gestures at. Approach: add intermediate push frames back to the Node-side observer at each point you care about (when the call lands at the Gateway, just before/after the parent DO call, etc.) and read `performance.now()` as each arrives; the deltas tease apart the legs.
  - **Gateway hooks (investigate first)**: We added some hooks in the Gateway code that may or may not allow sending push WS frames from inside the request path. Verify whether these are sufficient before designing extension code; if not, direct Gateway modifications are the fallback.
  - **Starting point — reuse the bench harness from the parse-validate release**: [`apps/nebula/test/browser/`](../apps/nebula/test/browser/) — `transactions.bench.ts` (single-call latency), `throughput.benchmark.ts` (concurrent ramp), and `harness-client.ts` already implement the Node-side test client → real WebSockets → deployed Worker pattern + ping subtraction. The current ping baseline measures `client → Gateway → Star → Gateway → client` round-trip. To answer this question, either add a parallel ping path that bypasses the Gateway (client → Star direct over WebSocket, if achievable, or client → Star via Workers RPC from a thin shim) and compare baselines, or instrument intermediate observers per the methodology above. Reuse the `Browser`-based harness, the existing `wrangler dev` autospawn, and the auth bootstrap. Cross-post-test methodology is documented in `apps/nebula/test/browser/RESULTS.md`.
  - **Tooling**: Use Node-side `performance.now()` (Cloudflare time-pinning makes in-Worker measurement unreliable — see the methodology link above); compare same-region and cross-region scenarios
