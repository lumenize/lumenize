...
- [ ] **Data-use consent flag (registry `Instances`, set at self-signup) — build at tester-onboarding.** Each F&F tester gets their own Galaxy; their Studio prompt/turn data is captured in their **Galaxy DO's SQLite** (the recorder corpus). **Home decided: the auth registry.** The self-signup / claim flow is an **auth feature** (already in `NebulaAuthRegistry` — `claimUniverse`/`createGalaxy`/`claimStar`, `packages/nebula-auth/src/nebula-auth-registry.ts`), so consent is captured right there — *not* a Nebula-concept leaking into auth. Add a **generically-named** column to the registry `Instances` table (schema in `packages/nebula-auth/src/schemas.ts` `REGISTRY_SCHEMAS`) — name it for the generic concept "consent to use your data to improve the product" (e.g. `improveProductConsent`), **never** `nebula`/`studio`-specific, so nebula-auth stays product-agnostic. Surface the notice at the slug-pick prompt (now: "we use your prompt data for feedback"; later: "let us use your data to improve the product?"). "Testers we may use" = `SELECT instanceName FROM Instances WHERE improveProductConsent = 1`. **Assume `true` (hard yes) for now** — not built yet. Surfaced 2026-06-22 (punting the R2 store → Galaxy DO; registry + generic name resolves the earlier layering worry).

- [ ] **⚠️ Studio live dev-loop version contract — `appVersion:'dev'` vs the content-hash ontology version (BLOCKS THE LIVE PREVIEW).** Found during the Phase-4 teardown (2026-06-21). In the assembled preview, `DevContainer.fetch()` injects `appVersion: 'dev'` into the shell, but `DevStudio.applyOntology` installs the ontology under a **content-hash** version (`git.hashBlob` of the `.d.ts` — required so the Star's Worker Loader `bundleId = galaxyId/version` cache never serves a stale validator). So the client always sends `'dev'`, the Star's current version is a 40-hex oid → **mismatch → `OntologyStaleError` on every live op**. Needs `wrangler dev` + Docker Desktop (can't surface under vitest-pool-workers; tests use explicit matching version labels, so they're unaffected). **✅ DESIGN RESOLVED 2026-06-21** — canonical in [nebula-dev-flows.md](nebula-dev-flows.md) **Decision 12 + Flow 1d**; build = [nebula-studio.md](nebula-studio.md) **Phase 5**. Resolution: inject the *real* content-hashed version by **push** (`DevContainer.setAppVersion`, replacing `'dev'`) + re-sync via the kept reload channel triggered from `Star.#installState` on `isNewVersion` — **not** a `.dev`-special branch in the hot path (Decision 11 holds). **NOT** deferred to the agentic engine after all (earlier steer superseded — it's a clean, mostly-testable Phase 5). Prerequisite for the first working live preview. Also tracked in `nebula-studio.md` Phase 5 + [[project_studio_uibuild_pivot]]. **✅ BUILT 2026-06-21** (`/build-task` Phase 5; affected suites green, the container round-trip = `it.skip` run with `wrangler dev`).

- [x] **✅ FIXED 2026-06-21 — Browser smoke test `3. round-trip` was RED (a Phase-4 escape).** `apps/nebula/test/browser/smoke.test.ts` step 4 registered the ontology on the **Galaxy** (`appendOntologyVersion`) and step 5 transacted on the **Star** expecting `committed` — which only worked when the Star lazy-pulled from Galaxy on a cache-miss. Phase 4 removed that path (`star.ts` Handler-1 cache-miss → `OntologyStaleError`, no Galaxy fetch), so the Star never got `'v1'` → `OntologyStaleError` → not committed → fail. Confirmed pre-existing (failed with Phase-5 stashed); escaped because Phase-4's verify never ran the `browser` project. **Fix:** added a server-side `StarTest.applyOntologyForTest({version, types})` (`@mesh(requireAdmin)`, compiles via `compileOntologyVersion` → `setOntology` — the Node-side harness can't import the Worker-only compiler) and repointed the smoke test's step 4 to install on the **Star** (`callStarApplyOntology`) instead of the Galaxy. Browser smoke now **3/3 green**. (Fix authored in a `spawn_task` chip session triggered mid-`/build-task`; restored into the main tree after I briefly mis-reverted it.)
