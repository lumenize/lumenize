...
- [ ] **`client.logout()` (deferred from nebula-frontend v3 / P7).** The client surface ([api-reference.md § client.logout](../website/docs/nebula/api-reference.md#clientlogout)) is small (POST the auth logout endpoint to revoke+clear the path-scoped refresh cookie → drop the in-memory access token → `disconnect()` so the factory mirrors `lmz.connection.state = 'disconnected'` → redirect), but it has **two unmet dependencies** that make shipping it now untestable code against a phantom endpoint: (1) the **nebula-auth server logout endpoint** (`POST /auth/{authScope}/logout`) doesn't exist — separate nebula-auth task; (2) "drop the in-memory access token" needs a `LumenizeClient` token-clear (private `#accessToken`; `disconnect()` does NOT null it, so a reconnect would reuse a revoked-session token) — a small **mesh** addition best landed with **P9** (mesh first-connect auth classification). **Land logout when the server endpoint + the mesh token-clear exist**, tested via §5.3.8 connection-lifecycle path 6 (logout). Until then `client.logout()` is absent; the api-reference entry is aspirational.
