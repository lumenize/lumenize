...
- [ ] **Turn Turnstile ON in prod + a narrow automation bypass (alpha — when public signup opens).** Pre-alpha runs with Turnstile **OFF** in prod (`TURNSTILE_SECRET_KEY` unset — F&F-only, no public signup, low bot-risk). At alpha, set the prod `TURNSTILE_SECRET_KEY` and add a **server-side bypass in `checkTurnstile` for automation only** — keyed on the **automation email / `test-` scope** (the magic-link request carries email + scope alongside the token, so both are available at check time; no separate header secret needed). **Never** disable Turnstile for real users; don't try to solve real Turnstile in a headless browser (designed to block that — the secret key only *verifies* tokens, can't mint them). Surfaced 2026-06-24 (prod-smoke design).
