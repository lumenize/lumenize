...
- [ ] **⚠️ Studio live dev-loop version contract — `appVersion:'dev'` vs the content-hash ontology version (DEPLOY-BLOCKING for the live demo).** Found during the Phase-4 teardown (2026-06-21). In the assembled preview, `DevContainer.fetch()` injects `appVersion: 'dev'` into the shell, but `DevStudio.applyOntology` installs the ontology under a **content-hash** version (`git.hashBlob` of the `.d.ts` — required so the Star's Worker Loader `bundleId = galaxyId/version` cache never serves a stale validator). So the client always sends `'dev'`, the Star's current version is a 40-hex oid → **mismatch → `OntologyStaleError` on every live op**. Deploy-gated (can't surface under vitest-pool-workers; tests use explicit matching version labels, so they're unaffected). **✅ DESIGN RESOLVED 2026-06-21** — canonical in [nebula-dev-flows.md](nebula-dev-flows.md) **Decision 12 + Flow 1d**; build = [nebula-studio.md](nebula-studio.md) **Phase 5**. Resolution: inject the *real* content-hashed version by **push** (`DevContainer.setAppVersion`, replacing `'dev'`) + re-sync via the kept reload channel triggered from `Star.#installState` on `isNewVersion` — **not** a `.dev`-special branch in the hot path (Decision 11 holds). **NOT** deferred to the agentic engine after all (earlier steer superseded — it's a clean, mostly-testable Phase 5). Prerequisite for the first working live preview. Also tracked in `nebula-studio.md` Phase 5 + [[project_studio_uibuild_pivot]]. **✅ BUILT 2026-06-21** (`/build-task` Phase 5; affected suites green, the container round-trip = deploy-gated `it.skip`).
