...
- [ ] **Tester data-share consent flag (per-Galaxy) — build at tester-onboarding.** Each F&F tester gets their own Galaxy; their Studio prompt/turn data is captured in their **Galaxy DO's SQLite** (the recorder corpus). At the Galaxy-slug signup prompt, surface a notice (now: "we use your prompt data for feedback"; later: "willing to share data to improve Lumenize Nebula?"). Persist a per-Galaxy `shareConsent` flag; "the testers we may use" = the consented set. **Assume `true` for now** — not built yet. **Open: where the flag lives.** The only central instance index is `NebulaAuthRegistry`'s flat `Instances` table (`packages/nebula-auth/src/nebula-auth-registry.ts`), but that's auth *infrastructure* and `shareConsent` is a Nebula-*product* policy — same layering smell as the rejected `dev`-slug reservation in nebula-auth (2026-06-21). Decide at build time: (a) pragmatic — a `shareConsent` column on `Instances`; (b) layered — track consent at the Nebula-platform layer keyed by galaxy slug, set by the signup prompt (Nebula-app code). Leaning (b). Surfaced 2026-06-22 when punting the R2 store in favor of the Galaxy DO.
