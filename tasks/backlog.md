...
- [ ] **Nebula email sender domain — fix the broken default + switch to `nebula.lumenize.com` once CF subdomain email lands.** The `NebulaEmailSender` default `from` is `auth@nebula.lumenize.com` (`packages/nebula-auth/src/nebula-email-sender.ts:35`), which is **NOT verified** for CF Email Sending — CF **silently drops** it (confirmed live, `tasks/nebula-scratchpad.md`, 2026-04-28; documented in the nebula browser harness at `apps/nebula/test/browser/worker/index.ts:49-54`). **(a) Footgun:** any deploy that doesn't override `AUTH_EMAIL_FROM` silently drops all mail → change the default to the verified `noreply@lumenize.io`, *or* make a missing/unverified sender fail loudly (pairs with the scratchpad's "surface email-send failures" item). **(b) Misleading comments:** the *mesh* test harness calls `auth@nebula.lumenize.com` "a verified sending domain" in 3 spots (`packages/mesh/test/browser/worker/index.ts:40`, `packages/mesh/test/browser/README.md:60`, `packages/mesh/test/browser/ws-roundtrip-browser.test.ts:10`) — **false**; correct them (they pass only because those run in test-mode and never actually send). **(c) The actual end state:** the app origin + JWT issuer are `nebula.lumenize.com`, so the **sender** should match it too (brand + DMARC alignment — a From-domain matching the app authenticates more cleanly). Pre-alpha sends from the verified `lumenize.io` (`noreply@lumenize.io`) as a stopgap. Cloudflare has been adding **subdomain support for Email Routing/Sending** — once it lands, verify `nebula.lumenize.com` and switch `AUTH_EMAIL_FROM` → `noreply@nebula.lumenize.com`. Surfaced 2026-06-25 (deploy-task review, Larry).
