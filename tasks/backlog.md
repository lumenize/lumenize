...
- [ ] **A `wrangler dev`(+Docker) / prod e2e + smoke lane** *(LOCAL half drafted 2026-06-24 → Task ② `tasks/nebula-local-smoke.md`; the **prod** UI-smoke half rides the deploy task ③)* — two related deliverables that share the same wrangler-dev-backed harness (the `browser`/`chromium` projects already auto-spawn `wrangler dev`; `global-setup.ts` already supports a `BENCH_BASE_URL` override pointing at a *deployed* worker). Surfaced 2026-06-24 during `/review-task` of `tasks/nebula-release-process.md`.
  - **(A) Promote the environment-gated codegen/Container `it.skip`s to `it.runIf(<gate>)`.** Today these e2e skips live in the **pool-workers** projects (`dev-studio`, `container`), skipped because pool-workers can't provide `env.AI` or construct a `Container`. A real `wrangler dev` can (Container ones also need Docker Desktop up). Work = a `wrangler dev`-backed project (bring up Docker in `globalSetup` for the Container tests), **relocate** these assertions into it, gate via env flags (`HAS_DOCKER` / `HAS_CF_CREDS`) — idiomatic, matches existing `describe.runIf(BENCH_BASE_URL)` / `it.runIf(MULTI_CLIENT_STRESS)`.
    - **Candidates (deterministic → unskip):** `container-node/dev-container.test.ts:135/146/160` (fetch 3-way + applyChanges round-trip, version contract, cold-boot re-push), `dev-studio/dev-studio.test.ts:226` (syncToDevContainer push), `dev-studio/codegen-loop.test.ts:389` (Workers-AI `json_schema` capability probe). Classify `dev-studio.test.ts:234` (applyOntologyChange ordering — no env note). `dev-studio.test.ts:276` (SFC mounts non-blank, m3) belongs in `chromium`.
    - **Stays skip-by-default** even when runnable: `dev-studio.test.ts:253/264` (the **live `chat()` turn** — non-deterministic live LLM, costs money, "not a deploy blocker").
  - **(B) A small (≤5) TRUE UI-level smoke suite (Playwright drives the rendered Studio), runnable against BOTH `wrangler dev` AND prod.** This is **net-new** — nothing today drives the real Studio UI. `test/browser/smoke.test.ts` is **API-level** (`NebulaClient` JS + fetch/cookie-jar `Browser`, no DOM); the `chromium` project is real-Chromium but only mounts **synthetic** mini-components (`document.body.innerHTML=…` + `Vue.createApp`) for reactivity tests. A UI smoke instead navigates the rendered Studio and **uses the UI**: minimal bar = **log in through the UI** (fill email, click send, complete the magic-link via the deployed `email-test` worker loop, land back) + **assert the key Studio UI elements render** (chat/app shell); richer = a ≤5 multi-step journey (set up state via the UI → check → manipulate → recheck). Use the `chromium` Playwright project + the same-origin proxy (`testing.md` § Real-browser tests).
