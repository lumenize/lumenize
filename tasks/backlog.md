...
- [ ] **`npm deprecate @lumenize/state` — it WAS published (`@lumenize/state@0.26.0` is live on npm, confirmed 2026-06-15).** The package source was deleted from the monorepo in nebula-frontend v3/P11 (`045d2ac`, superseded by Vue `reactive()`). Since it's published, deleting the source isn't enough — run `npm deprecate @lumenize/state@"*" "Superseded by Vue reactivity (@vue/reactivity) inside @lumenize/nebula/frontend; no longer maintained."` so npm consumers see the deprecation notice. (Unpublish is discouraged + blocked >72h after publish; `npm deprecate` is the right mechanism.) Only one version (0.26.0) to deprecate. Do at/before the next release.
- [ ] **(ADR-002 gap, surfaced by the nebula-frontend P10 rich-type probe — root cause narrowed 2026-06-15) A cyclic resource value is rejected by server-side ontology validation.** **NARROWED via a two-case probe: it's the CYCLE itself, NOT the type complexity** — (CASE-A) the FULL `interface R { label; when:Date; counts:Map; tags:Set; self?:R }` with a NON-cyclic nested `self:{…}` VALIDATES; (CASE-B) a MINIMAL `interface R { label; self?:R }` with a CYCLIC `v.self=v` FAILS (`$input.self expected "(string | undefined)"`). So typia's type-generation is correct (it accepts a non-cyclic `R` for `self`); the cyclic VALUE arrives at the validator in a non-`R` form. **Prime suspect: `@lumenize/structured-clone`'s "super JSON format"** (the JSON-merge-patch-friendly W4 format) round-trip not reconstructing the cycle as a real in-memory object reference before typia validates server-side — the `expected "(string | undefined)"` smells like typia seeing an unresolved `$ref` *string* placeholder where it expects `R`. **Second suspect:** the typia visit-tracking fork (cycle support shipped, `tasks/icebox/typia-visit-tracking.md` Phase 2) not engaging in the ontology-bundle-generated validator. NOT a JSON.parse/stringify degradation (Date/Map/Set survive intact). **Definitive next diagnostic:** log the postprocessed value's `self` identity (`value.self === value`?) at the `apps/nebula/src/resources.ts` validation site — broken cycle there ⇒ structured-clone super-JSON decode; intact cycle but still rejected ⇒ the fork. Repro: the `it.skip`'d "a cyclic value survives…" test in `apps/nebula/test/test-apps/baseline/rich-type-roundtrip.test.ts`. Low-pri (cyclic resource values are rare) but an ADR-002 conformance gap. A `create`/`put` whose value contains a cycle (`value.self = value`, type `interface RichResource { …; self?: RichResource }`) resolves `'rejected'` (validation): the validator reports `$input.self expected "(string | undefined)"` and the cycle is not short-circuited. **Isolated:** (a) Date/Map/Set resource values round-trip fine over the real wire (the W4 surface preserves them — `rich-type-roundtrip.test.ts`), (b) a NON-cyclic nested `self: { … }` of the same recursive type validates, (c) Nebula validation uses `@lumenize/ts-runtime-parser-validator`, which DOES bundle the visit-tracking typia fork that ships cycle support (`tasks/icebox/typia-visit-tracking.md` Phase 2). So the gap is specific to **cyclic values on the ontology-validation path** — likely a recursive-type-extraction quirk (self resolving to `string` when combined with Date/Map/Set siblings) or the fork's visit-tracking not engaging in the runtime-generated ontology validator. Repro: the `it.skip`'d "a cyclic value survives…" test in `apps/nebula/test/test-apps/baseline/rich-type-roundtrip.test.ts` (assertions intact). **Low-pri** (cyclic resource values are rare), but it's an ADR-002 ("every surface round-trips the full structured-clone value space, incl. cycles") conformance gap — worth closing or explicitly documenting the cyclic-value limitation. Investigate whether the ontology validator build actually engages the fork's per-call visit-tracking; cross-check against `tasks/icebox/typia-visit-tracking.md`.
