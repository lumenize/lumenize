...
| **D7** | **A minted token never exceeds the caller's own access.** Wearing another identity's `sub` requires administering that identity's scope (`hasAdminOverScope`), and the token carries the **caller's** `profileId`, never the target's. | Leaving either unbounded — today any admin can mint for any identity in **any universe**, and the minted token carries that person's `profileId`, which is write access to their profile (S1's owner check, zero reads). Mechanism, rename and phases: **[nebula-mint-narrower-token.md](nebula-mint-narrower-token.md)**. |
