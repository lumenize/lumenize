...
- **Known gap deferred to v3 wiring**: property writes on objects retrieved FROM collection entries (`map.get('k').name = 'x'`) don't path-route through the wrapper — Vue's instrumentation hands back its own reactive, not our path proxy. Mutating the collection itself syncs; mutating an entry's interior does not. Decide treatment during the v3 port (recurse the wrapping through instrumented `get`, or document as assign-the-entry). **v3-port obligation (Mn9):** whichever treatment is chosen, pin it with a **capable-of-failing probe** — either `map.get(k).field = v` syncs (recursion chosen) OR it does NOT sync **and** emits a `@lumenize/debug` warning (documented-as-footgun chosen) — so this sub-case can't ship as silent local-only divergence (the M10 failure mode this detour exists to close). **Lean:** document-as-footgun + warning for v3 (recursion through Vue's instrumented `get` is the larger change); the warning makes the leaky-abstraction edge visible rather than silently broken. Revisit recursion if the warning proves noisy in real apps.
