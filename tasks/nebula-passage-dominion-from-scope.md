...
| *(assembled inline in the mint handler)* | `canMintFor(callerClaims, subject, activeScope)` | verdict | the only **policy composed from** the predicates rather than being one; named so the passage-for-dominion substitution stops being an available edit — § *Constraints* |
...
- **`security.md`** — the four mint-side invariants for `/mint-narrower-token` are written in pattern terms and must be restated in scope terms. ⚠️ **The binding rule, stated once because the faithful-looking translation is the dangerous one: the mint's bounds are the AUTHORITY predicate — `scopeAdmin` ∧ scope-at-or-above — never the passage one.** Minting confers dominion, and dominion is downward-only (ADR-015 clause 1); passage may run upward and grants nothing there (clause 3), so it can never widen a mint. Restated in scope terms the four are:
  ...
  ⚠️ **Read (a) or (b) as "within X's *passage*" and a `{u}.{g}` admin impersonating a `{u}.{g}.{s}` subject can mint `activeScope = {u}` — a universe-admin token.** Today only (b) blocks that, and (b) is written entirely over `buildAuthScopePattern`, which this task deletes. `docs/vision/auth.md` § *Impersonation*: impersonation "is never an escalation."
  - 🔒 **The four compose into ONE named predicate, `canMintFor(callerClaims, subject, activeScope)`, and that is the point rather than tidiness.** The warning immediately above is a **guard where a structural impossibility belongs** (`calibration.md` §2). While the four are assembled inline in the handler, "use passage here instead of dominion" is a one-token edit that type-checks, reads plausibly, passes every behavioural criterion whose scopes do not happen to expose it, and mints a universe-admin token. Behind a named predicate it is not an edit a caller *can* make: the handler asks one question, the answer composes dominion the way this file pins, and substituting the wrong verdict means editing the predicate itself — a diff no reviewer reads past.
  - ⚠️ **YAGNI does not apply, and the reason is worth stating because the reflex says otherwise.** There is one caller, so the usual extract-for-reuse argument is absent — but `workflow.md` § *YAGNI gates capability, never generality* gates **capability**, not shape, and this adds none: same four checks, same one call site, same behaviour. What it buys is that the file's single most dangerous mistake stops being available. It is also the **only** policy in this work composed *from* the predicates rather than *being* one, so it is the one place the "no site re-derives" objective would otherwise stop short of the spirit ADR-007 and ADR-015 are written in.
  - **Where it lives:** with the mint, in `nebula-auth` — not exported to `apps/nebula`, which has no business minting. Its four limbs each delegate (`hasDominionOver` for (a) and (d), `isAtOrBelow` for (b)'s structural mirror, the subject's bit for (c)), so it composes the predicates rather than re-deriving them.
...
- 🔒 **The mint asks ONE question.** `/mint-narrower-token`'s handler calls `canMintFor(callerClaims, subject, activeScope)` and contains no containment check of its own; the four invariants live inside it, each limb delegating to `hasDominionOver` or `isAtOrBelow`. *Reds against re-inlining any limb — including the redundant (a) gate, whose removal must not quietly take its `insufficient_scope` code and caller-facing message with it.* ⚠️ **Assert the SHAPE, not only the behaviour**: an inline re-assembly satisfies every other mint criterion here while restoring the exact edit this predicate exists to foreclose.
