...
> ⚠️ Shipped 5.3.3 shape — superseded by [api-reference.md](../website/docs/nebula/api-reference.md) for the v3 surface (`subscribe` → `Disposable` `ResourceSubscription`, `ontologyVersion` → `appVersion` in user-facing options). Kept for the handler-plumbing picture; the docs are the contract.
...
client.resources = {
  ...
  transaction(
    ops: Record<string, OperationDescriptor>,                  // keyed by resourceId
    options?: {
      ontologyVersion?: string;
...
interface SnapshotMeta {
  ...
  /**
   * Org/permission-tree node the resource is attached to. Always present on
   * app resources; absent on framework resources like ('lmz', 'orgTree').
   * Load-bearing for the Atomic-append pattern (`listSnap.meta.nodeId`).
   */
  nodeId?: number;
...
type TransactionResourceResolution =
  ...
  | {
      ...
      server: Snapshot;
      base:   { value: unknown; eTag: string };           // common ancestor — the value `local` was based on; required for a correct 3-way textMerge (added 2026-06-10, deep-review B4)
...
- [x] In-flight transactions during disconnect — left as-is (10s timeout resolves with `'timeout'`; app retries via the always-resolve discriminated union; server idempotency short-circuits on `newETag`). Pausing the timer while disconnected is premature optimization. **⚠️ Superseded for v3 (2026-06-11):** the "app retries" assumption breaks for the v3 synced-state-middleware write path — a debounced `v-model` transaction has no app await-site, so a `'timeout'` rollback silently reverts text the disconnected banner promised was "queued." v3 **connection-gates** the write path: while `lmz.connection.state !== 'connected'`, flush + the timeout/infra-error timers are suspended; a disconnect never rolls back; on reconnect, in-flight transactions re-submit (same `newETag`, idempotent) and held writes flush. Owned by [debounce-serial-queue.md](debounce-serial-queue.md) (flush gate) + [factory-conflict-outcome.md](factory-conflict-outcome.md) invariant 10 (rollback gate) + invariant 8 (reconnect snapshot held). The original pin was correct for app-submitted transactions; it's the framework-submitted path that needs the gate.
...
| Flash-class wiring | **`getBindings` option.** Headless tests pass nothing — flash is a no-op. Default `lumenize-conflict-revert` flash applies on **all** rollback outcomes (`'use-server'`, `'validation-failed'`, `'permission-denied'`, `'retries-exhausted'`); `'committed'` flashes `lumenize-commit-success`. (Superseded the earlier use-server-only sketch.) |
...
- **Rollback failure-outcome tests** ✅ in-scope work shipped 2026-06-04 — see [archive/validation-failed-rollback.md](archive/validation-failed-rollback.md). Root cause was live-reference capture in `#middlewareFn` (fix: `structuredClone` at capture site). Roll-back outcomes covered: `validation-failed`, `permission-denied`. **`ontology-stale` reclassified to NO-rollback (2026-06-11, review-round-3):** it's a "your client is stale, reload" signal, not a per-write rejection — `#processMiddlewareOutcome` now treats it as a no-op (optimistic state stays painted; the response is `onShouldRefreshUI` → reload), matching the documented contract (api-reference § TransactionOutcome; factory-conflict-outcome invariant 3). The bindtostate test was flipped to assert no-rollback (capable-of-failing). Deferred roll-back siblings (`timeout`, `retries-exhausted`) tracked in [nebula-scratchpad.md](nebula-scratchpad.md) § "Rollback failure-outcome sibling tests (deferred)".
...
| **Factory before `connect`** ~~order-invariant~~ → **superseded by replay (deep-review m2, 2026-06-11)** | ~~`createNebulaClient(...)` must be called BEFORE the connection resolves~~ — replaced by the "Replay current state immediately" decision below (and in the 5.3.6 pinned table): the factory **captures the client's current connection state at creation** and tracks transitions via its observer, so registration order is irrelevant and `lmz.connection.*` is always correct. The old order-invariant was a footgun for the Studio LLM; replay removes it. (No security relevance — `lmz.connection.*` is cosmetic UI status, not an auth boundary.) | Replay is ~5 LOC and strictly more forgiving than the order-invariant; the earlier "adds API surface" objection didn't pan out. |
...
- [ ] **Rewrite template-string-dependent doc chapters in [coding-your-ui.md](../website/docs/nebula/coding-your-ui.md) against `.vue` SFCs.** *(Appears done in the working tree — TodoForm.vue/OrgTreeNode.vue/App.vue are SFCs; no "Component tag case" bullet or runtime-compiler discussion remains. Verify and check off.)* Affected sections (current titles): "Recursive components" (template-string TreeNode example becomes an SFC), the org-tree worked example ("Worked example: rendering the built-in tree" — template strings → SFCs throughout), the conflict-modal in "Optimistic concurrency" / "use-this verdict — async modal," the form patterns in "Forms: explicit save." Also remove the "Component tag case" bullet (kebab-vs-Pascal rule) and the runtime-compiler discussion (no in-DOM compiler when Galaxy / dev Stars pre-compile).
...
#### Phase 5.3.7-v3 — Scaffold + factory + Vue integration (estimate revised — see below)

> **Estimate (deep-review m14):** the original "~2 days" is stale — it predated (a) the four isolation detours now front-loaded (debounce / textMerge / collection-sync / conflict-outcome, each built+tested before wiring) and (b) the nebula-auth global-refresh-endpoint work the "all config optional" item hides. Treat v3 as **detours (built green in isolation) + wiring**, not a single 2-day push. Don't quote a single number; track by the detour + wiring checklists.

Implements the API surface fixed by v1's doc rewrite. The doc is the spec; this phase makes the doc true.

> **▶ BUILD KICKOFF (2026-06-11) — these are the active work now.** The round-3 review is fully closed (all decisions resolved, committed `7221c92`); we paused the review walkthrough to knock off these detours, because collapsing each detour-spec into tested code shrinks the source-of-truth space (shipped code ∥ spike ∥ docs ∥ task files) every later decision has to explore. **Build in spike code** (not the real `packages/nebula-frontend` package yet, and not shipped `apps/nebula/src`) — the production port into the package is still v3. Order below; **start with textMerge** (pure, zero-dep, the dependency root). For a pure fn the property tests ARE the isolation harness (no mock-client needed). Each completed detour → its spec file collapses into "implemented + tested in spike." Build state: don't touch the uncommitted `packages/mesh/src/lumenize-client-gateway.ts` (inert mid-fix for the deferred `disconnect-cleanup` red test, task_3e018327 — orthogonal to these client-factory detours). The detours' designs already bake in this review's resolved decisions (B4 base unifying rule, connection-gating, ontology-stale no-rollback, M9 fall-through, M10 collection-sync) — read each detour file for the pinned invariants.

**Pre-v3 isolation detours (build + property-test in isolation BEFORE the v3 wiring below).** Pattern established by [debounce-serial-queue.md](debounce-serial-queue.md): the invariant-heavy cores are cheaper and safer to validate against a mock-client harness than to debug inline once everything is wired. Build these first, then v3 becomes "compose validated cores," not "write gnarly logic inline." Dependency order:

1. **Pure helpers** — [factory-textmerge.md](factory-textmerge.md) (3-way LCS merge) + `deep-equals`. Pure functions, no deps; build first (the conflict/outcome `@longform` auto-resolver depends on textMerge).
2. **Write-path** — [debounce-serial-queue.md](debounce-serial-queue.md) (D0/D1, exists) + [factory-collection-sync.md](factory-collection-sync.md) (M10 Map/Set mutator interception). Share one harness (mock `client.transaction` capturing submitted tuples) — same write-capture→submission path.
3. **Conflict/outcome state machine** — [factory-conflict-outcome.md](factory-conflict-outcome.md): the `TransactionOutcome` + `TransactionResourceResolution` engine (mixed-fate, per-resource rollback, `use-this` chain + `maxRetries`, `base` threading B4, handler fall-through M9, async-resolver-suspends-timeout, hold-pending-fanouts). Depends on (1) textMerge + (2) the submission queue.

Already spike-validated (no fresh detour — property-test during the port): refcount + `effectScope` auto-subscribe + grace (spike Q4 exactly-once-unsubscribe + the scope-bridge). Mechanical (no detour): subscribe→`Disposable`, eTag auto-derive, connection-state replay, `appVersion` rename.

**Prerequisite**: all four isolation detours — [debounce-serial-queue.md](debounce-serial-queue.md) (D0/D1), [factory-textmerge.md](factory-textmerge.md), [factory-collection-sync.md](factory-collection-sync.md), [factory-conflict-outcome.md](factory-conflict-outcome.md) — complete (green in isolation) before this phase's wiring starts. Their production ports ARE part of v3 — the work below assumes validated designs to translate. Also prerequisite: [nebula-star-root-admin.md](nebula-star-root-admin.md) **Part 1** (founder admin-on-ROOT seeding) — the flagship bootstrap, its two-client race test, and coding-your-ui's "the founder gets one — admin on root — at Star creation" claim all hard-depend on it.
...
- [ ] **Spike-port cleanup** (must address during the port, NOT after):
  ...
  - Seed `lmz.connection = { state: 'disconnected', connected: false }` initially (at [create-nebula-client.ts:82](../apps/nebula/spike/alpine-adapter/src/create-nebula-client.ts:82)) so first-paint reads aren't `undefined`. `lastConnectedAt` stays unset until first connect. Also pre-seed `ui: {}` and `app: {}` (app-owned conveniences, not reserved — first-level `store.ui.*` access works with no init; pinned 2026-06-10 in api-reference § Reserved state paths after the docs-panel review found every `store.ui.*` example otherwise throws).
...
- [x] **Narrow `claims` to non-null on NebulaClient (deep-review m12; implemented 2026-06-11 via generic `LumenizeClient`).** ✅ DONE. The originally-pinned "type-only override" is structurally impossible (TS has no body-less accessor override → TS2610; and `Readonly<NebulaJwtPayload>` isn't assignable to base `Readonly<JwtPayload> | null` because `JwtPayload` requires `emailVerified` that `NebulaJwtPayload` lacks → TS2416). Implemented option (a) instead (review-round-3): **`LumenizeClient` is now generic over its claims payload** — `abstract class LumenizeClient<TClaims extends { sub: string } = JwtPayload>` with `get claims(): Readonly<TClaims> | null` ([lumenize-client.ts:373/483](../packages/mesh/src/lumenize-client.ts:373); `#claims` typed `Readonly<TClaims>`, assigned through a documented trust-boundary cast at the two parse sites). The default `= JwtPayload` keeps every other subclass unchanged. **NebulaClient extends `LumenizeClient<NebulaJwtPayload>`** and re-declares the getter as `get claims(): Readonly<NebulaJwtPayload> { return super.claims as ... }` ([nebula-client.ts:227](../apps/nebula/src/nebula-client.ts:227)) — a **behaviorally-neutral re-declaration** (the runtime getter is the inherited one; this only drops `| null`), justified by the `ready` contract. Verified: mesh + nebula `tsc --noEmit` clean; a negative probe confirmed `NebulaClient.claims.sub` compiles non-optional while base `LumenizeClient.claims.sub` still errors `TS18047 possibly null`; mesh client tests 55/55. **Predicted TS-error cleanup didn't materialize** — dropping `| null` only widens guarantees, so existing `client.claims?.`/`=== null` sites stay legal (merely redundant); no consumer/test mis-typed. Pinned in api-reference § client.claims. Code that runs *before* `ready` (admin/scripting) still guards with `?.` explicitly.
...
- [ ] **Org/permission tree delivery — DEDICATED CHANNEL (design B; decided 2026-06-11 after adversarial verification; supersedes "Option 0: orgTree-as-a-resource").** The tree is **not a resource**: it's mutated incrementally via `client.orgTree.*` (never `transaction()`, which sends a whole value) and read reactively at **`store.lmz.orgTree`** — under the framework-reserved `store.lmz.*` prefix (next to `store.lmz.connection`), **NOT** `store.resources.*` (which stays purely user data). It never enters the resource `Subscribers`/`Snapshots` tables. This **dissolves the entire M8 reserved-namespace cluster** (no reserved rids, no transaction guarding, no `lmz`-type guard — an app may freely use any resourceId incl. `'orgTree'` and any type name incl. `lmz`) and **collapses M2/M3/M4** into one cohesive tree module. Verified across 6 adversarial probes (`tasks/` round-3 verification, 2026-06-11): nothing requires the tree to be a resource (`read('lmz','orgTree')` had zero callers), the dedicated `clientId`-keyed registry structurally can't collide with the resource `Subscribers` table, the store move is clean (the synced-state middleware's `^resources\.` regex never matches `lmz.*`, so NO `rt==='lmz'` carve-out is needed), no new exposure beyond M7, and the channel is net-smaller than the special-cases it replaces. The historical design-space record is § DAG-tree-as-special-resource (Option 0 — reconsidered & replaced, kept for history). Resolves deep-review B2/M2/M3/M4/M8/M14.
...
- [ ] Carry-forward tests: all tests under `apps/nebula/spike/alpine-adapter/test/**` (enumerate with `ls`/grep at port time; ~32 at time of writing — Phase 0a factory-basics + Phase 0b e2e (smoke, transaction-roundtrip, cross-client-fanout) + Vue probes Q1–Q5) across unit + e2e + browser projects.
...
- [x] **Expose `client.claims` on LumenizeClient** ✅ SHIPPED (deep-review m4 — verified 2026-06-11: `#claims` field + frozen getter + populate-on-refresh in [lumenize-client.ts:384/432/689/930](../packages/mesh/src/lumenize-client.ts:384); mesh docs + tests done). Residual: optional `calls.mdx` / `security.mdx` cross-ref sweep. Original item (mesh-level prerequisite for the per-user-keying task) below for history: NebulaClient inherits via `extends LumenizeClient`; Nebula examples use `client.claims.sub` for per-user-keyed resource lookups. Decision rationale captured during 2026-05-19 design discussion:
  ...
  **Doc updates in `@lumenize/mesh`:**
  - [website/docs/mesh/lumenize-client.mdx](../website/docs/mesh/lumenize-client.mdx) — add a `client.claims` section. Position alongside other public accessors / properties.
...
- [ ] **Mid-edit fanout contract — ship `textMerge` + hold-pending-fanouts in v3 (decided 2026-06-05).** *Design + isolation tests: [factory-textmerge.md](factory-textmerge.md) (the merge fn) + [factory-conflict-outcome.md](factory-conflict-outcome.md) (invariant 8, hold-pending-fanouts).* Server fanout arriving while a user is mid-typing will visually clobber their in-progress text under the default `'use-server'` resolver. This is by-design (Nebula's optimistic + server-is-truth model), but the user-visible UX is bad for text fields. Recommendation captured in [coding-your-ui.md § Concurrent edits and long-form text](../website/docs/nebula/coding-your-ui.md) and [resources.md § Text fields specifically — don't leave the default](../website/docs/nebula/resources.md#text-fields-specifically--dont-leave-the-default). Two pieces, both shipping in v3:
  - **`textMerge(server, local, base)` helper** from `@lumenize/nebula-frontend`'s top-level export — 3-way LCS-based merge (~100–300 LOC, or pull in `diff-match-patch` if licensing allows). Auto-registered for `@longform` fields by the annotation→resolver compile pass (see [ontology.md § Annotations](../website/docs/nebula/ontology.md#annotations)); hand-written handlers only for custom merge logic. Tagged `new-in-v3` in [api-reference.md § textMerge](../website/docs/nebula/api-reference.md#textmerge). Test: simulate concurrent-edit conflict on a `@longform` field, assert both non-overlapping edits preserved.
    - **`base` in the `'conflict-pending'` payload (deep-review B4, 2026-06-10; capture-point corrected 2026-06-11 after adversarial verification).** A correct 3-way merge needs the common ancestor; the payload carries `base: { value, eTag }` (api-reference § TransactionResourceResolution) — without it the only expressible merge is `textMerge(server, local, base=server)`, which collapses to local-wins and silently drops the server edit. **Unifying rule: `base` is always the value at the baseline eTag the next submission uses — it re-anchors whenever that baseline advances.** The factory holds the value at each anchor in browser memory (the same snapshot it already keeps for optimistic rollback); three re-anchor sites:
      1. **First divergence** — on the *first* optimistic write of an edit burst, `base` = the last-confirmed value the store held *before* that keystroke overwrote it. NOT "at transaction-submit time" — by submit time the store already holds the user's edits, so a submit-time capture yields `base === local` and silently re-introduces the degeneration.
      2. **Commit boundary** — when a buffered write chains onto a cleanly-committed prior transaction (`InFlightWithBuffer → InFlight`, [debounce-serial-queue.md](debounce-serial-queue.md)), `base` re-anchors to that transaction's just-committed value (the value it submitted), riding alongside the eTag-chaining the queue already does. Without it, sustained typing across a commit — the common `@longform` case under `maxWait` — double-counts the user's own already-committed text into the merge's overlap/garble path.
      3. **`use-this` re-conflict** — `base` becomes the previous conflict's `server` snapshot (the value the re-submit was based on), NOT the original pre-write.
      **`base` is never a server-side history lookup — and it *cannot* be** (verified 2026-06-11): the same-actor in-place debounce overwrite ([resources.ts](../apps/nebula/src/resources.ts) `#writeSnapshot`) destroys intermediate eTags' values, so a second client's exact divergence point is frequently unrecoverable from history (a server backscan finds nothing, or the surviving row IS the conflicting `server` value → the `base===server` degeneration). The merge runs on the client against values the client already holds; the server's conflict response carries only its *current* snapshot (`server`). This keeps conflict resolution decoupled from history retention, so the R2 history offload ([nebula-resource-history-r2.md](on-hold/nebula-resource-history-r2.md)) never intersects the merge path (ADR-004 corrected to match — history supplies at most the last *meaningful* version, not the divergence point). The auto-registered `@longform` handler must merge against `base.value`, not `server.value`. Tests (capable-of-failing, all client-side → land with the v3 factory port): (a) concurrent-edit conflict where BOTH sides changed the field → both edits survive (a `base=server` impl drops the server edit); (b) **first-divergence** — type via the middleware path, conflict → `base` equals the pre-typing value (a submit-time-capture impl yields `base===local`, fails); (c) **commit-boundary re-anchor** — type, let T1 commit, keep typing (buffered), a third party commits, T2 conflicts → both the post-commit local edit and the third party's edit survive (a no-re-anchor impl carries the stale original `base` and garbles, fails).
...
- **Additional merge helpers beyond `textMerge`: `setUnion`, `counterMerge`, etc.** — `textMerge` + hold-pending-fanouts moved INTO v3 (decided 2026-06-05 — see the "Mid-edit fanout contract" item under v3 correctness above; tagged `new-in-v3` in [api-reference.md § textMerge](../website/docs/nebula/api-reference.md#textmerge)). The remaining sibling helpers stay deferred: ship them from `@lumenize/nebula-frontend`'s top-level export so per-type handlers can call them from a `'use-this'` verdict. This resolves the design question of "should we add a `'crdt'` resolution kind?" — answer: no, `'use-this' + helper-fn` is the right factoring (helpers don't add framework primitives; users opt in per-type). `setUnion(localSet, serverSet)` and `counterMerge(local, server, base)` follow when the demo or a real app needs them. **For true CRDT-backed real-time collab, see the next bullet — different scope entirely.**
...
7. **DAG-tree-as-special-resource (reactive DAG binding).** ⚠️ **SUPERSEDED 2026-06-11 by design B (the dedicated-channel "Org/permission tree delivery" checklist item under Phase 5.3.7-v3).** This Option-0 reasoning — "reuse the resource-subscribe plumbing" — was reconsidered: the reuse wasn't free (it required the M2/M3/M4 special-cases + the M8 reserved-namespace cluster to stop app resources colliding with the fake resource), and adversarial verification (2026-06-11) showed a *separate* `clientId`-keyed tree channel is net-smaller AND dissolves M8 entirely. The "don't build a parallel `client.dag.subscribe()`/separate table" instinct was the wrong call *for this case*: the tree genuinely isn't a resource (incremental-mutation API, universal visibility, synthesized-not-stored), so a dedicated channel is the honest model. Kept below as the design-space record. The DAG tree is conceptually a resource — clients eventually want to bind UI to it (`{{ store.resources.lmz.dag.value.nodes[42].label }}`, tree-view components, reactive permission badges). The (then-)cleanest design **reuses the resource-subscribe plumbing** rather than building parallel `client.dag.subscribe()` / `handleDagUpdate()` / a separate `DagSubscribers` table — the whole consolidation work in 5.3.1 (one Subscribers schema, one fanout path) was about not making exactly that mistake.
   ...
   **What needs synthesis** (~20–30 LOC, contained):
   - **`Snapshot` shape for DAG**: `value = dagTree.getState()`, `meta.eTag = crypto.randomUUID()` per mutation (or monotonic counter), `meta.validFrom`, `meta.deleted = false`. ⚠️ **Corrected 2026-06-10 — see the authoritative v3 checklist item above (`('lmz', 'orgTree')` delivery).** Two details in this historical sketch are now WRONG: (1) `meta.typeName` must be `'lmz'` (the rt), NOT `'DagTree'` — `Star.#broadcast` routes the client store path off `typeName`, so `'DagTree'` would land the snapshot at the wrong path; (2) the check-skip is keyed to the **exact `(lmz, orgTree)` tuple via a resourceId allow-list**, NOT a `lmz.*` prefix skip — a prefix skip is a cross-user read bypass. The resourceId is `orgTree` (purpose-named), not `dag`.
   ...
   **Phase triage**: ~~don't implement DAG-binding yet; wait until the Studio demo (or a real app) actually wants a reactive DAG view~~ **superseded 2026-06-10, design settled 2026-06-11**: the v1 docs make the tree worked example a flagship (coding-your-ui § Worked example) and the first-run container-create bootstrap reads it, so tree delivery is pulled into v3 scope — see the authoritative **Org/permission tree delivery (design B, dedicated channel)** checklist item under Phase 5.3.7-v3 (client surface `client.orgTree` / `OrgTreeState`, read at `store.lmz.orgTree`; the tree is delivered on its own channel, NOT as a resource). The scale-out decision (the old Options A/B/C below — different axis: how the *broadcast* fans out at scale) stays deferred until a real workload exposes it. The exploration below (Options A–D) is historical design-space record — treat the v3 checklist item as authoritative for the pinned details.
