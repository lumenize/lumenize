...
- **The invite mechanism** → [nebula-auth-identity-mint.md](nebula-auth-identity-mint.md). Invites keep their scoped links.
...
- **Touches** [nebula-auth-identity-mint.md](nebula-auth-identity-mint.md) only at the boundary: invites keep scoped links, so its mechanism is unaffected by the scope-less default.
