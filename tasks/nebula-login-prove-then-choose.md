...
- **Nothing durable changes shape, so this is not wipe-gated.** `MagicLinks` rows are ephemeral, the signup ticket is a cookie, mint-all writes existing row types, and the acceptance-at-first-refresh flip updates an existing column.
...
| **`acceptedAt` flips at a membership's FIRST successful refresh — one writer; the consume-time flip is removed.** The refresh presents that membership's own path-scoped cookie, so the proof is self-carrying and no cross-membership rule exists. Acceptance then means what ADR-012 wants — the person actually entered the scope — and it raises a bar that is lower today than believed: corporate scanners prefetch links (`consumeAndLogin`'s JSDoc — why links are multi-use), so today a guessed-address invite is "accepted" by the victim's own mail gateway with no human act; a fetch-and-follow scanner mints cookies it discards and never refreshes, so under this rule nothing flips. Mechanics: the KV refresh record gains one boolean at mint; a hit with it false fires a `waitUntil` registry flip + KV rewrite — once per membership, off the response path, zero-write steady state kept. The manufacture test re-pins so restoring a consume-time flip reds, and the column's other reader — the unfinished-claim test in the contested-slug resolution (`acceptedAt IS NULL`) — keeps its meaning: a scanner-consumed claim link now correctly stays unfinished. Unaccepted rows render with an invitation badge — display only, the click is ordinary navigation. Accepted residue: a JS-rendering detonation sandbox could still flip the one invited scope; the named escalation is the consume JSDoc's interstitial POST-on-click. | **(A)** keep the scoped-consume flip — mint-all makes "usable but never accepted" a permanent state, and the scanner hole stays open. **(C)** flip everything the bare consume mints — a victim's unrelated login would accept an attacker's invite, dissolving the defense. **(D)** an explicit accept endpoint on the tree — a new cross-membership authz rule (cousin of the rejected lazy mint) that still leaves the invite-link path as a second writer. |
...
- **Unaccepted memberships are offered, and acceptance flips only at first use.** The tree shows an invited-but-unaccepted scope with its invitation badge; a bare consume leaves `acceptedAt` NULL even while minting its cookie; the destination's first refresh flips it. *Reds against filtering the tree on acceptance, and against restoring the consume-time flip — the manufacture test pins the writer.*
