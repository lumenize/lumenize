...
- **Re-grounds the login-flows-restoration row** (backlog § *Nebula Auth*): this task subsumes its SPA half and changes the flow its recipe pins (the scoped request goes scope-less; the landing becomes the Scopes 302), so the restoration sequences after this task and its row updates when this lands.
...
| **m6 folds in: pending claims are single-flight per address, converge-to-latest.** A claim for an address already holding an unfinished claim (unaccepted, unproven) REPLACES it — old scope row and membership deleted (safe: nothing proved it, and no DO was ever addressed, so nothing is placed), new slug minted, new link sent; a dead older link's consume lands on the ordinary invalid-token redirect. Same-slug double-submits already converge via the resume path; a different slug means the person changed their mind, and latest wins. The deletion rides the ADR-016 acting-record projection like every destructive act — here the actor is the address superseding itself. The empty m6 `it.skip` in `identity-mint-point.test.ts` gains its body, and the backlog row retires when this lands. Trigger honesty: this task ships the double-clickable button that row names as its immediacy trigger. | **Refuse-with-resend** — no deletes, but it strands a typo'd slug until pending-claim expiry exists and dead-ends the honest changed-my-mind case. **Accepted residual** — ships a known dud button to F&F day 1 for one saved afternoon, with the damage (two scopes, one profile) cleanable only by hand. |
...
- **Two claim submissions for one address converge to ONE Universe.** A second claim before the first is proven replaces it — latest slug wins, one consumable link, one Universe minted. *Reds against the second mint; the m6 `it.skip` stub becomes this test's body.*
...
- **`docs/vision/auth.md` narrows, gains, and stops asserting the old model.** § *activeScope*'s Today-differs block rewrites down to what still differs after this ships; § *Discovery* is added; and no sentence in the doc still asserts unauthenticated discovery or consume-time acceptance — §§ *The Registry*, *Profiles*, and *Founding a Star* each carry one today. *Checkable beyond the Today-grep: `grep -n 'marked accepted\|discovering where you are a member' docs/vision/auth.md` returns only rewritten lines.*
