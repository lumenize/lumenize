...
- **Computed inverse aggregates**: The ontology knows `Todo.assignedTo: Person[]` is a relationship. Can the query engine compute `todoCount` on Person at query time without the vibe coder declaring it as a stored field? This would avoid needing cross-resource migrations (see on-hold `nebula-5.5-schema-evolution.md` `migrate` query parameter discussion) for common aggregates like counts, sums, and existence checks.
