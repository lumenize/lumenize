...
So building the colon-grammar now would be speculative generality for a hypothetical. The most likely *real* future consumer is **access control for the DOs that Cloudflare Think uses** (reached via the integration shim) — but whether those DOs even route through Nebula's `onBeforeCall`, or are secured at the shim boundary instead, is unspecified. The work that would answer this is `tasks/think-nebula-integration.md` (its DO footprint & containment question is exactly this). Building the grammar before we know the requirement is committing to an answer we haven't earned.
...
- Most likely trigger: Think DO access control, once `tasks/think-nebula-integration.md` specs whether those DOs route through `onBeforeCall`.
...
- [think-nebula-integration.md](../think-nebula-integration.md) — the most likely source of a real helper consumer (its DO footprint & containment question is exactly this); [shim-hardening.md](../shim-hardening.md) — the production follow-up.
