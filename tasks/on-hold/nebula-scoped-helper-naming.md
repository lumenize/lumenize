...
So building the colon-grammar now would be speculative generality for a hypothetical. The most likely *real* future consumer is **access control for the DOs that Cloudflare Think uses** (reached via the integration shim) — but whether those DOs even route through Nebula's `onBeforeCall`, or are secured at the shim boundary instead, is unspecified. The shim work that would answer this is itself parked (`tasks/think-nebula-shim-security-review.md`, `tasks/shim-hardening.md`). Building the grammar before we know the requirement is committing to an answer we haven't earned.
...
- Most likely trigger: Think DO access control, once `tasks/think-nebula-shim-security-review.md` specs whether those DOs route through `onBeforeCall`.
...
- [think-nebula-shim-security-review.md](../think-nebula-shim-security-review.md), [shim-hardening.md](../shim-hardening.md) — the most likely source of a real helper consumer.
