...
- **`@check-example` Phase 1** — 23 runtime doc blocks are backed by real-Star + real-chromium tests (see `tasks/nebula-frontend.md` § @check-example Phase 1). The remaining `@skip-check` → `@check-example` conversion is tracked there (§5.3.8 for-docs probe backlog) and driven by the `/doc-example-audit` + `/convert-doc-examples` skills — not by this file.

## What remains (tracked elsewhere, not here)

- **Remaining `@skip-check` conversion** — `tasks/nebula-frontend.md` §5.3.8 (for-docs probes) + the Studio-bootstrap example, which stays `@skip-check` pending a human `@skip-check-approved`.
