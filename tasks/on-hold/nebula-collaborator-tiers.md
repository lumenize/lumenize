...
**Why it is paused, in his words: the design mixes the Registry domain and the mesh domain, and that is tricky enough to be worth not thinking hard about until after pre-alpha unless forced.** That mixing is not incidental — it is what §§ *Where the endpoint lives* and *The carried context payload* exist to resolve, since the registry structurally cannot pre-stage a DAG grant and Nebula structurally cannot mint a membership. **Pre-alpha pays for the pause with training or a code workaround for its handful of users** (accepted, same date); ⚠️ **The interim this once described is GONE, corrected 2026-08-21.** [nebula-galaxy-collapse-and-chat.md](../archive/nebula-galaxy-collapse-and-chat.md) no longer enrolls a collaborator as a Galaxy admin via the scope-admin bypass. It now enrolls a **genuine non-`scopeAdmin` peer** through the shipped invite — a membership at `{u}.{g}` (which confers passage on its own, since `hasPassageInto` admits a caller whose `authScope` is at-or-below the callee) plus one `write` grant at the session node. That is scenario 5 of [archive/nebula-invite.md](../archive/nebula-invite.md) at the Galaxy root, which this file's own § *Context* already named as its Galaxy arm. **What is left for this file is narrower than it was**: the named-role grant BUNDLE across many nodes, the valid-combinations UI, the self-signup redeem hook, and the enterprise consumer — not the single-grant collaborator, who now exists.
...
- **Gated on** [nebula-galaxy-collapse-and-chat.md](../archive/nebula-galaxy-collapse-and-chat.md) — it lands the first `DagTree` on a non-leaf node, which is the permission surface `write@Galaxy-root` needs.
...
- **Shares substrate with** [nebula-request-access.md](nebula-request-access.md) (the **pull** half to this **push** half) and the paused F&F invites ([nebula-pre-alpha.md](../nebula-pre-alpha.md) § *What remains*) — one `/invite` / `accept-invite` / `InviteTokens` path, not a fork.
