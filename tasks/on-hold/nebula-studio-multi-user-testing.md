...
**Status:** ⏸️ **ON HOLD, and NARROWED 2026-09-08** — the tab half was mined into [nebula-testing-with-personas.md](../nebula-testing-with-personas.md), which now owns how testing works from inside the Studio. What remains here is the fate of `/create-star` and how a pre-created tenant Star is founded. Originally parked 2026-08-17 until we designed the testing story. Nothing here blocks pre-alpha; the questions were surfaced while reviewing [nebula-registry-route-guards.md](../archive/nebula-registry-route-guards.md) and are parked here so they are decided once, together, instead of ad-hoc per endpoint.

**The favored default, stated first and now MINED: multi-user testing happens ON the `.dev` Star, not on pre-created tenant Stars** — a tab strip where the single preview iframe sits, one iframe per test user. That is [nebula-testing-with-personas.md](../nebula-testing-with-personas.md)'s goal 4, and it is pinned there. It is repeated here because open question 2 below turns on it: once those tabs work, revisit whether a real use case for admin-created tenant Stars exists at all.
...
2. **Does anything still need admin-created tenant Stars once `.dev` multi-user tabs work?** ⚠️ **Shipping personas is this question's trigger** — the tabs are that task's goal 4, so this becomes answerable rather than speculative the moment it lands. If yes, `/create-star` (or a successor) gets its real second consumer; if no, `/create-star` retires outright: `createGalaxy` now bundles the `.dev` INSERT (built 2026-08-30; its JSDoc carries the rationale), so once the wipe kills `develop()`'s legacy repair ([nebula-pre-alpha.md](../nebula-pre-alpha.md) § *⑥ The wipe*, the post-wipe cleanup) the endpoint has zero production callers.
3. ✅ **Answered 2026-09-08 and moved** — decision 1 in [nebula-pre-alpha.md](../nebula-pre-alpha.md) § *What remains* settles it as BOTH: real invited identities that accept like anyone, driven by `impersonate()`. The synthetic-subjects thread is closed with no `synthetic` column. What the Studio shows per tab is [nebula-testing-with-personas.md](../nebula-testing-with-personas.md)'s.

## Relationships

- **Mined by** [nebula-testing-with-personas.md](../nebula-testing-with-personas.md), which took the tab default and open question 3. Nothing here contradicts it.
