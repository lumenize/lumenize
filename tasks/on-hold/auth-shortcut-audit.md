...
**Status**: Draft. Applies [ADR-009](../docs/adr/009-real-auth-path.md) (real email login is the default; synthetic client-side mints are a last resort) to existing code. Prerequisite for the full sweep: the **`lumenize.io` catch-all → email-test Worker** (any `@lumenize.io` address testable via real login). Finding #1 (below) is a **prerequisite for [nebula-chat-history-multiuser.md](nebula-chat-history-multiuser.md)**'s 3-participant test, so it leads.
