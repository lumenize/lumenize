...
## What it is for
The one case the server cannot see: **different email, same person, same browser**. Same-email-across-scopes is already linked at mint. A manual **merge-profiles** feature is the fallback for everything else (different browser, cleared cookie) and remains unbuilt.
...
## Relationships
- **Depends on** [nebula-auth-identity-mint.md](../nebula-auth-identity-mint.md) §3 (`profileId` reuse at mint) — this is the *remainder* of that model, covering only the case email-match cannot see.
