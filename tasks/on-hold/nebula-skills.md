...
> **Edited 2026-09-05 — Phase 0 is absorbed, and four things below are settled by [nebula-guidance-file-tree.md](../archive/nebula-guidance-file-tree.md).** (1) *Phase 0 landed there*: the `SKILL.md` format with a three-line frontmatter, the always-on catalog generated from that frontmatter into the platform `AGENTS.md`, activation by the model through the `read_file` tool, and three seed skills — `define-ontology`, `wire-a-view`, `define-the-cast`. Sub-agents, executable steps, versioning and eval gating stay parked here. (2) *The Galaxy trust grade changed*: § *Ownership & trust split* treats a user-developer's files as data never instructions behind a Galaxy-tier product-alignment verifier; the built tree injects the Galaxy `AGENTS.md` as **instructions under the poster's own authority**, bounded by a property of the tool table — no loop tool sits above the chat floor — rather than by a verifier. The verifier stays the **Universe** tier's, where one file steers many apps. (3) *Discovery is decided*: the open question below — in-harness vs model-self-select — resolved as a catalog in the always-on layer that the model activates from through the read tool. (4) *`rules/*.md` did not ship*: integration point 1's tree is `AGENTS.md` + `skills/` this round; nothing in the platform layer is conditional yet, and the file is sectioned so a later split is mechanical. The un-park trigger's "registry-DO prompt tree" is rejected — the platform layer is files in code, embedded at deploy.
...
- ~~**Where skills physically live**~~ — **RESOLVED, and built 2026-09-05**: files in a tree the LLM walks — the platform level in code (`apps/nebula/platform/skills/<name>/SKILL.md`, embedded at deploy), the Galaxy level in the Workspace repo under `.agents/skills/` (reserved, no bodies yet); the registry-DO answer this row used to give was dropped 2026-09-02. See integration-point-1 + [`nebula-guidance-file-tree.md`](../archive/nebula-guidance-file-tree.md) § *Skills carry the conversation*.
