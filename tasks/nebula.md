...
5. **Generation engine (Think + Kimi 2.7)** — Studio's code generation runs on Cloudflare Think + Kimi 2.7 (the CMA-vs-Think bake-off + the May prototype settled this; Claude is an eval baseline only, not a product model). Likely pulls the Think integration shim forward (`tasks/shim-hardening.md`). Gate before chat UI: prove the generation loop produces working ontology + UI against the live platform. `tasks/nebula-studio-llm-strategy.md`. *(Supersedes the earlier "Claude Code drives the loop" pre-Studio plan, dropped 2026-06-16.)*
6. **Studio prerequisite spikes (gate before planning Studio).** Exploratory — not hand-reviewed; expect them to deviate as we learn. Run before finalizing the Studio plan: **`tasks/kimi-ui-gen-viability.md`** (the generation-viability gate — can Kimi 2.7 produce working `.vue` + `.d.ts` that compile in DevStar, run in preview, with reactivity + access control; start here), **`tasks/think-nebula-integration.md`** (Think↔Nebula multi-tenant fit: DO footprint/containment + memory-plug + shim verifiability; `tasks/shim-hardening.md` is the production follow-up), **`tasks/preview-iframe-spike.md`** (in-window iframe preview + ~3s reload; lower priority). The vibesdk read (#4) feeds all three.
