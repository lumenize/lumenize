...
1. **✅ Structural DO scope isolation — DONE 2026-06-16 (commit `7c83407`).** Replaced the TOFU `aud`-lock in `NebulaDO.onBeforeCall` with structural scope-from-instance-name for the **tier DOs** (Star/Galaxy/Universe). **Was the blocker for ≥2 Stars under one Galaxy:** under TOFU the shared Galaxy DO locked to the first caller's `aud` and rejected all others, so only the first Star under each Galaxy could fetch its ontology — the rest were non-functional, and the dev Star + any production Star share a Galaxy. Shipped Fix 1 only (the `<scope>:<local>` helper-naming grammar, Fix 2, had no real consumer and is deferred — `tasks/icebox/nebula-scoped-helper-naming.md`); the `ResourceHistory` fixture was removed. `tasks/archive/nebula-do-scope-isolation.md`.
2. **✅ Dev Star (instance + semantics) — DONE 2026-06-16; ⏳ class-collapse PENDING.** Studio's sandbox is the reserved `{u}.{g}.dev` instance, reusing all Star machinery (3-tuple routing, per-Star SQLite/permissions, root-admin bootstrap). What shipped 2026-06-16 (`fa9d4fb`) was the dev-instance routing/isolation/wipe semantics **via a `DevStar` subclass, which still exists in code**; the **`tasks/nebula-dev-flows.md` Decision 2 collapse to "no `DevStar` class"** (delete the class, `DEV_STAR`→`STAR`, `#starBinding()`→always `STAR`, wipe moved onto plain `Star`) is canonical-but-**not-yet-built** — tracked in `tasks/nebula-studio.md` (Phase 3.5 moves the wipe; Phase 4 deletes the class). On an ontology change, DevStudio prompts the user to wipe `.dev` data via the **Flow 1b** server→client mesh call (additive edits preserved, breaking ones reset; lazy migration deferred — `tasks/archive/nebula-lazy-schema-migrations.md`). URL-level branching is post-demo (`tasks/icebox/nebula-branches.md`). Build: `tasks/nebula-studio.md`.
...
| — | Dev Star (reserved `dev` slug instance + wipe semantics) | **Complete** — instance/semantics built + verified 2026-06-16 (`fa9d4fb`), via a `DevStar` subclass; the Decision-2 collapse to *no `DevStar` class* is pending (→ `nebula-studio.md` Phase 3.5/4) | `tasks/archive/dev-star.md` |
...
### Post-demo candidate index (gates re-checked 2026-06-22)
