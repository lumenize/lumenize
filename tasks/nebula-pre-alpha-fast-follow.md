...
⚠️ **Open design point (Larry's catch, hand review 2026-08-26): the one-copy serve forbids a star-keyed vite `base`, and the copy has one.** The copied `dist/` carries `base: /app/{u}.{g}.dev/` baked at build time (the Galaxy only ever builds the dev preview); served at a tenant star's prefix, its asset URLs point back at `.dev`'s working-tree `dist` — wrong bytes, silently. Candidates, undecided: **(a)** a star-free asset prefix — assets served at a galaxy-keyed route, so only the document path varies (Studio's model); **(b)** vite's runtime base (`experimental.renderBuiltUrl`) resolving assets against the document's own prefix; **(c)** serve-time `<base>` injection — priced against this item's no-templating decision, which it would reopen. Relative `base: './'` is NOT a candidate: an SPA deep link's document resolves `./assets` against the deep path, which recreates the silent miss.
