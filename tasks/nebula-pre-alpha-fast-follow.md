...
✅ **RESOLVED before it could bite (Larry's catch + decision, hand review 2026-08-26): no baked base exists to be wrong.** The one-copy serve forbids a star-keyed vite `base`, so the collapse builds the target directly: the scaffold builds with `base: './'` and `serve.ts` injects `<base href>` for whatever prefix it is serving under (`HTMLRewriter`; the collapse's vite-base bullet carries the derivation). `dist-prod/` is therefore one copy valid at every star's prefix with nothing to re-bake at publish, and a custom domain injects `/`.
