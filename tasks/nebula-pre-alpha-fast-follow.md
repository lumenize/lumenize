...
⚠️ **Same-origin is load-bearing**, exactly as it is for the persona seat handoff — see Item 7's reversed bullet. If the preview moves cross-origin, both mechanisms need re-deriving together, not separately. [nebula-scope-moves-to-subdomain.md](nebula-scope-moves-to-subdomain.md) moves it to the dev Star's own host (2026-09-16), so this premise ends with that build; re-deriving this item waits for its own trigger.
