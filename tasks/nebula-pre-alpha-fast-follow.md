...
**Placed here 2026-08-24**, out of the collapse's § *Serving* route table (its one ⏭️ deferred row). **Never in doubt — only deferred.** The same Galaxy `fetch` handler and `serve.ts` that serve `.dev`'s working-tree `dist/` serve any other star its **published copy**: publish **copies** `dist/` to a per-star directory (Larry's split-`dist` shape, 2026-08-24 — naming open: `dist-prod` vs `published/{s}`) and commits, so the serve path never reads through git and both tiers ride the same `getFile` seam; **the publish commit — and the shipped ref pointing at it — is provenance, not the serving mechanism**. Two obligations ride the copy: it must carry its **pinned ontology label** (the working tree moves on after publish — bake it at build or record it beside the copy; the derive-from-what-you-serve invariant, owned by [nebula-ontology-history-file.md](nebula-ontology-history-file.md)'s tabled section), and the published directory is **publish-owned** — the LLM's `write_file` is denied there at the same Galaxy chokepoint that guards the ontology history file. *(The earlier tag-lookup serve was superseded by this — reading blobs at a ref on every request was the complexity this deletes.)*
