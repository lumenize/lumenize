...
|---|---|---|
| `codegen-gate.ts:196` | `checkTypeScript` — the SFC semantic pass (tsc) | the `typeCheck` step of `build` |
| `codegen-gate.ts:170`, `:178` (and `parse` at `:162`) | SFC pass 1 — `@vue/compiler-sfc`, tsc-free | **deleted**; `vite build` parses the same SFC in the same cycle |
...
**Nothing about vite is in scope, because vite 8 already landed.** `container/app/package.json` declares `vite ^8.2.0` and `unplugin-swc ^1.5.10`, and `container/app/vite.config.ts` already wires the swc transform. ⚠️ **That plugin is REQUIRED, not an optimization, and this file previously claimed the opposite.** Rolldown/oxc emits TC39 stage-3 decorators verbatim and exits 0, and a generated app does reach decorators — the config comment says how: `./nebula` → `@lumenize/nebula/frontend` → `NebulaClient`'s `@mesh()`. Dropping the plugin ships a `SyntaxError` from a green build ([[rolldown-no-tc39-decorators]]). The backlog's *Move to vite 8* row is stale and is marked done in the trim § *Relationships* already scopes.
