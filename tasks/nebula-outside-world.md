...
⚠️ **This file carries a TRIPWIRE for a risk accepted elsewhere: the preview iframe is same-origin with Studio, and that is only tolerable while every author of generated code is a person somebody invited** ([nebula-galaxy-collapse-and-chat.md](archive/nebula-galaxy-collapse-and-chat.md) § *Costs / risks*). The moment ingested email, a webhook payload, a fetched page or an uploaded document can reach the codegen prompt, prompt injection becomes an author — and planted client-side code drives a viewing admin's Studio DOM. ⇒ **The origin split ([ADR-021](../docs/adr/021-every-scope-has-its-own-host.md), which moves the preview onto its own host) ships in or before whatever change first lets outside content influence generation** — not as a follow-up.
