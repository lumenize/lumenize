...
🔒 **The `*Guard` suffix is the CONTRACT, and it is load-bearing (decided with Larry 2026-08-11).** Two families of guards now exist in this repo and **they fail differently**:

| Family | Where | Contract |
|---|---|---|
| **`require*`** | mesh (`nebula-do.ts`, `dag-tree.ts`) | **THROWS** — `requireDominion`, `requirePassage`, `requirePermission` |
| **`*Guard`** | these route steps | **RETURNS** a `Response` to refuse, a `Request` to replace, `undefined` to pass |

⚠️ **A route step named `require…` would be a 500-shaped bug, not a style preference.** `router.ts:147` is a **blanket catch with no typed-error branch**, so a step written by analogy with its throwing neighbour returns `500 internal_error` where a 403 belongs. That is the same defect as the `onBeforeCall` citation above, reachable through the **name** instead of through the reference — and the first draft of this file had it, naming these steps `requirePassage` / `requireDominionOverInstance` one letter from `requireDominion`, which throws.

⚠️ **`enforce*` is retired repo-wide** — it was a one-member category (`enforceScopeReach`) whose verb implied a distinction it never carried, since it throws exactly as `require*` does. [nebula-dominion-vocabulary-rename.md](nebula-dominion-vocabulary-rename.md) renames it to **`requirePassage`** (the mesh boundary guard), which is why that name is free for the mesh side and unavailable here. ⚠️ **Do not "unify" the mesh `requirePassage` and this pipeline's `passageGuard`** — same verdict, two boundaries, two error contracts, and collapsing the names re-creates exactly the confusion the suffix exists to prevent.
