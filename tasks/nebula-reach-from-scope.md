...
- **No active task file states authority as pattern-coverage.** `grep -rn 'authScopePattern\|pattern-covers' tasks/*.md` returns nothing (frozen `tasks/archive/` is exempt). ⚠️ `nebula-collaborator-tiers.md` carries it in at least three places; leaving them re-anchors the next panel on the model this file deletes, which is the whole reason this lands before identity-mint's review.
