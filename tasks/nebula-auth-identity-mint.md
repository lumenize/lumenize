...
> 🔄 **The reach half is being replaced — [nebula-reach-from-scope.md](nebula-reach-from-scope.md) lands FIRST (decided 2026-08-05).** A token will carry the member's scope, and reach will be `isAdmin ∧ scope-at-or-above-this-node`, with no derived pattern. **Nothing in this file's mechanism changes** — an invite still mints a membership at the path scope with a per-entry `isAdmin` — but the vocabulary below and the tier table are written against the model being deleted. **Write the criteria against the target, and do not let `/review-task` anchor on the pattern grammar.**
...
**The mint invariant:** the identity lands at the **path** scope, which the router has already proved the caller administers. So what is minted never exceeds the inviter's own authority, at any tier. No self-elevation.
...
- **[ADR-015](../docs/adr/015-scope-authority-flows-downward.md)** — authority flows downward only; the bare `admin` bit is never authority; restraint is a UI warning, never an authz refusal.
...
- **Tier coverage** — a star-tier, a universe-tier and a platform-tier invite each land a membership **at the scope named in the path**, and the resulting token reaches what the reach model says it should. ⚠️ **Phrase this over the membership and the observed reach, never over a pattern string** — [nebula-reach-from-scope.md](nebula-reach-from-scope.md) deletes the pattern, so a criterion asserting `{u}.{g}.*` would red on correct code the week after it is written.
