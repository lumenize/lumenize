...
> 🚨 **OPEN FOR REVIEW (raised 2026-07-25, after `claim-star` shipped):** `#mintIdentity` now has a **fourth** caller — open, unauthenticated Star self-signup — which this file predates. It widens §5's threat model from *an authenticated co-scope admin* to *any stranger*. See the flags in §3 and §5; the second §5 question is a genuine design trade.
...
1. An admin can invite a peer who becomes an **admin at the invited scope**.
   - 🔓 **What this adds at the Galaxy tier is a galaxy-BOUNDED principal — not missing capability** (noted 2026-07-25, corrected same day). ⚠️ Do **not** read the mechanics below as "a Galaxy can't be operated": a universe admin's `{u}.*` matches `{u}`, `{u}.{g}` and everything beneath, so they already do everything inside every Galaxy they own. Nothing is blocked.
     - The mechanical fact: no `Identities` row can exist at a 2-segment scope (`create-galaxy` mints no founder, and there is no `claim-galaxy`), so no refresh cookie can be set at `/auth/{u}.{g}/` and nobody can *authenticate at* a Galaxy. Callers authenticate at the universe and name the Galaxy in `activeScope` — the shape prod uses, and **correct, not an interim**. `bootstrapUniverseAdmin` in `apps/nebula/test/browser/auth-bootstrap.ts` does exactly that and should **stay** after this lands.
     - What is genuinely absent is a principal whose authority **stops** at one Galaxy: a collaborator on one app who is not an admin of the whole universe. That is least privilege, and it is the actual reason to want this — `issueInvites(universeGalaxyStarId, …)` is already generic on scope, so it mints that row without new mechanism.
...
- **`accept-invite`** (the router's `accept-invite` branch → [`handleAcceptInvite`](../packages/nebula-auth/src/worker-token.ts)) and **magic-link** both funnel through **`consumeAndLogin`** ([here](../packages/nebula-auth/src/worker-token.ts)) — real login per ADR-009, a bodiless 302 + refresh cookie.
- **The scope-admin bypass** ([`requirePermission`](../apps/nebula/src/dag-tree.ts) — the `accessAdmin` early-return, before any DAG lookup): `requirePermission` returns early for any `access.admin` caller **before any DAG lookup**, and `enforceScopeReach` ([`enforceScopeReach`](../apps/nebula/src/nebula-do.ts)) admits them. **This carries the whole enrollment story** — an admin at `{u}.{g}` reaches every node on the Galaxy *and* the child `.dev` Star (`{u}.{g}.*` covers `{u}.{g}.dev`) with **no grant written and no new code**. There is no Galaxy self-grant and none is needed (`Star.onBeforeCall`'s self-grant, [`Star.onBeforeCall`](../apps/nebula/src/star.ts), is a request-access-climb terminus, not an access path). ⚠️ This same bypass is what the gate above is about — it is currently **unconfined**.
...
Today `#mintIdentity` mints a **fresh `profileId` for every new `(email, scope)` row**, so one person in two Galaxies gets two profiles. The fix, in [`#mintIdentity`](../packages/nebula-auth/src/nebula-auth-registry.ts) **above** its `INSERT INTO Identities (sub, profileId, …)`:
...
⚠️ **`#mintIdentity` is the single implementation site precisely because it has FOUR callers**, all authority points that must inherit the reuse — [`claimUniverse`](../packages/nebula-auth/src/nebula-auth-registry.ts), the bootstrap platform-admin mint inside [`requestMagicLink`](../packages/nebula-auth/src/nebula-auth-registry.ts), [`issueInvites`](../packages/nebula-auth/src/nebula-auth-registry.ts), and **[`claimStar`](../packages/nebula-auth/src/nebula-auth-registry.ts)**. Fixing it at the invite loop alone would let a human invited into someone else's Galaxy mint a **second** profile when they later claim their own Universe.

🚨 **The fourth caller is NEW (open Star self-signup shipped 2026-07-25) and it is UNAUTHENTICATED — this file was designed and reviewed before it existed.** `claim-star` is gated only by Turnstile, which bounds scripted abuse, not identity. That changes who can reach §3's reuse rule: **any stranger, with no account at all**, can POST `claim-star` with a chosen email and mint an `Identities` row — at a Star they control, carrying **that email's existing `profileId`** — before the person is notified or has consented to anything. §5 identifies the same primitive but argues it against an *authenticated admin of some scope*; the open endpoint removes even that bar. **Needs a reviewer's eye, not a unilateral edit** — see §5.
...
`#requireOwnerOrAdmin` ([`#requireOwnerOrAdmin`](../packages/nebula-auth/src/profile.ts)) passes when `scopes.some(s => matchAccess(pattern, s))` over `getScopesForProfile(profileId)`, and it gates **public-field writes + `privateNotes` read/write**. Today that is near-vacuous (a profile spans one scope). **After §3 it spans every scope the human is in — and someone else can unilaterally add a scope to it:** `issueInvites` mints the Identities row at *issue* time (`emailVerified=0`), so typing a victim's email into `POST /{u}.{g}/invite` inserts a row carrying the victim's reused `profileId` **before they accept or are even notified**. The inviter then reads their private notes and rewrites their global identity.

🚨 **REVIEW THIS AGAINST THE OPEN ENDPOINT — the bar is now lower than this section assumes.** The paragraph above reasons about an **authenticated admin of some scope**. Since `claim-star` shipped (2026-07-25), the same row-with-the-victim's-`profileId` is mintable by an **unauthenticated stranger** (§3), so "who can start this attack" went from *a co-scope admin* to *anyone who can pass a Turnstile challenge*. Two questions for the panel, neither of which should be settled unilaterally:
1. **Does the §5 fix still fully close it?** Dropping the scoped-admin branch means the attacker cannot read `privateNotes` or write public fields — and `getScopesForProfile` has exactly one consumer (`profile.ts`), so it goes genuinely dead for authz. That appears sufficient. Confirm, don't assume.
2. **Is a residual leak acceptable?** Even with §5 applied, an attacker can *cause a `profileId` to be associated with a scope its owner never joined*. Nothing reads that for authz afterward — but it is written durably, it is attacker-chosen, and it is unauthenticated. Decide whether that needs bounding at the `claim-star` end (e.g. do not reuse an existing `profileId` for an **unverified** self-signup mint — defer the link until `emailVerified` flips) or is accepted.
⚠️ Option (2)'s parenthetical would put a **conditional** in `#mintIdentity`, which §3 deliberately keeps single-behaviour for all four callers — so it is a real design trade, not a tweak.
...
1. **Per-invitee admin mint (auth).** Reshape the body to `invitees` (§1) and thread each entry's normalized `isAdmin` through `handleInvite` → `issueInvites(instanceName, invitees, origin, verifiedAccess)` (which re-asserts admin-over-scope in-method — §2); its existing per-email loop reads `invitee.email` + `invitee.isAdmin`. **A/C** → drop the hardcoded `/* isAdmin */ false` in that loop; **B** → `setIdentityAdmin(sub, true)`. **Promote-only.**
   - **Call sites:** replace every site matching `grep -rn "emails" --include="*.ts" packages/nebula-auth apps/nebula | grep -v node_modules` (~15 at time of writing), **plus** `packages/nebula-auth/README.md` (~:201, which diagrams the body shape) and `apps/nebula/test/test-helpers.ts` (`createSubject`, imported by ~17 baseline files — wire its currently-dead `isAdmin?` option through here; the collapse task's collaborator test needs it). ⚠️ **Do NOT touch `@lumenize/auth`** — the MIT package has its own `#handleInvite` / `body.emails` ([`#handleInvite`](../packages/auth/src/lumenize-auth.ts)) and its own consumers.
...
- 🔓 **UN-SKIP OBLIGATION — this task owns a deferred test in another lane.** [nebula-star-founder-provisioning.md](archive/nebula-star-founder-provisioning.md) Phase 1 (deletion warn-don't-block) carries `it.skip('deletes a scope WITH another user attached, showing the bounded warning')` in `apps/nebula/test/ui-smoke/delete-scope.test.ts`. It is skipped **only** because no invite affordance exists to attach a second identity — deliberately not faked with an out-of-band registry seed (`workflow.md`: prefer `it.skip` over an ossifying stand-in). **Acceptance criterion for this task:** once an admin can invite a peer, un-skip it and assert the confirm screen renders *"Warning — 1 other user will lose access: …"*, the **Delete button stays ENABLED**, and the delete completes. (The registry-level behavior is already covered in `nebula-auth-registry.test.ts` + `identity-authority.test.ts`; what un-skips here is the **UI rendering** of the bounded warning.)
