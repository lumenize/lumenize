...
**NEXT (Larry, 2026-06-26):** the first slice to build is **Docker / real-container coverage in GHA** — confirmed below that the Docker part was never done (CI runs the container tests under pool-workers as prototype/skip, never a real container). **Gated on:** the concurrent **hosted-sandbox session**'s edits landing in `feat/nebula-studio` + being pulled down first (it's editing things so the suite runs in the Claude Code hosted sandbox). Don't start until that merges.
