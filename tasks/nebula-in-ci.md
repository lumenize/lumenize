...
3. **❌ Remaining blocker — auth routes `503` (NOT the container, NOT email delivery).** The 2 login-requiring tests fail because `POST /auth/{scope}/email-magic-link → 503 Service Unavailable (2ms)` and a `refresh-token → 503` — *fast* 503s, so the route errors immediately. Consistent across CF-email *and* the Resend switch, so it's **not** the email provider/timeout. `refresh-token` 503ing (no email involved) points at a **common auth-infra cause** under the prod config in `wrangler dev`-on-CI (a binding? the rate limiter? the self-ref `AUTH_EMAIL_SENDER`? the send throwing?). Worker-side root cause **TBD** — `@lumenize/debug` forwarding didn't surface it (namespace mismatch); only wrangler's request log (`--log-level info`) gave the 503.
4. **Next:** diagnose the `503` (broader `DEBUG`, or known auth-setup cause). The container half is proven; once auth works in this lane, ui-smoke goes green in GHA. Then confirm the hosted-sandbox target + the Phase-2 hosted-capability work (Resend boot + the AI REST swap, token now in place). The `--no-optional` whack-a-mole **belongs to ui-smoke's harness only** (no always-on lane runs vite).
