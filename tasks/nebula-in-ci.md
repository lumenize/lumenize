...
**Build note (2026-06-28):** harness scaffolded — [`.github/workflows/container-spike.yml`](../.github/workflows/container-spike.yml) (`workflow_dispatch`) runs the `ui-smoke` lane on `ubuntu-latest` with Docker + CF creds, writing a `CLOUDFLARE_ACCOUNT_ID` line into `.dev.vars` to satisfy the `HAS_CF_CREDS` gate. **Awaiting a manual dispatch on `feat/nebula-studio` for the GHA boot result; the hosted-sandbox target is separate.** Can't be answered from the macOS dev session (ui-smoke already runs there — the unknown is *headless Linux*).
