...
**Status**: ✅ **RAN + GO — 2026-06-17** (`experiments/container-agent-channel`, commits `cd60945`/`e6fb249`/`8a97fe5` on `feat/nebula-studio`). No kill fired across Q1–Q5; mechanism proven **local + deployed** — warm channel ~30 ms edge-local, cold first command ~1.36 s (faster than vite's), streaming + HMR-parity + server-side-only boundary all ✅. **One caveat:** a `vite build` starves the command channel on the default ¼-vCPU instance (size dev instance for build bursts). Full results in `experiments/container-agent-channel/FINDINGS.md`; harvested to memory [[agent-channel-container-exec]]. **→ unblocks `tasks/nebula-devcontainer-node-type.md` (build (2)).** A **spike**: throwaway, no production code merges (workflow.md § Experiments). **Deployed Worker + its container app DELETED 2026-06-17** (results captured). Experiment **code kept tracked on purpose** — the #2 build (`nebula-devcontainer-node-type.md`) builds on its proven shape; prune the workspace entry + `git rm` after that build lands.
