...
### 5. `@lumenize/state` (definitely) + `@lumenize/ui` (conditionally) — ported from JurisJS

The JurisJS port lands as **two packages**:

- **`@lumenize/state`** — `StateManager` + path helpers. ~340 LOC. NebulaClient binds to this as its local store (no shadow cache). **Ports first** — it's risk-free regardless of how the renderer question resolves.
- **`@lumenize/ui`** — `DOMRenderer` + ComponentManager + Juris orchestrator on top of `@lumenize/state`. ObjectDOM templates with the function-vs-value reactive pattern. **Ports if and only if** the LLM-generation spike (item 6 below) shows ObjectDOM is the right shape for generated UIs.

Why split? `@lumenize/state` is clearly correct work — the store is needed regardless of how the UI is rendered. The renderer is the only thing under question. Splitting de-risks the framework decision and gives NebulaClient's headless-mode story (Node tests, scripts) for free.

Pre-port inventory done — see `tasks/lumenize-ui.md`. Subscribe wiring (the StateManager↔NebulaClient bridge middleware) lives in `@lumenize/state`, NOT `@lumenize/ui`, so it works the same whether the UI is ObjectDOM or vanilla HTML+JS-against-StateManager.

### 6. Pre-Studio milestone — Claude Code drives the loop directly

**Before Studio exists, prove the platform supports code generation by having Claude Code drive the whole loop directly, against the live platform.** No chat UI, no tool orchestration we built — just Claude Code (the CLI) reading the ontology + `@lumenize/state` (and conditionally `@lumenize/ui`) docs + Nebula API types, and generating ontology + UI artifacts that get pushed to the dev-mode Star.

**This milestone is also the LLM-generation spike that gates the `@lumenize/ui` port** (item 5 above). The spike shape: hand Claude an ontology + StateManager docs + DaisyUI class reference + 2–3 example components in *both* shapes (ObjectDOM and vanilla HTML+JS-against-StateManager), see which it produces more reliably, then commit to a path. Styling is settled (DaisyUI, renderer-agnostic — see `tasks/lumenize-ui.md` § Styling) so the spike isolates the renderer question. The full milestone is bigger than just that spike — it's actually building a small real app — but the spike is the early signal.

Why this is its own milestone, not just a spike:
- **De-risks Studio.** If Claude Code can't reliably produce working ontology + UI against the platform we've built, no amount of chat-UI polish saves Studio. If it can, Studio is the wrapper around a proven pattern.
- **Demo-able on its own.** "Here's me building an app by talking to Claude Code that talks to my platform" is a real demo. A useful fallback if Studio's chat UI isn't ready when investors want to see something.
- **Shapes Studio's design.** Whatever prompts, tool surface, and feedback loop work well for Claude Code generation are what Studio's prompts and tools should be modeled on. Don't design Studio in a vacuum and hope.

What "pre-Studio" includes:
- Subscriptions (Phase 5.3) shipped end-to-end.
- NebulaClient subscribe wrappers (Phase 7) shipped — `@lumenize/ui` is the bound store.
- `@lumenize/ui` shipped with the must-keep set + subscribe-wired middleware.
- Dev-mode lazy migrations + dev-mode branching shipped.
- Documentation/types pinned so Claude Code has clean context: ontology format, Lumenize UI component shape, NebulaClient API surface.
- A way to push generated artifacts to the dev-mode Star (the same `deploy_to_dev` mechanism Studio will eventually use, even if the UX is "paste this into the terminal" for now).

Stop point for this milestone: a small but real app (todo list, kanban, simple CRM) generated by Claude Code, running on the live platform with all the access-control + reactivity guarantees intact.

### 7. Studio (Phase 9, renamed)
- Active end-of-line goal. Wraps the proven generation pattern from milestone 6 in chat UI + tool orchestration.
- Includes:
...
- Deferred for now: detailed Studio narrative, demo storyboard. We'll write that out after the prerequisite work is in place — and after milestone 6 has shown us what generation patterns actually work.
...
- **`@lumenize/ui` extraction inventory**: ✅ Done — see `tasks/lumenize-ui.md` § Pre-port inventory.
...
> The original "Studio cold-start spike (1 day)" was promoted to its own milestone (item 6 above) — it's bigger than a 1-day spike and is now the gate before Studio implementation begins.
