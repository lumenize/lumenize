...
**Status**: **NO-GO / REJECTED (2026-07-04) — keep the stamp.** Larry's own correction invalidated the headline benefit: **instance names are permanent** (`idFromName` → fixed ID → fixed storage; you can't rename an instance to reach its storage, and naming-scheme / custom-domain changes mint *new* instances rather than renaming existing ones), so "renames brick DOs" (benefit #1) collapses to only rare *binding* renames. With the recurring write cost already eliminated by the interim guard, and the alarm improvements separable from removal, the case no longer clears the migration risk + loss of the first-write-wins bug-detector. **Decisions: keep the stamp; keep the interim write-guard permanently** (`lmz-api.ts` `setBindingName`/`setInstanceName` — writes only on first stamp, then 0/call; do **not** refactor the two keys into one — the one-time single-write saving isn't worth the storage migration). **Separable follow-up preserved:** if alarm-using features (soft-delete reaper, dead-admin liveness) ever need `callContext` in their handlers, do a small standalone "persist `callContext` in alarm rows" task then — it does **not** require removing the stamp. § Benefits analysis below is retained as the decision record. _(Briefly flipped to GO earlier today before the instance-name correction; git holds that.)_
