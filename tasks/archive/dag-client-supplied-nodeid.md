> ✅ **DONE + ARCHIVED 2026-07-02.** Client-supplied UUID `nodeId` shipped to prod (retired SQLite rowid); committed `44e4432`. Frozen point-in-time record — don't edit. The post-ship prod-recovery saga (incomplete `wrangler delete` wipe → CF-dashboard purge; JWT-key `.dev.vars`-quote restore bug → 500 on refresh-token; stale-NXDOMAIN client DNS) is captured in the `wrangler-delete-leaves-do-data` memory + `apps/nebula/scripts/deploy.sh`. **Phase 1 was skipped** (by decision), so the D6 sql-migrations-against-real-data proof was *not* captured before the Phase-5 wipe — see `nebula-pre-alpha.md`.
