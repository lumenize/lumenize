...
2. **One JS module per ontology version.** `createValidator()` emits a single JS module containing assert/parse functions for all resource types in the ontology. Stars fetch this one string, not N strings per type. Shared runtime helpers dedupe naturally inside the bundle. `parse(value, { typeName })` looks up the right function by name inside the loaded module. (Open question in Phase 1 spike: does the DO facet loading mechanism accept a module that exports multiple functions, or does it require a single class? This constrains the bundle shape.)
