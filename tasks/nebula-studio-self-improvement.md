...
| Billing | All judging on **Workers AI (Cloudflare bills)**; zero Anthropic per-token | Hard constraint. **AI Gateway is a proxy/observability layer, not a billing route** — routing *to* Anthropic through it is still Anthropic per-token. The no-per-token route = a Workers AI *destination* model. There is **no flat "Claude SDK credits" judge path** — confirmed, don't go looking. |
