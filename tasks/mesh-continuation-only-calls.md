...
**Status**: **🚧 PHASE 1 BUILD IN PROGRESS (`/build-task`, 2026-07-03) — core mechanism BUILT + GREEN; some criteria remain.** Feasibility premise CONFIRMED under pool-workers (D15/criterion 10: a DO early-acks then does a real 2s post-ack gap and the fire-back STILL lands — `DurableObjectState.waitUntil` keeps it alive). Built: the full method split (`call`→`#buildEnvelope`→`dispatchEnvelope`, early-ack, shared `executeEnvelope` owns admit→`{$ack}`→chain+fire-back under `ctx.waitUntil`, `__executeChain` folded away/M3, `__handleResponse` door on DO/Worker/Container + the Gateway/D17, `__localChainExecutor` narrowed + Container's deleted, exactly 2 mesh RPC entries), the client in-heap leg (D16 — handler keyed by callId, delivery re-resolves, dedup, `onErrorOnly`/N6), `callRaw` demoted to a loud deprecated throw, D6 error routing (tier-1 sync-throw + tier-2 admission-reject→local handler + post-ack throw→fire-back), and Q5 injectable `CLIENT_CALL_TIMEOUT_MS`. **`packages/mesh`: 398/398 pool-workers tests GREEN + type-check clean; per-file coverage baseline captured** ([reference/mesh-continuation-coverage-baseline.md](reference/mesh-continuation-coverage-baseline.md)). Tests migrated off `callRaw` (call-context/lumenize-do/lumenize-worker/call-target-validation/lumenize-client + Gateway); new failure-mode tests added (N8 handler-throws-at-sink, N9 cold-caller-zero-state, D6-tier-2, client success/error-RESULT + M4 dedup). **NOT YET DONE (remaining Phase-1 criteria):** (4) the mandatory `enforceScopeReach` security matrix in `apps/nebula` test-apps (B5/D5); (7) broadcast-to-disconnected-client error (m4) — Worker-callee fire-back IS covered; (8) Flow-C DETERMINISTIC RESULT-path re-resolution (M5) — Gateway supersession/PUSH re-resolution is covered; (12) the `wrangler dev` exploratory confirmation + findings note; and running the **browser** (needs CF creds) + **container** (needs Docker) projects (client/container code changed). **One design deviation flagged for Larry:** the Gateway `__handleResponse` RESULT leg does NOT apply `onBeforeCallToClient` (the RESULT is solicited + client-side callId-deduped; the gate guards *unsolicited* pushes) — see the door's JSDoc. **`packages/fetch` breaks at runtime** (still calls `callRaw`) — its best-effort adapt is Phase 2. Scope = the **best-effort tier only** (guaranteed tier: [`on-hold/mesh-call-durable.md`](on-hold/mesh-call-durable.md)). Sole active work on the `pre-alpha` branch — [nebula-pre-alpha.md](nebula-pre-alpha.md) is on hold until this ships.
