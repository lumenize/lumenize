...
- [ ] **Also audit every EXISTING 3-arg `call()` site (Larry, 2026-07-03) — the resilience profile of 4-arg INVERTED.** Before this task, 3-arg was the *safe* choice for client-crossing / long / hibernation-prone calls (an awaited result stranded); now 4-arg is the *resilient* one (the handler travels / stays in-heap, delivery re-resolves — survives WS reconnect, tab sleep, DO hibernate). `grep` every `lmz.call` and run the decision tree below.
