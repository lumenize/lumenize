...
**Status**: **DESIGN PINNED — BOTH `/review-task` STAGES RESOLVED (framing + conformance, 2026-07-02→03)** — D1–D17 + the method split; all Stage-2 reconciliation + framing (8 fixes, no structural findings) + conformance (11 fixes) applied. Headline conformance corrections: **M1** the response-leg security gate re-checks **origin→node containment**, NOT responder identity (`originAuth` is origin-propagated, never re-stamped — the earlier N4 note was wrong); **M2** the client/Gateway `CALL_RESPONSE` disposition under early-ack is now pinned; **M3** `executeEnvelope` calls `executeOperationChain` directly (interface change, not a flag toggle); **M4** client in-heap map lifetime + dedup; **M5** a deterministic Flow-C test; `waitUntil` is per-node-type. **Next: Larry reviews the 3 flagged judgment calls + commits → `/build-task` Phase 1.** Nothing built yet. Scope = the **best-effort tier only** (guaranteed tier: [`on-hold/mesh-call-durable.md`](on-hold/mesh-call-durable.md)). Sole active work on the `pre-alpha` branch — [nebula-pre-alpha.md](nebula-pre-alpha.md) is on hold until this ships.
