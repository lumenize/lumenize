...
**Status:** Design intent complete (co-written 2026-08-19, replacing the prior file wholesale after a section-by-section mining pass); **phases are NOT written**. `/review-task` **Stage 1 ran 2026-08-19** (18 findings, 0 refuted) and its gate edits are applied — ⚠️ **one decision is OPEN**: the abuse-bound question in § *Who may invite*, plus the smaller Pass-2 call on a `NebulaClient` convenience method (§ B). From here: resolve the OPEN item → write phases → **Stage 2**. § *Acceptance criteria* is Pass-2 input: it says what must be true, deliberately not in what order.
...
**Landing deliverables:** the mesh.md facade pattern is already stated (edited 2026-08-19) — cite this bridge there as the canonical instance once it exists. Update the workers-projects snapshot line for nebula-auth (dual-layer, derived per-file) at the same time; that edit must wait for the code, or the map lies. **`docs/vision/auth.md` owes edits the moment the route dies, or the accepted doc contradicts itself:** re-anchor the R1–R7 worked example and R6's "the one route that takes a scope" on a surviving scoped token-bearing route (the `/auth/:scope/…` moves in [nebula-registry-scope-in-url.md](nebula-registry-scope-in-url.md) are the natural anchor), sweep R3's `subRateLimitGuard` exemplar, resolve § *Grants*' abuse-bound sentence per the OPEN decision in § *Who may invite*, and retire the two "Today's code differs" blockquotes that point at this task. **Repoint the backlog rows that restate the deleted route's mechanics:** § *Nebula Auth*'s rate-limits row (its "the mitigation … exists" claim) and § *Testing & Quality*'s ui-smoke row (its Bearer-authed `POST /invite` mechanics).
...
**Sequencing signal for Pass 2:** the F&F gate rides the direct-invite slice — the A-mint criteria minus the scenario-5/`InviteStatus` bullets, plus B — so phase that slice first; the scenario-5 additions (Star method, grant write, `InviteStatus`) stack on it.
...
- **Tier coverage** — star-, galaxy-, universe-, and platform-tier invites each land a membership at the named scope, with the resulting token holding exactly the dominion the rule says. ⚠️ Phrase over the membership and the **observed verdict**, never a pattern string. The galaxy limb is load-bearing twice: no claim path can authenticate at a 2-segment scope, so the invite is the sole production mint there — and the invitee's accept-invite login AT a galaxy scope is a principal never producible before this task.
...
- **Reusable-within-TTL:** a second GET of the same invite link within TTL logs in again (the scanner-then-human sequence), and the row dies only at the expiry sweep. *Mutation: restore delete-on-consume — both limbs red.*
- **Live-session convergence on promotion:** establish a non-admin session, re-invite with the bit WITHOUT clicking the new link, refresh the pre-existing session — the JWT now carries it. *Mutation target: `setIdentityAdmin`'s KV re-put loop; the accept→refresh limb in the first criterion stays green under that mutation, which is why this limb exists.*
- **Re-inviting an existing member emits the `invite-existing` template**, never `invite-new`. *Reds against today's behavior (the variant exists, unemitted).*
- **The prod provider delta is applied and scoped:** the deployed config carries `EMAIL_PROVIDER=resend` prod-only, and the harness/`wrangler dev` lane still sends via the CF Routing catch-all (`waitForEmail` keeps working — that unchanged `/live` loop is the check).
