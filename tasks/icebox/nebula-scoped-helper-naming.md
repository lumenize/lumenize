...
- A bare-UUID helper name `12345` derives scope `12345`, and `isAtOrAbove('12345', 'acme.app.tenant-a')` = **false → rejected** by the containment predicate itself. No real tenant's scope is ever under a random UUID, so cross-tenant movement is structurally impossible regardless of any registry.
