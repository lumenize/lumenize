...
**Q1 — Confinement holds, caller-agnostic.** Trace: token in `Sec-WebSocket-Protocol` (`fetch-ws.ts:39-41`, same as a browser) → signature verified **once at WS upgrade** (`entrypoint.ts:45` `verifyNebulaAccessToken`: sig + `iss` + `aud`/`sub` + `access.authScope` + `isAtOrAbove(authScope, aud)`) → Gateway mints `originAuth` from the **verified attachment**, replacing anything the client sent (`lumenize-client-gateway.ts:534-546`) → `onBeforeCall` structural scope-lock (`nebula-do.ts` `requirePassage`; the hard cross-tenant boundary is its `hasPassageInto` clause) → DagTree `requirePermission` (`dag-tree.ts:152-163`). Nothing distinguishes a Think DO from a browser → an in-DO client holding U's scope-S token confers no more authority than U in S.
