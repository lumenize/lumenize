...
**Why a plain login cannot serve instead — LOGIN NEVER MINTS.** Mint is authority-point-only (*"NEVER call from a login path"*), so a magic link for a scope with no founder is issued and emailed, then rejected on consume (`getAndVerifyIdentity` → null → `302 …invalid_token`, no cookie). A `Scopes` row with no founder is a scope nobody can ever enter — which is why Phase 2 is non-optional, not a convenience. (Trace: [nebula-auth-registry.ts](../packages/nebula-auth/src/nebula-auth-registry.ts) `login` 'Magic link for non-member'; [email-login.ts:252-267](../apps/nebula/test/lib/email-login.ts).)
...
### Phase 1 — deletion stops blocking + a bounded warning (PREREQUISITE)
...
✅ **The README's false `claim-star` assurance is GONE — dropped from this phase's scope 2026-07-23.** [`packages/nebula-auth/README.md`](../packages/nebula-auth/README.md) used to document `claim-star` as SHIPPED with Turnstile (old `:35` / `:71` / `:124`), promising a control on an endpoint that does not exist. All three lines were removed as part of the **post-surrogate-sub architecture rewrite** of that README — the route table had to be rebuilt from `router.ts` anyway, and a false line inside an otherwise-accurate table is worse than one inside a wholesale-stale doc (a reader has no reason to doubt it). The rewrite left a fenced "**Not yet built**" pointer to this file in its place, naming the `REGISTRY_ENDPOINTS` + `TURNSTILE_ENDPOINTS` double-registration above. ⇒ **This phase's README work is now purely ADDITIVE** — document `claim-star` when you build it, as part of building it. There is no sweep left to do, and nothing stale to unlearn.
...
⛔ **Depends on Phase 2** — nothing but `claim-star` mints a star founder, and this phase re-grounds fixtures *onto* one. (No seed change is involved; the founder-preference phase was cut — see §The DAG root grant.)
...
### Phase 4 — the signup UI (minimal), served from Galaxy
