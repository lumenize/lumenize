...
| `requireAdmin` (**every file matching** `grep -rlE '\brequireAdmin\b' packages apps --include='*.ts'` — 26 files, 57 occurrences under `src`, 135 repo-wide once tests are counted) | `requireDominion` | verdict | ⚠️ **it is the dominion guard wearing the bit's name** — "require admin" is one reading away from *the bare bit is dominion*, the bug that has shipped twice. It already delegates |
...
| `Subscribers.accessAdmin` (**a column**; 35 occurrences under `src`, 72 repo-wide) | `dominionAtSubscribe` | verdict | a **frozen** dominion verdict, named for the claim field it is not. The `AtSubscribe` half is load-bearing: it is confined at subscribe time and never re-read live |
...
|---|---|
| **The renames land in their own file, first** | Folding them into the behaviour changes — a rename inside a diff that also moves control flow is unreviewable as a rename, and this is a few hundred call sites of mechanical noise laid over the changes that must not be gotten wrong. ⚠️ **Counts stated here are dated samples, not inventories** (`tasks/README.md` § *Inventories*) — re-run the greps rather than trusting them. |
...
| **`require*` means THROWS, repo-wide; a step that RETURNS a `Response` is `*Guard`** | Keeping `enforce` vs `require` as the carrier — it points the wrong way (`require` throws in Solidity and in `assert`, and at every one of this repo's own `requireAdmin` / `requirePermission` sites) and would rename all 59 to `enforce*` to say what `require*` already says. The route steps in [nebula-registry-route-guards.md](nebula-registry-route-guards.md) take `*Guard` instead; that file's § *The shape* carries the table, because a route step named `require…` returns a **500** where a 403 belongs. |
...
- 🔒 **No `isAdmin` in the sibling packages is renamed.** `grep -rnE '\bisAdmin\b' packages/auth/src packages/mesh/src` returns the **same hits before and after**. *Reds against a repo-wide replace — which breaks mesh's unrelated `isAdmin` silently, the failure mode that has no test.* ⚠️ **Stated over the SYMBOL, not over the diff, and the earlier "no path under `packages/auth/` or `packages/mesh/` appears in `git diff --name-only`" form was WRONG — corrected 2026-08-11 by running it.** [`packages/mesh/src/lmz-api.ts`](../packages/mesh/src/lmz-api.ts)'s `__handleResponse` JSDoc **cites `enforceScopeReach` by name**, so this rename makes that comment stale and Phase 1 *must* touch `packages/mesh` to fix it. A diff-shaped criterion would have forced the builder to choose between a green criterion and a correct comment. ⚠️ **That one line needs THREE fixes and only one is this file's** — see § *Phase 1*.
- 🔒 **The column rename reaches existing storage, on both tables.** ⏳ *Build-time only — cannot be pre-verified at review.* After the rename, a DO whose storage predates it reads and writes `dominionAtSubscribe` without error on **both** `Subscribers` and `QuerySubscribers`. *Reds against an inline-DDL edit that only reaches freshly-created tables — a schema change that announces nothing on storage it never touched.*
...
- **No standing-guidance statement cites a symbol this file deletes.** `grep -rn 'hasAdminOverScope\|enforceScopeReach\|requireAdmin\|accessAdmin' docs .claude tasks website/docs` — every hit gets a verdict. ⚠️ **Guidance citing a deleted symbol is worse than guidance citing a deleted model**: the reader greps, finds nothing, and cannot tell whether the rule is stale or they are looking in the wrong place. ⚠️ **Do not trust a "known homes" list — the one that stood here was FALSE and was corrected 2026-08-11 by running the grep.** It named `durable-objects.md` and `workflow.md`; **both contain zero hits.** What the grep actually returns, outside this file and its three siblings: `.claude/rules/mesh.md` (5), `.claude/rules/security.md` (3), `.claude/rules/calibration.md` (1), `docs/adr/015-passage-and-dominion.md` (3), `docs/adr/007-shared-node-security-core.md` (1), `website/docs/nebula/auth-flows.md` (2), plus several `tasks/` files. ⚠️ **Re-run it; that list is a 2026-08-11 sample.** ⚠️ **"Standing guidance" is the CLAUDE.md list** — it includes `.claude/skills/*/SKILL.md` and **agent memory**, neither reachable by that grep, and memory is the worst of them: it loads every session unprompted and nothing type-checks it.
...
⚠️ **One line in `packages/mesh` must be touched, and it carries THREE defects of which only the first is this file's.** [`lmz-api.ts`](../packages/mesh/src/lmz-api.ts)'s `__handleResponse` JSDoc reads *"the response leg was **rejected at admission** (e.g. `enforceScopeReach`, D5)"*:

1. **`enforceScopeReach` stops existing** — this file's rename makes the citation dangle. **Fix it here.**
2. **`admission` is a RETIRED reserved term** (ADR-015 § *Terminology* replaced it with `passage` on 2026-08-09, and `reach` may not become its umbrella noun either). A sweep that fixes only the symbol leaves the sentence asserting a word the ADR deleted. **Fix it here too** — it is the same sentence, and leaving it is how a retired term survives a vocabulary pass.
3. **`D5` is a task-file handle in SOURCE**, which `workflow.md` forbids for the same reason as the `D16` handles in Phase 2. ⚠️ **This one is NOT this file's** — the line is mesh's, the handle belongs to a mesh task file, and converting it needs that file's context. **Leave it and note it**, rather than guessing at what `D5` meant.

⚠️ **This is exactly why the sibling-package criterion is stated over the `isAdmin` SYMBOL rather than over the diff** — see § *Acceptance criteria*. The hazard is renaming mesh's unrelated `isAdmin`; touching a mesh comment that names one of our symbols is required, not forbidden.
...
- ⏳ **Delete the task-file handles shipped in source — there are FOUR, not one.** `grep -n 'D16' apps/nebula/src/subscriptions.ts` returns `:34`, `:51`, `:62` and `:182` (2026-08-11). `workflow.md` forbids a task-file handle in code — it is unresolvable the moment that file archives. ⚠️ **Only `:34` and `:51` are touched by the rename**; `:62` and `:182` are prose comments the sweep would sail past, so "this rename touches the line anyway" justifies **half the work and must not be read as covering it all.** Each becomes a clause of prose or an ADR citation.
