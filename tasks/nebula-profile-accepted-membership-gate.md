...
- **One shared site with** [nebula-passage-dominion-from-scope.md](nebula-passage-dominion-from-scope.md) — `#requireOwnerOrAdmin`'s per-scope containment check became `scopes.some(s => hasDominionOver(claims.access, s))` there (✅ landed). The acceptance contract this file owns is unaffected, so write the criteria over acceptance semantics and they survive either model.
