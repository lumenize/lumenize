...
1. ✅ **Decided 2026-09-07/08 — personas ride `impersonate()`; acceptance is enforced at the MINT and by the Registry's reads; no `synthetic` column.** The narrower-token mint refuses a subject whose membership is not accepted, so an `act`-bearing token exists only over a membership the person took up, and the Profile keys off `sub` like every other guard. Personas are real `@lumenize.io` accounts, invited and accepted through inbound mail ported to production — the early phase of `nebula-testing-with-personas.md`, Larry's to write — so no column and no second door. The owner stacked behind a persona in the app is acceptable. The argument and the alternatives are in § *② Personas*; the Profile work is the row below the personas row.
...
| # | Item | Task file | Gate |
|---|---|---|---|
| ② | **Personas** — synthetic users the LLM defines, each in its own preview tab | none yet, Larry's — § *② Personas* | deploy, plus `data` for the `synthetic` column |
| — | **Profile access control** — acceptance enforced at the mint and by active-by-default Registry reads, the Profile keying off `sub` like every guard, and the Profile composed onto the Resources plane behind an allowlist | [nebula-profile-access-control.md](nebula-profile-access-control.md) — Pass 1 drafted 2026-09-08 | **data** — the Profile's storage changes, free only before the wipe |
...
- **Refined 2026-09-08 (Larry): the check moves to the MINT, the column is withdrawn, and the Profile joins the Resources plane.** Enforcing acceptance where a token is born — the mint refuses an unaccepted subject, and the Registry's membership reads return active rows unless asked for pending ones by name — lets the Profile guard drop `!act` and key off `sub` like everything else, with `security.md`'s rule (1) left without an exception. Personas need no `synthetic` column: each is a real `@lumenize.io` account, invited and accepted through inbound mail ported to production, and the digest never needed the column because every persona action carries `act` naming the owner. The Profile, the one resource host not on the plane, composes `ResourceDataPlane` behind four `@mesh` methods before the wipe, which gives impersonated edits their acting-principal record and history from the plane rather than a copy. All in [nebula-profile-access-control.md](nebula-profile-access-control.md).
