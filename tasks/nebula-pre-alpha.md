...
- ✅ **Local UI smoke** (the `ui-smoke` Playwright lane) → [archive/nebula-local-smoke.md](archive/nebula-local-smoke.md). Its `it.skip`s have named owners: [backlog.md](backlog.md) § Testing & Quality (the `.dev`-login blocker) and [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) (surfaces the collapse deletes or rewrites).
...
- ✅ **Reactive AI chat** (chat = `Message` Resources on DevStudio) → [archive/nebula-reactive-ai-chat.md](archive/nebula-reactive-ai-chat.md). Open non-blocking deferrals: ephemeral `onChatResult` removal; D-corpus (Galaxy corpus fate); D-echo hardening (server-validate `author`); Phase-5 viewport UI driver.
...
- 🔧 **`createGalaxy` creates `{galaxy}.dev` in the SAME synchronous DO method (decided 2026-08-17; small, independent; this bullet is the spec).** Today two App.vue creation flows call `createGalaxy` then `createDevWorkspace` back-to-back, and `develop()` lazily repairs galaxies where the second call never landed — a client-side invariant with a client-side repair loop, and a day-1 F&F failure shape (first app → galaxy with no `.dev` star). The DO method is synchronous end-to-end, so a second `INSERT` is atomic with the first. ⚠️ **The two paired App.vue call sites MUST change in the same commit** — after bundling, the old second call answers 409 `slug_taken` into a user-facing catch. ⚠️ **Deliberately mint NO membership at `.dev`**: dominion from the creator's own membership is the access; a founding row would be a redundant copy of structural authority. Post-wipe every galaxy is bundled, `develop()`'s repair is dead code, and `/create-star` drops to **zero** production callers — whether it retires outright or waits on a testing use case is [on-hold/nebula-studio-multi-user-testing.md](on-hold/nebula-studio-multi-user-testing.md)'s open question; deleting the `create-star` table row IS the deregistration ([archive/nebula-registry-route-guards.md](archive/nebula-registry-route-guards.md)).
- 🔧 **Chat history UI wiring → durable multi-user reactive thread.** Design + phases: [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) (its Status line is the authority; whole-file review deliberately deferred to just before its build). Feeds THE GATE — identity-attributed user turns are the highest-value capture signal.
  - 🏗️ **Auth foundation LANDED (undeployed) + PROD WIPE batched at THIS milestone.** [archive/nebula-auth-surrogate-sub.md](archive/nebula-auth-surrogate-sub.md) is built (surrogate `sub`; registry + Workers KV); deploying it needs a greenfield prod **wipe** (re-key + DO-class delete), batched as ONE CF-dashboard worker-delete + redeploy — **keep F&F invites paused until then**. Wipe runbook: [archive/do-exports-and-toolchain-upgrade.md](archive/do-exports-and-toolchain-upgrade.md) § *Phase 4* (⚠️ NOT the surrogate-sub file's frozen `migrations`-shaped narration — superseded by the declarative-`exports` conversion). Smoke-check at the wipe: real-CF KV login→refresh cross-colo (a KV miss falls back to the registry `RefreshTokenIndex` — a deliberately-live path). **Follow-ons riding this milestone:** the "login flows" restoration ([backlog.md](backlog.md) § Nebula Auth) and the profile→presence schema changes. ⚙️ **Ops step at this deploy: set the `RESEND_API_KEY` secret** — `deploy.sh`'s required-secrets preflight refuses to deploy without it and prints the exact command (value from the gitignored root `.dev.vars`); what stays open → [backlog.md](backlog.md) § *Nebula Auth*, the Resend row.
    ...
    - 🪟 **The wipe is a CLOSING WINDOW for free schema surgery — batch it here.** Greenfield means every DB is created fresh, so changes that would otherwise need a migration are **free right now and expensive forever after** we have real users.
      ...
      6. ✅ **BUILT 2026-08-20 — `Snapshots.actingToken` replaces `changedBy` with the full ADR-016 record (via the shared `projectActingToken`, imported over the new Node-safe `@lumenize/nebula-auth/claims` subpath); the same-actor coalesce key derives from IDENTITY only (`sub` + the `act` chain's subs — `identityKey` in [resources.ts](../apps/nebula/src/resources.ts)); `projectActClaim` deleted; the push/read boundary allow-lists what leaves the DO (`WireActingToken` — identity + display `profileId`s, never the asserted `access`); the auth.md § *Attribution* sentence deleted.** The pre-build check on [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) Phase 2 CONFIRMED both simplifications — recorded there. Post-panel: ADR-016's *"Resources is not conformant yet"* Corollary retensed to ✅ conformant, and the registry's one inline ADR-016 record (`executeScopeDeletion`) now routes through the shared projection. Deploys at the wipe gate with everything else. *(Original spec below, kept for the record.)*
         **Replace Resources' `changedBy` with the full acting claims, and DERIVE the coalesce key** — [ADR-016](../docs/adr/016-record-the-acting-principal.md) conformance. `SnapshotMeta.changedBy: ActClaim` ([resources.ts:36](../apps/nebula/src/resources.ts); one JSON `TEXT` column at `:91`, parsed at `:133`) holds `{ sub, act? }` — enough to name the actor, not the `profileId` or asserted `access` ADR-016 requires. **The blocker is a conflation, not a cost:** `:212`'s `JSON.stringify(current.meta.changedBy) === changedByJson` makes the stored record *itself* the same-actor key. ⚠️ **Do NOT just widen it** — the coalesce window is **1 hour** (`:107`) while `ACCESS_TOKEN_TTL` is **15 min**, so one window spans several tokens with differing `jti`/`iat` and an hour-long editing session would stop coalescing, multiplying snapshot rows on the highest-volume write path. **The fix:** store the **entire claims object** *in that column's place* (same rule as every other ADR-016 site — not a second field beside it), and compute the same-actor key from **identity only** (`sub` + the complete `act` chain); if either changes, new snapshot. Coalescing behaviour is provably unchanged — `{ sub, act }` is already all `changedBy` holds, so the derived key *is* today's stringify, made explicit. Keep `profileId` (derived from `sub`; would spuriously split on a unification re-point) and `access` (asserted authority) **out** of the key. **Rename the field to `actingToken`** — the first ADR-016 site (`executeScopeDeletion`'s record) uses it, and it is [ADR-016](../docs/adr/016-record-the-acting-principal.md)'s own phrase. ⚠️ **Name it for the TOKEN, never for a role** — every role name inverts (`actor.sub` reads as "the actor" but is the person acted *upon*; the actor is `act.sub` — the misreading ADR-016 exists to prevent). ⏳ **DELETE `projectActClaim` in [resources.ts](../apps/nebula/src/resources.ts)** when you build this (exported; `#buildChangedBy` its only production caller, plus one unit test) — it exists solely to keep the widened JWT `act` claim out of this column while the column is still narrow; its JSDoc says so at the site. It is a **zero-reader rename right now** (nothing outside `resources.ts` reads the column; the only other repo-wide hit is a comment at `nebula-client.ts:1631`), while the Galaxy collapse's chat attribution is its first real consumer — so build before the collapse. ⚠️ **Check before building:** [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) Phase 2's parallel `{sub → profileId}` `Snapshot.meta` map — with both parties' `profileId` on the record it may be redundant. Also: `Snapshot.meta` rides pushes, so the **push boundary needs its own projection** (never ship a writer's `access`/`jti` to subscribers — beyond ADR-008 intra-Star, and worse for global Profile snapshots under ADR-012); a disclosure boundary, not a compatibility one, since there are no readers yet. ⚠️ **A doc edit rides this item as a phase criterion:** delete the one sentence in [`docs/vision/auth.md`](../docs/vision/auth.md) § *Reading the history*'s `> **Today's code differs.**` blockquote noting the Resource attribution is narrower than the target — that sentence only, the rest is retired by other work. 🗓️ **No child task file: this bullet is the spec** (decided 2026-08-19). Build immediately after ①, before the collapse.
...
- ⚠️ **GATE — preview survives redeploys (NOT built; rides the collapse — see above).** Any
  ...
  **cold-boots every active preview**: the "Waking your preview…" interstitial does **not** self-heal on
  ...
  seamless before/right-after inviting. Direction is pinned at the collapse `/review-task`, not here.
