...
**Status**: **⏸️ ON HOLD (2026-07-01)** — paused until the mesh continuation-only refactor ships ([mesh-continuation-only-calls.md](mesh-continuation-only-calls.md) — no held Promises across hops; the sole active work on the `pre-alpha` branch). Resume this master plan after it lands. **Identity-stamp removal — REJECTED (2026-07-04), keep the stamp:** [archive/decision-mesh-identity-stamp-removal.md](archive/decision-mesh-identity-stamp-removal.md) — instance names are permanent, so the rename-safety benefit was invalid; the interim 2-line stamp write-guard in `packages/mesh` stays permanently. A `callContext`-in-alarm-rows follow-up remains available as a standalone task if alarm features need it. Everything below is unchanged from the active program.
