...
- ✅ **THE DEPLOY WORKS AGAIN, AND THE MOUNT SERVES (2026-08-29).** The compilers left the Worker for the container build job — [nebula-move-compilers-out-of-the-worker.md](nebula-move-compilers-out-of-the-worker.md), built; its § *Success* carries the re-measured numbers and `scripts/check-worker-graph.mjs` reds if a compiler import ever reaches the entry graph again. The deployed-FUSE-serves-empty blocker that survived the first deploy was OURS and fell the same day — the mount serves the VFS's `/workspace` subtree and Galaxy seeded the root (bisect: `experiments/fuse-bisect/RESULTS.md`; fix: `WS_ROOT`/`wsPath()`; the resolved row in [backlog.md](backlog.md) § *Nebula*). Proven deployed: `build-box` passes at `world: fuse` (real container `vite build`, dist served at `/app/{scope}.dev/`) and `studio-codegen-rest` completes a real turn with one build cycle.
...
- 🔧 **Chat history UI wiring → durable multi-user reactive thread.** Design + phases: [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) (its Status line is the authority; whole-file review deliberately deferred to just before its build). Feeds THE GATE — identity-attributed user turns are the highest-value capture signal.
  - 🏗️ **Auth foundation LANDED (undeployed) + PROD WIPE batched at THIS milestone.** [archive/nebula-auth-surrogate-sub.md](archive/nebula-auth-surrogate-sub.md) is built (surrogate `sub`; registry + Workers KV); deploying it needs a greenfield prod **wipe** (re-key + DO-class delete), batched as ONE CF-dashboard worker-delete + redeploy — **keep F&F invites paused until then**. Wipe runbook: [archive/do-exports-and-toolchain-upgrade.md](archive/do-exports-and-toolchain-upgrade.md) § *Phase 4* (⚠️ NOT the surrogate-sub file's frozen `migrations`-shaped narration — superseded by the declarative-`exports` conversion). Smoke-check at the wipe: real-CF KV login→refresh cross-colo (a KV miss falls back to the registry `RefreshTokenIndex` — a deliberately-live path). **Follow-ons riding this milestone:** the "login flows" restoration ([backlog.md](backlog.md) § Nebula Auth) and the profile→presence schema changes. ⚙️ **Ops step at this deploy: set the `RESEND_API_KEY` secret** — `deploy.sh`'s required-secrets preflight refuses to deploy without it and prints the exact command (value from the gitignored root `.dev.vars`); what stays open → [backlog.md](backlog.md) § *Nebula Auth*, the Resend row.
    ...
    - 🪟 **The wipe is a CLOSING WINDOW for free schema surgery — batch it here.** Greenfield means every DB is created fresh, so changes that would otherwise need a migration are **free right now and expensive forever after** we have real users.
      ...
      2. ✅ **BUILT 2026-08-29 — every compiler left the Worker for the container build job** (both of them: tsc AND `@vue/compiler-sfc`, which this item predated). [nebula-move-compilers-out-of-the-worker.md](nebula-move-compilers-out-of-the-worker.md) is the owner; needed no DO class migration, so nothing of it rides the wipe. (`bundle-tsc.mjs` was NOT retired — it runs at image build now, over source copied into the context.)
      3. ✅ **DECIDED 2026-08-29 — NO SPLIT.** Item 2's re-measure puts the main Worker at **2,342.79 KiB** with a deploy-time `Worker Startup Time` of 32–35 ms — inside the ≤~2.7 MB cheap-and-stable tier (`create` ~340–390 ms, [`experiments/do-cold-start-bundle-ab/RESULTS.md`](../experiments/do-cold-start-bundle-ab/RESULTS.md)), nowhere near the unpredictable middle. A split would only buy back the per-crossing hop cost the collapse deleted. The guard against silently re-entering the middle tier is behavioural, not a number: `scripts/check-worker-graph.mjs` reds on any compiler import reaching the entry graph, and a future heavy NON-compiler dep is a fresh decision at its own review.
...
- ✅ **GATE — preview survives redeploys: DISCHARGED STRUCTURALLY by the collapse** (BUILT 2026-08-28; criterion in [nebula-galaxy-collapse-and-chat.md](nebula-galaxy-collapse-and-chat.md) § *Final verification*). `dist` is durable in Galaxy SQLite and the container is out of the read path — a redeploy or eviction has nothing to revert, no interstitial, nothing to re-push. The old failure class (cold boot reverts the container disk to the baked image; a self-heal that fired only on the stuck signature) died with the `/dev-container` serving route. ✅ **The deploy-only confirm ran 2026-08-29** — post-redeploy, `GET /app/{star}/` on `test-nebula` served the last-built `dist` with the same pre-redeploy hashed asset (recorded in the collapse file's status banner); the build-box mount limbs pass deployed AND locally.
