...
- 🚨 **THE DEPLOY IS BLOCKED — nothing below can ship until it is fixed.** `wrangler deploy` is rejected with `Script startup exceeded CPU time limit`; the two compilers in the Worker bundle (tsc at 70%, `@vue/compiler-sfc` at 12%) do their table-building at module scope. Design + measurements: [nebula-move-compilers-out-of-the-worker.md](nebula-move-compilers-out-of-the-worker.md) (Pass 1 drafted 2026-08-28, phases await hand review). Removing both takes the bundle from 12,957 KiB to 2,294 KiB and startup from 362 ms to 51 ms.
