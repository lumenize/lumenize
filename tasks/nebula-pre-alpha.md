...
1. ✅ **Decided 2026-09-07/08 — personas ride `impersonate()`; acceptance is enforced at the MINT; no `synthetic` column.** The narrower-token mint refuses a subject whose membership is not accepted, so an `act`-bearing token exists only over a membership the person took up, and the Profile's owner branch drops its `act` clause. Personas are real `@lumenize.io` accounts, invited and accepted through inbound mail ported to production — the early phase of `nebula-testing-with-personas.md`, Larry's to write — so no column and no second door. The owner stacked behind a persona in the app is acceptable. The argument and the alternatives are in § *② Personas*; the Profile work is **built** (§ *Shipped*), so ② is unblocked on all three of its goals.
...
**Mostly assembly, not new mechanism** (verified on disk 2026-09-02): `invite()` returns the minted `sub` (`InviteeSummary`); `getIdentityScope` filters on `acceptedAt` since 2026-09-08, so a persona is impersonable only once it has accepted — which is why ② provisions each one through real inbound mail; `impersonate(sub, activeScope)` is built and driven (`impersonation-lifecycle.ts`); `dagTree().setPermission` attaches the grants. ⚠️ **The no-send-mint gap claimed here is retired — its premise was false** (checked 2026-09-08, `nebula-testing-with-personas.md` § *Verified on disk*). A `.dev` wipe leaves an accepted persona accepted on the same `sub`, because `resetDevData` never reaches the Registry and `#mintIdentity` early-returns the existing row — so no wipe re-bounces a magic link, and a re-invite sends the link-less `invite-existing` letter instead. What survives is narrower and is that file's § *Open* J.
...
- **`impersonate()` or a real login per persona** — decision 1 above. ⚠️ **Its premise expired 2026-09-08; the last bullet below is the answer.** An impersonated token carries `act`. ADR-012's owner branch used to refuse that, so an impersonated persona never owned its own profile. It no longer reads the chain. A real login needs the credential isolation the cookie does not give, and a mailbox per persona.
...
- **`act` is disqualifying for a persona (Larry, 2026-09-06), so impersonation is out.** Display derives only from the stamped acting token, so every record an impersonated persona wrote would show the owner stacked behind it, and so would presence — the one surface a persona exists to make look real. Two ways remain to a persona token with no `act`: per-persona cookies (above), or **synthetic sessions** — impersonation's parent-side mint and in-process renewal, no cookie, minting WITHOUT `act` when and only when the target membership is flagged `synthetic` and the caller holds dominion over the scope, the owner recorded at the mint as ADR-016 already requires of a session establishment. That makes the column load-bearing and folds decision 1's two halves into one: the flag licenses the mint, set only by the provisioner at creation and never toggled, so no human identity can ever be minted without a trace. Claude's input: synthetic sessions — no cookie change, no first-refresh wrinkle, the persona indistinguishable from a real user inside the app; `security.md`'s mint-side rule gains a stated exception for synthetic subjects.
- **Decided 2026-09-07 (Larry): the accepted-only owner branch, not synthetic sessions.** Larry's design: change who may edit and read a Profile's private fields to consider only ACCEPTED memberships — an `act` token passes the owner branch when the subject's membership at the token's `authScope` is accepted. It holds because acceptance is the one thing a manufacturer cannot produce, and it grants nothing new: a scope the person accepted is one whose admin already reaches the profile through the admin branch. Personas therefore ride `impersonate()` as built; the provisioner creates each synthetic membership already accepted, which only the `synthetic` flag licenses (a human's invite is never pre-accepted, or the manufacture returns through the front door). The owner shown stacked behind a persona in the app is acceptable — the user-developer may not render it as a stack at all. The Profile gating fix was owed regardless, planned for after the invite and self-signup acceptance rejigger with a home/discovery page, so this is its moment.
- **Refined 2026-09-08 (Larry): the check moves to the MINT, the column is withdrawn, and the Profile joins the Resources plane.** Enforcing acceptance where a token is born — the mint refuses an unaccepted subject, because the Registry read it uses is active-only under its plain name while the session-lifecycle callers ask for pending rows by name — lets the Profile guard drop `!act`, leaving `security.md`'s rule (1) with `forwardWithSubject` as its worked case. Personas need no `synthetic` column: each is a real `@lumenize.io` account, invited and accepted through inbound mail ported to production, and the digest never needed the column because every persona action carries `act` naming the owner. The mint and the owner branch shipped 2026-09-08 → [archive/nebula-profile-access-control.md](archive/nebula-profile-access-control.md). The Profile's move onto the Resources plane, which gives impersonated edits their acting-principal record and history, was split out after that file's Stage 1 the same day: it is a change to the plane's guard model, so it is ④'s (§ *The Profile — the first host whose guard is not a grant* there).
...
Adjoins [nebula-testing-with-personas.md](nebula-testing-with-personas.md), which absorbed the multi-user-testing file's tab design, and shares the subject/grant/scope core with [on-hold/nebula-request-access.md](on-hold/nebula-request-access.md), the pull half — share it, don't fork.
...
- **Still open, and this file's to settle:** the ADR-018 sizing sentence (every persona is a Registry membership row), and ADR-016, which binds the executor's mints and grants as authority events. ⚠️ **Impersonate-or-real-login is CLOSED** — decided 2026-09-07/08 (the two bullets above): personas ride `impersonate()`, because the owner branch no longer reads the actor chain, so ownership-keyed behaviour is testable through an impersonated persona.
- **Criteria to carry:** a wiped `.dev` Star is re-established from the file alone — provision from a seeded `personas.md`, wipe, run, and the tree and grants match the prose (⚠️ each unchanged persona keeps the SAME `sub` — the membership survives a `.dev` wipe and is reused; the older "fresh `sub` per name" wording was falsified 2026-09-08) (mutation: drop create-if-missing → the second run fails on "already exists"); and a plan naming an unknown node is refused before anything runs (mutation: remove validation → a grant lands on nothing).
...
**Post-wipe cleanup:** `develop()` in `App.vue` still creates a missing `.dev` for apps that predate `createGalaxy` bundling it. Once no such app exists it is dead code, and deleting it takes `/create-star` with it: `NebulaClient.createDevWorkspace` is its one production caller, and `createGalaxy` has bundled the `.dev` INSERT since 2026-08-30. Two `/live` scenarios still POST it — `passage-not-dominion` and `invite-roundtrip` — so they get repointed at `createGalaxy` in the same change. **Whether anything still wants admin-created tenant Stars is answered by then, and the answer looks like no:** ② personas makes the `.dev` tab strip the testing story, which is the use case that endpoint was being kept for. ⚠️ **A question dies with it, so do not re-open it later:** an admin-pre-created, member-less tenant Star was the one state a stranger could capture by claiming its slug, and retiring the endpoint means nothing can create that state — the only member-less star rows left are `.dev`, refused earlier as a reserved name. The policy was remediation rather than prevention anyway (Larry, 2026-09-08): a Universe or Galaxy admin already deletes any scope beneath them, members and all, with the members surfaced as a warning rather than a refusal ([ADR-015](../docs/adr/015-passage-and-dominion.md)) — that frees the slug and the real founder claims it fresh.
