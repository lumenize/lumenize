...
- ⚠️ **GATE — passage and dominion computed from the scope (NOT built) → [nebula-passage-dominion-from-scope.md](nebula-passage-dominion-from-scope.md).** A token carries the member's scope; `passage` and `dominion` are computed from it and every site calls one of them, deleting the derived wildcard pattern. **Amends ADR-015** (commitment intact, mechanism replaced). ✅ **NOT wipe-gated** — nothing stored changes shape, verified.
  - ⚠️ **It is invite-gated for ONE reason, and the reason changed on 2026-08-11.** It used to sit in Wave 2, sequenced early for **unlearning cost** — five files described the model it replaces. The `dominion`/`passage` sweeps paid that in prose without the code changing, so that argument retired itself. What gates it now is its `Missing` **item 4**: the Registry's HTTP route gate has **no `scopeAdmin` conjunction**, so a non-admin at `{u}` reaches a descendant scope's registry routes today. Harmless while Larry is the only user; a hole the moment invites go out.
  - ✅ **Exactly one hole, and that is audited rather than assumed** (2026-08-11, every direct caller's enclosing function read). Five sibling sites are *structural* checks where a `scopeAdmin` conjunction would be wrong; two hold the conjunction spelled across a function. **Do not let this item inflate into a security sweep** — the rest of it buys inspectability, not incident response.
  - **Soft-blocks** [nebula-auth-identity-mint.md](nebula-auth-identity-mint.md), whose design intent is written in the vocabulary this replaces.
...
- **Provision-a-subject-into-{scope, role}** — the unification: Universe-admin invite (pre-provisioned
  ...
  **moved to § *Invite-gated* on 2026-08-11**, so its full entry lives there. In one line: a token carries the
...
- 👤 **LARRY HAND-REVIEW — ADR-011 onward, then restructure/merge/split wholesale.** Scheduled **after the Galaxy
  ...
  - 🔴 **RATIFY ADR-015 / ADR-016 / ADR-018 — DUE NOW, and this is the only thing still tracking it.** All three are
