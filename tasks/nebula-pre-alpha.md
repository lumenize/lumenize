...
| — | **Turn-liveness heartbeat — the server signals at round/build boundaries so a long quiet turn stops flashing `failed`** — *no task file* | deploy | **Slotted right after ② (Larry, 2026-09-02)** — not a peer of the wipe-critical spine. Low-risk polish, but it touches the codegen streaming path, so it lands near the guidance work (①) rather than drifting into "much later". The client already consumes the signal (`signalTurn` in [turn-liveness.ts](../apps/nebula/src/turn-liveness.ts)); what's missing is a truthful server emit during the non-streaming stretch — a cold first token, or the container build — which today crosses the `TURN_IDLE_MS` idle window and paints the banner before the next chunk self-heals it |
