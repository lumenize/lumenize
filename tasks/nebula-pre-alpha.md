...
1. ✅ **Decided 2026-09-07 — personas ride `impersonate()`, the Profile's owner branch reads ACCEPTANCE, and the `synthetic` column lands.** An `act`-bearing token passes the owner branch when the subject's membership at the token's `authScope` is accepted — a manufactured scope has only an unaccepted row, and an accepted one is a scope whose admin already reaches the profile. The flag licenses the provisioner to create a synthetic membership already accepted; a human's invite never is. The owner stacked behind a persona in the app is acceptable (the user-developer need not render it as a stack). The argument and the alternatives are in § *② Personas*; the gating fix is the row below the personas row.
...
| — | **Profile gating on acceptance** — the owner branch admits an `act` token whose subject's membership at the token's `authScope` is accepted; private-field reads follow; ADR-012's tripwire amended; the `synthetic` flag licenses a pre-accepted synthetic membership | to write — small, security-model; § *② Personas* | deploy, plus `data` for the flag |
