...
**Status (2026-07-04):** prod `nebula.lumenize.com` is **redeployed at HEAD `ada3f31`** (clean tree) — the mesh continuation-only refactor + `callAsync`, reactive chat-as-Resources-subscriptions, and the DevContainer stuck-flag fix are all **live**. **Paused-next = F&F invites**, gated on the ⚠️ items under **[Remaining → Invite-gated](#invite-gated-needed-before-the-first-ff-invite)** (capture-live, consent UI, preview-survives-redeploys). Sole open mesh thread = the `callAsync`-inventory follow-ups (`#pendingTurns`/`#pendingSubscribes` in [mesh-client-callasync.md](mesh-client-callasync.md)).
...
- **Turn-log inspection v0 (manual)** — registry-resolve the user's `{u}` → super-admin (delegated) token →
  ...
  *(Fanout target moves from `Galaxy.getTurns` → `DevStudio` `Message` Resources now that Child 3 relocated turns.)* **→ its API/inspection client is now folded into the live-verification harness** ([claude-live-verification.md](claude-live-verification.md)) — build once, there.
...
- **Live self-verification harness** (Claude drives + inspects a running Nebula; subsumes inspection-v0; acceptance vehicle for the chat-history + preview gates): `tasks/claude-live-verification.md`
