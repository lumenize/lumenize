...
- ✅ **THE DEPLOY WORKS AGAIN, AND THE MOUNT SERVES (2026-08-29).** The compilers left the Worker for the container build job — [nebula-move-compilers-out-of-the-worker.md](archive/nebula-move-compilers-out-of-the-worker.md), built; its § *Success* carries the re-measured numbers and `scripts/check-worker-graph.mjs` reds if a compiler import ever reaches the entry graph again. The deployed-FUSE-serves-empty blocker that survived the first deploy was OURS and fell the same day — the mount serves the VFS's `/workspace` subtree and Galaxy seeded the root (bisect: `experiments/fuse-bisect/RESULTS.md`; fix: `WS_ROOT`/`wsPath()`; the resolved row in [backlog.md](backlog.md) § *Nebula*). Proven deployed: `build-box` passes at `world: fuse` (real container `vite build`, dist served at `/app/{scope}.dev/`) and `studio-codegen-rest` completes a real turn with one build cycle.

- ✅ **BUILT 2026-08-30 — `createGalaxy` creates `{galaxy}.dev` in the SAME synchronous DO method** (from this bullet, no child task file; mutation-validated in `nebula-auth-registry.test.ts`; the spec below stands as written, its post-wipe riders included). Today two App.vue creation flows call `createGalaxy` then `createDevWorkspace` back-to-back, and `develop()` lazily repairs galaxies where the second call never landed — a client-side invariant with a client-side repair loop, and a day-1 F&F failure shape (first app → galaxy with no `.dev` star). The DO method is synchronous end-to-end, so a second `INSERT` is atomic with the first. (The both-App.vue-sites-in-the-same-commit rider is spent — done in the build commit.) ⚠️ **Deliberately mint NO membership at `.dev`**: dominion from the creator's own membership is the access; a founding row would be a redundant copy of structural authority. Post-wipe every galaxy is bundled, `develop()`'s repair is dead code, and `/create-star` drops to **zero** production callers — whether it retires outright or waits on a testing use case is [on-hold/nebula-studio-multi-user-testing.md](on-hold/nebula-studio-multi-user-testing.md)'s open question; deleting the `create-star` table row IS the deregistration ([archive/nebula-registry-route-guards.md](archive/nebula-registry-route-guards.md)).
- ✅ **Chat history UI wiring → durable multi-user reactive thread — BUILT with the collapse (2026-08-28), ARCHIVED 2026-08-30.** [nebula-galaxy-collapse-and-chat.md](archive/nebula-galaxy-collapse-and-chat.md) is the record (per-author subs, byline back-fill, name modal). Feeds THE GATE — identity-attributed user turns are the highest-value capture signal.
...
- ✅ **Galaxy collapse — BUILT 2026-08-28, every deploy-staged criterion discharged 2026-08-29/30, ARCHIVED.** [nebula-galaxy-collapse-and-chat.md](archive/nebula-galaxy-collapse-and-chat.md) is the frozen record; the code is the authority. Its follow-through kept going after archive-eligibility: the compilers task ([archive/nebula-move-compilers-out-of-the-worker.md](archive/nebula-move-compilers-out-of-the-worker.md)) restored the deploy, and the 2026-08-30 one-good-way pass finished the ontology story (install ONLY by lazy-pull, first-touch `getCurrentOntology` arm, registry as a directory of files).
