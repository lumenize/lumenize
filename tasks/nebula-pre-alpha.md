...
**Status**: **▶️ UNBLOCKED (2026-07-04) — ready to resume.** The gating mesh continuation-only refactor SHIPPED + is archived ([archive/mesh-continuation-only-calls.md](archive/mesh-continuation-only-calls.md) — all deferrals incl. the dev-studio harness rewrite closed 2026-07-04, commit `d0afe8e`). The DevContainer wakeup fix also shipped ([archive/nebula-container-wakeup-fix.md](archive/nebula-container-wakeup-fix.md), commit `8d122c9`). **Paused-next = F&F invites.** Sole open mesh threads = the `callAsync`-inventory follow-ups (`#pendingTurns`/`#pendingSubscribes` in [mesh-client-callasync.md](mesh-client-callasync.md)). **Identity-stamp removal — REJECTED (2026-07-04), keep the stamp:** [archive/decision-mesh-identity-stamp-removal.md](archive/decision-mesh-identity-stamp-removal.md) — instance names are permanent, so the rename-safety benefit was invalid; the interim 2-line stamp write-guard in `packages/mesh` stays permanently. A `callContext`-in-alarm-rows follow-up remains available as a standalone task if alarm features need it. Everything below is unchanged from the active program.
...
**✅ ALL 3 CHILDREN DONE + ARCHIVED (2026-07-01) — the multi-user chat substrate is complete.** A 3-child thread, built ONE AT A TIME (extract nuggets up + archive each before the next):
...
- ✅ **Child 3 — `reactive-ai-chat`** — **DONE + archived** ([`tasks/archive/nebula-reactive-ai-chat.md`](archive/nebula-reactive-ai-chat.md); reviewed Stage-1+2, all phases 0–6, 538 nebula tests green, mutation-verified, fan-out PASS; commits `3767048`→`c46e6a0`). **`Turn`→`Message`** ontology rename (message+role is the industry shape; the row was always message-shaped; no prod migration — never deployed; + optional `status?`/`thought?`/`author?`). Chat is now **Resources on DevStudio**: user posts a `role:'user'` `Message` (`postUserMessage`, `author`=email, no optimistic echo); the Child-2 query sub (`Message where session==DEFAULT_SESSION_ID`, fixed id, single `SESSION_NODE_ID`) is the durable delivery + history channel → history-restore on refresh, disconnect-recovery, multi-participant, all via one late-joiner-restores-full-history property. **D-streaming = option (b):** the assistant's live progress rides a **transient `svc.broadcast(handleStreamChunk)`** push (NOT tokens — the codegen reply is a synthesized status; the loop gained an `onProgress` seam) to permission-filtered session-query targets (`targetsForQuery`, the point-of-action recheck), then **ONE durable `Message`** is committed (`commitAssistantMessage`) and the client reconciles the ephemeral stream by id — so the ephemeral `onChatResult` path is **demoted** to a live-only optimization. Closes the "thinking forever" hang by construction (reply is a durable Resource). Session seeded idempotently (`ensureSession`/`ensureResource`). Packaging: session constants live in a client-safe leaf (`chat-constants.ts`). **THE GATE's capture-extension** can now ride this Message-as-Resource model (capture = reading those Resources). **Deferred (non-blocking):** Phase-5 viewport UI driver (needs `nebula-studio-ui` rebuilt with the current client — not in-repo; windowing mechanism is Child-2-verified), a `wrangler dev`/prod exercise of the real `chat()` streaming wiring (in-session local spin blocked by CF-email non-delivery under local dev — set `EMAIL_PROVIDER=resend`), ephemeral-path removal, D-corpus (Galaxy corpus fate), D-echo hardening (server-validate `author`).
...
- ✅ **Data-use consent flag** — **BUILT + verified + archived 2026-06-30** ([`archive/nebula-consent-flag.md`](archive/nebula-consent-flag.md)). Nullable `improveProductConsent` column on `NebulaAuthRegistry.Instances` (the **first prod DO schema migration**, via `@lumenize/sql-migrations`); recorded at `claimUniverse` (Universe-level, per-level-ready — galaxy/star later is UI-only); corpus = `WHERE improveProductConsent = 1 AND instanceName != PLATFORM_INSTANCE_NAME`; assume-`true` for F&F. **Not yet in prod** (confirmed 2026-06-30 — no prod push since it landed; last deploy was 06-26): it self-applies on the next `apps/nebula` deploy, so **verify it ran post-deploy** (D6, the only prod-path proof). **⚠️ D6 proof NOT captured (2026-07-02):** the plan was for [`dag-client-supplied-nodeid`](archive/dag-client-supplied-nodeid.md) Phase 1 to deploy current HEAD and capture the proof *before* that task's Phase-5 prod wipe — but **Phase 1 was skipped by decision** and then the wipe ran, so the sql-migrations-against-*real-data* proof never happened. On the fresh post-wipe prod the `improveProductConsent` column self-applies **greenfield** (no pre-existing rows to migrate), so the column exists but the against-real-data migration proof is **deferred** (to alpha, or the next real prod schema change). The **consent UI** (the human consent moment) is a separate, not-yet-built item → **Wave 3 GATE** below.
...
- **Chat thread → reactive AI chat** — **✅ DONE** (Child 1 composable data-plane + Child 2 query subscriptions + **Child 3 `reactive-ai-chat` shipped 2026-07-01**, see Current focus). Each chat message = a `Message` Resource FK'd to a `Session`; one query sub `Message where session==DEFAULT_SESSION_ID` gives history-restore / disconnect-recovery / multi-participant; relocated off Galaxy onto DevStudio ([`tasks/archive/nebula-reactive-ai-chat.md`](archive/nebula-reactive-ai-chat.md)). Live turn-delivery prereq shipped 2026-06-29 ([`archive/resilient-turn-delivery.md`](archive/resilient-turn-delivery.md)). **Rejected alt (2026-06-29) — merge Galaxy+DevStudio:** DevStudio's heavy startup (`@cloudflare/shell` + `isomorphic-git` + `@vue/compiler-sfc` + codegen) + long in-DO model-inference `await`s would contend on Galaxy's single-threaded tenant-facing ontology-read path, plus a DO-class + data migration on deployed prod — stay separate, relocate turns instead.
...
- **DevContainer wakeup-after-hibernation — DIAGNOSIS DONE + FIX designed & spiked (2026-07-01).** Diagnosis archived at `tasks/archive/nebula-container-wakeup-recovery.md`: the failure is **(b)** the CF stale-`running` flag — a lifecycle problem, NOT the wake *call* failing → **orthogonal to the mesh work; `callDurable` does not help.** Fix = `ctx.abort()` on stuck-detect on the preview-GET path, entirely **below the mesh layer** (touches only `dev-container.ts` + its test — never the shared `callRaw`/DevStudio surface). Two `/review-task` Stage-2 passes done; **Phase 0 spike confirmed the abort lever** on real CF, but the stuck flag **self-heals in every off-prod scenario** → it's a **rare-race safety net**, not the common path. **DONE + COMMITTED (2026-07-04, `8d122c9`) + ARCHIVED** at `tasks/archive/nebula-container-wakeup-fix.md`: Phases 1–2 built + verifier-panel + ui-smoke-verified; the persist-before-abort durability was deploy-tested on CF (a macrotask yield between the sync `kv.put` and `ctx.abort()` is load-bearing — [[miniflare-local-abort-wipes-storage]]). Phase 3 deploy-verify against a real prod stuck stays opportunistic.
...
- Resilient chat delivery (DONE 2026-06-29): `tasks/archive/resilient-turn-delivery.md` · preview auto-refresh (DONE 2026-06-29): `tasks/archive/preview-ready-autorefresh.md` · query subscriptions (DONE, archived): `tasks/archive/nebula-query-subscriptions.md` · reactive AI chat (Child 3, DONE + archived — message = child Resource FK'd to Session; option-(b) transient stream + durable Message): `tasks/archive/nebula-reactive-ai-chat.md`
