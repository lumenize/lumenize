/**
 * Local live tests for @lumenize/proxy-fetch production Worker
 * 
 * Run `wrangler dev` in another terminal first, then run this test.
 * 
 * Usage: node test/production/local-live.test.mjs
 */
...
async function waitForCallback(reqId, maxAttempts = 20, delayMs = 200) {
  for (let i = 0; i < maxAttempts; i++) {
    ...
    console.log(`  Attempt ${i + 1}: still pending...`);
...
test('Health check', async () => {
  ...
  assert.strictEqual(data.service, 'proxy-fetch-live-test');
