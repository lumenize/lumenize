/**
 * Factory mechanics against the faithful mock client (jsdom `frontend` project).
 *
 * Ported from the spike's `factory-basics.test.ts`. The factory no longer owns
 * the queue/engine — the MockClient runs the real conflict-outcome engine over a
 * recording transport, and the factory injects its Vue-reactive store adapter
 * via `client.bindStore(...)`. So these exercise the real factory ↔ engine ↔
 * store data flow (debounce, optimistic paint, commit/rollback/use-server),
 * minus a Star. Each test targets a pinned-semantic invariant.
 */
...
function setup(initialState: Record<string, any> = {}) {
  ...
  // submission mechanics, not debounce timing (that's debounce.test.ts).
...
describe('middleware', () => {
  ...
  it('middleware sees context.source flowing through internal writes', () => {
    ...
    // Server fanout uses source: 'remote' (via the bound adapter's applyFanout).
    client.simulateFanout('todo', 'task-1', { value: { title: 'hello' }, meta: { eTag: 'abc' } });
    // Connection state uses source: 'remote' (via the factory's observer).
...
// ──────────────────────────────────────────────────────────────────────────
// PINNED #5: Synced-state middleware emits a debounced write when the user
// writes under resources.<rt>.<rid>.value.* (→ client.resources.write).
// ──────────────────────────────────────────────────────────────────────────
describe('synced-state middleware', () => {
  it('submits a transaction with current eTag on local write under resources.*.value', async () => {
  ...
  it('does NOT submit when no meta.eTag exists (never-subscribed resource)', async () => {
    const { store, client } = setup({});
    store.resources = { todo: { 'task-orphan': { value: { title: 'orphan' } } } };
    // Now write under .value — no baseline eTag, so the synced-state middleware
    // must drop it (and warn) rather than submit against a phantom baseline.
    store.resources.todo['task-orphan'].value.title = 'edited';
  ...
  it('does NOT submit for remote/rollback/computed writes', async () => {
    const { store, client } = setup({
      resources: {
  ...
  it('rolls back optimistic write on validation-failed outcome', async () => {
    ...
    client.txnResponder = () => ({ result: 'validation-failed', errors: { bad: true } });
  ...
  it('writes server snapshot on conflict → default use-server outcome', async () => {
    const { store, client } = setup({
      resources: { todo: { 'task-1': { value: { title: 'orig' }, meta: { eTag: 'eTag-v1' } } } },
    });
    // Server returns a conflict fact; the engine's default handler resolves it
    // as `use-server`, adopting the server snapshot.
    client.txnResponder = () => ({
      result: 'conflict',
...
describe('connection state', () => {
  ...
  it('is pre-seeded so first-paint reads are defined (not undefined)', () => {
    const { store } = setup({});
    expect(store.lmz.connection.state).toBe('disconnected');
    expect(store.lmz.connection.connected).toBe(false);
    expect(store.lmz.connection.lastConnectedAt).toBeUndefined();
  });

  it('a binding on store.lmz.connection.connected re-fires only when `connected` changes', () => {
    const { store, client } = setup({});
    const seen: boolean[] = [];
    effect(() => { seen.push(store.lmz?.connection?.connected ?? false); });

    client.simulateConnectionState('connecting'); // connected stays false → deduped, no re-fire
    client.simulateConnectionState('connected');  // false → true → re-fire
    client.simulateConnectionState('reconnecting'); // true → false → re-fire

    // Seeded `connected: false`, so 'connecting' (also false) is deduped by the
    // top-level deep-equal skip — unlike the spike, which seeded `connection: {}`
    // (undefined) and saw an extra leading entry.
