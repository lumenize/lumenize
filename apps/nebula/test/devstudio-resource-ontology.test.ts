/**
 * DevStudio Session/Message ontology — compile-level + bundle-id unit checks
 ...
 *   1. `Message.session` is authored as a real to-one relationship to `Session`
...
import { compileOntologyVersion, SESSION_MESSAGE_TYPES, SESSION_MESSAGE_ONTOLOGY_VERSION, SESSION_MESSAGE_BUNDLE_ID } from '@lumenize/nebula';
import { TOOL_ARGS_BUNDLE_ID } from '../src/codegen-loop';

describe('devstudio Session/Message ontology', () => {
  it('authors Message.session as a to-one relationship to Session (write-shape + ADR-006 embed-guard apply)', () => {
    ...
    expect(row.relationships.Message?.session).toMatchObject({
      target: 'Session',
      cardinality: 'one',
    });
  });
  ...
  it('bundle id is disjoint from the tool-args id and from Star\'s {galaxyId}/{version} form (review M2)', () => {
    ...
    expect(SESSION_MESSAGE_BUNDLE_ID).not.toBe(TOOL_ARGS_BUNDLE_ID);
    ...
    const preSlash = SESSION_MESSAGE_BUNDLE_ID.split('/')[0];
