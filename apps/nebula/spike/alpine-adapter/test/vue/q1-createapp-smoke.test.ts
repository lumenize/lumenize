 ...
 * template strings + `v-*` directives) works inside jsdom. If this fails,
 * the rest of the spike is moot.
 *
 * Three asserts:
 *   1. `createApp` mounts a component on a jsdom element and compiles
 *      its inline template.
 *   2. Reading a `ref` inside the template renders its initial value.
 *   3. Writing to the ref schedules a re-render (next tick) and the DOM
 *      reflects the new value.
 */
import { describe, it, expect } from 'vitest';
import { loadVue } from './harness';
