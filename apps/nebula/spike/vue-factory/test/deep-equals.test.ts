/**
 * deepEquals property tests — the second pure helper in detour #1
 * (tasks/nebula-frontend.md § Pre-v3 isolation detours). The helper predates
 * this suite (copied from `@lumenize/state` for the spike's write-dedup path);
 * these tests pin its documented semantics for the v3 port: plain JSON-shaped
 * data + Date — NOT Maps, Sets, TypedArrays, or class instances.
 */
...
describe('cycles and aliases (ADR-002 — structured-clone value space)', () => {
  ...
  it('Map/Set compare by reference only — never falsely equal', () => {
    ...
    expect(deepEquals(new Map([['a', 1]]), new Map([['a', 1]]))).toBe(false); // documented: no deep Map support in the spike
