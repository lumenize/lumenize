/**
 * 3-way text merge (LCS-based) for long-form text fields — pre-v3 isolation
 * detour per `tasks/factory-textmerge.md`. Doc-as-spec:
 * `website/docs/nebula/api-reference.md` § textMerge. Ports to
 * `@lumenize/nebula-frontend`'s top level during v3; hand-rolled (no
 * diff-match-patch dependency) per the spec's build-vs-borrow D0.
 *
...
// in the ontology pipeline; this detour owns only the merge function and this
// resolver shape (tasks/factory-textmerge.md § Auto-registration). Shapes
// follow api-reference.md (v3 names — NOT the spike client's old
// `ConflictResolution`/`resolution:` discriminant).
// ──────────────────────────────────────────────────────────────────────────
