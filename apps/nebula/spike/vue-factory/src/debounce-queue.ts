/**
 * Debounce + serial-per-resource submission queue — pre-v3 isolation detour
 * per `tasks/debounce-serial-queue.md` (D0/D1). Validated here against the
 * pinned invariants, then ported to `packages/nebula-frontend/src/debounce.ts`
 * during v3.
 *
 ...
 * ConflictResolverVerdict machine) is `tasks/factory-conflict-outcome.md` —
...
export interface DebounceQueueConfig {
   ...
   *  Default reads the spike client shape `{ resolution: 'committed', eTag }`. */
