...
export class NebulaClientGateway extends LumenizeClientGateway {
  override onBeforeCallToClient(envelope: CallEnvelope, connectionInfo: GatewayConnectionInfo): void {
    // PROFILE-fence (tasks/nebula-profile-store.md § Routing): a push from the global Profile DO carries
    // PUBLIC fields ONLY and public profile read is OPEN (the unguessable profileId handle IS the
    // capability), so cross-scope delivery is intentional — skip the same-aud check. `metadata.caller`
