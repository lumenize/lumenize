...
- **M4 — `@mesh()` decorators.** Only methods decorated with `@mesh` (TC39 stage 3 decorators) are callable over `lmz.call()`; everything else on the node is uncallable. You may then read properties and call methods on whatever that method returned, but none of that ever hands you `ctx`, `env` or `svc` themselves, or an undecorated method. `constructor` and `__proto__` are refused outright.
