...
- **Start capturing metrics now** so the next conversation has a curve, not a point: builders invited/activated, apps created, time-to-first-shipped-app, coach interventions per shipped app. **Add collaborators-invited-per-builder** — see the signal below, which is the thing it would confirm.
- **A real (small) demand signal worth stating out loud (2026-07-31).** Three of the four builders we can observe brought in a second person: Jennifer wants Austen on Gigi's Almanac, Sydney wants Cass on the Secret Santa idea, Timothy invited his brother Matthew. Morgan is the fourth and plans to work alone. That is **3 of 4** against a retired prior of roughly 1 in 10 (`strategy.md` § *The positioning*).
  - ⚠️ **Present it as n=4 with the composition disclosed.** Two of the four (Jennifer, Morgan) are on the team, so this is not an arm's-length sample. An investor who has seen a thousand decks prices a disclosed small-n better than a dressed-up one — and the named counterexample is what makes it read as observation rather than selection.
  - **Timothy's is the load-bearing one** and the one with a gap: it happened on a *competing* agentic builder, so the pull came from the work rather than from our product or pitch. **Research:** confirm which platform (Replit or Lovable) before it goes on a slide, and whether Matthew ended up with real access or just watched.
  - What it supports is the **shape** — the buyer is one person and the workspace is multiplayer — so account expansion rides invitation rather than a sales motion (`enterprise.md` § *The invitation is the land motion*).
