...
>
> **In fairness to the original: ~8 s wasn't invented — but its stated MECHANISM was backwards.** A *cold* prod login does measure **8558 ms**. Decomposing it (2026-07-21) shows why that never justified the conclusion drawn from it: **7245 ms of it is the `POST /email-magic-link` server handler**, and the email arrives **inside that window — 0 ms after the response returns, every run.** So *"it's email-pipeline-bound, not cold-start"* is exactly inverted (it is cold-start-bound and not email-bound), and *"warm and cold both land at ~8s"* is false: warm runs the same handler in **~1.2 s**, a full login in **~2.5 s**. Quote the row matching your scenario: **~8.5 s** cold standalone prod command · **~2.5 s** warm prod login · **~1.4 s** first email in a test run · **~0.9 s** each additional login in that run. And provider cannot matter — it only touches the phase costing ~0 ms.
>
