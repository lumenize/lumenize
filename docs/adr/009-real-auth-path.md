...
>
> **In fairness to the original: ~8 s wasn't invented — it measured the wrong thing for the claim it backed.** A standalone prod login from a fresh Node process does measure **~6.2 s** (verified 2026-07-21), and hand-timing that would land near 8 s. But it was cited to justify *"too slow to put in every unit test"*, where the governing number is the **in-suite marginal cost, ~0.9 s** — a ~7× gap made of process boot, WS handshake and prod cold-start, none of which a test in a warm suite pays. Quote the row that matches your scenario: **~6.2 s** standalone-prod-command, **~1.4 s** first email in a test run, **~0.9 s** each additional login in that run.
>
