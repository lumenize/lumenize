...
- **Nothing enforces this mechanically yet.** Until a shared helper exists, it is a convention each site must follow — and the sites disagree today, which is the evidence it needs writing down. A single `recordActingToken(claims)` helper is the obvious consolidation (ADR-007's composition principle) when the mechanism lands. ⚠️ **Name the stored field for the TOKEN, not for a role** — `actingToken`, this ADR's own phrase. `actingToken.sub` reads as the token's subject, which is what it is; every role name inverts, because `sub` is the person acted *upon* and the actor is `act.sub`. The first site (`executeScopeDeletion`, 2026-07-29) tried `actor`, then `actingClaims`, before landing there.
