...
**Date**: 2026-07-21
**Status**: Proposed — ratification **deferred until [`nebula-passage-dominion-from-scope`](../../tasks/nebula-passage-dominion-from-scope.md) has BUILT**, moved on 2026-08-11 from that task's Stage 2 (which has now run). ⚠️ **This is a different deferral from [ADR-016](016-record-the-acting-principal.md) and [ADR-019](019-derived-artifacts-record-observations.md) — do not read the three as one.** Those are deferred past pre-alpha as a scheduling call. This one is deferred for a specific gap: the text had a **line-by-line review with Larry on 2026-08-10/11** and survived a Stage-2 conformance panel, so the *model* is settled — what it has not had is **code running against it**. That task is the exercise. The same week's implementation-adjacent scrutiny moved this model twice (a member may invite a non-admin peer at their own scope; the mint's four invariants collapsed to one), so ratifying now would ratify on the strength of review alone.
**Deciders**: Larry
