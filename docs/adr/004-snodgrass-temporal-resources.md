...
- Every write closes the current snapshot and opens a new one; `[validFrom, validTo)` intervals tile time with exactly one current snapshot per resource (`validTo = END_OF_TIME` sentinel).
...
- Hot storage (the Star's SQLite) keeps current + recent; deep history offloads to R2 keyed `<scope>/<resourceId>/<validFrom>`, with the Star keeping metadata.
...
| Per-resource history DO | Rejected — capacity, fan-out, and cost; deep history offloads to R2 instead (see the R2 design). |
...
- Audit is built in: "what changed, when, and by whom" (the `actingToken` record rides every snapshot) is a lookup.
