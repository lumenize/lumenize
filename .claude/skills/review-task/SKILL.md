...
A good manifest is numbered, says *why* each entry matters, and ends with the specific source files the task makes claims about. Include the `tasks/backlog.md` sections adjacent to the task's domain — not only the rows the task already cites — because the scope lens's tripped-trigger sweep needs rows the task has never mentioned. Mark superseded/icebox inputs as reference-only so nobody reports their staleness as a finding. Cap inlined ADRs at the `README.md` index plus the two or three plausibly in scope, and tell reviewers to open the rest from disk if they suspect a conflict.
...
const FRAMING_LENSES = [
  ...
  { key: 'scope-discipline', rules: 'CLAUDE.md (repo root) + tasks/README.md + .claude/rules/workflow.md',
    lens: 'Is this scoped to what we actually need NOW? Flag YAGNI strictly per .claude/rules/workflow.md § "YAGNI gates capability, never generality" — READ that bullet and apply it as your checklist rather than from memory. It is the single source for: the present-consumer test (tests count as consumers), the capability-vs-shape distinction, and a COST TEST that REVERSES the default flag direction when the general form costs the same or less than the narrow one. Flag INTERIM-SCAFFOLD HARDENING — complexity (ESPECIALLY guard/security machinery) added to *defend or elaborate a known-temporary artifact* (a placeholder/test scope, a stub, a hardcoded fixture, a deferred-feature interim): ask whether the design would exist at all under the *real / near-term* model, or ONLY because of the throwaway scaffold — if the latter, the fix is to drop the scaffold, not harden it. (A panel optimizes within the frame it is handed; this is the lens that questions whether the frame is itself an artifact.) Flag INTERMEDIATE-GOAL ALIGNMENT — work exceeding what the current milestone (e.g. the demo) needs, or milestone-critical work conflated with post-milestone work. Flag CORRECT DEFERRAL — post-goal work entangled in the active path instead of split to an on-hold file with a pointer. Flag BUILD-ORDER / PREREQUISITES — is the file sequenced right against its dependencies, are blockers named and ordering correct (a prerequisite task that must ship first). Flag TRIPPED BACKLOG TRIGGERS — sweep tasks/backlog.md (the sections the manifest names, plus a grep for trigger language: "real once", "fires when", "Immediacy trigger", "when built") for dormant rows whose stated trigger this task's DELIVERABLES would trip; a tripped trigger with no disposition in the task file is a finding, and the fix is a recorded disposition — fold it in, or an accepted residual with the row updated — never silence. (Live catch, 2026-09-01: the m6 pending-signup single-flight row, whose trigger "real once a stranger can double-click a button" was tripped by the login task's claim affordance — the panel pulled it out of the backlog when nothing in the task cited it.) The fix is usually a scope-cut or a deferral, not more spec. ⏳ PRE-ALPHA EXCEPTION (temporary — remove when pre-alpha ships): a task file need NOT be independently deployable, and interdependence with a later pre-alpha task file is the intended workflow — do NOT flag either as a scope/build-order problem; deferred testing is fine when the later pre-alpha file carries explicit acceptance criteria for it.' },
  { key: 'prose-voice', rules: '.claude/rules/prose-voice.md + docs/vision/auth.md and docs/adr/015-passage-and-dominion.md as the reference voice (read a section of each, not the whole files)',
    lens: 'Is this written the way Larry writes, for a reader who reads every word once? Flag ABSTRACTION WITHOUT ITS CASE — a sentence resting on "the reflex", "the shape", "the mechanism", "what is replaceable" or a similar bare abstract noun, where the concrete case is missing or buried below it; the fix is to state the specific thing and delete the summary above it. Flag UNDEFINED COINAGE — a term used as though already defined, without the definition and the rejected alternatives that ADR-015 gives dominion and passage; a new term needs Larry to approve it first. Flag MISSING EXAMPLE — a section that defines something and shows no literal value. Flag NO JOURNEY — a Context section that never tells the reader what follows, or a decision presented as a conclusion plus justifications rather than problem, first attempt, why it fails, what we do. Flag BACKWARD RESTATEMENT — a point made once and made again later in the same file; a forward-looking summary is fine, a backward one is not. Flag EDITOR NOTES IN THE BODY — commentary about the file\'s own wording, or dated build status, which belong in frontmatter and the task file respectively. Do NOT flag cross-file repetition of a sibling\'s REASONING: one or two sentences sparing the reader a jump is wanted here, and only restated counts, dates, inventories and exact wording are forbidden (.claude/rules/workflow.md). Run `node scripts/check-prose.mjs <file>` and report every budget it fails, but do not stop there — the budgets count symptoms and cannot see whether an example is a good one or whether the reader is being led anywhere.' },
