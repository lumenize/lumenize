...
⚠️ **A suite result is only true of the tree that produced it, so the closing run MUST come after the LAST edit — including every fix the panel prompts.** Report the number that run actually printed, and check the totals moved by the number of tests you added (`testing.md` § baseline). Bit 2026-08-28: a delta was recorded, the panel's fixes then added nine tests and changed production behaviour, and the criterion still carried the pre-fix number — in a phase whose own text, one bullet earlier, had written down that exact lesson. ⚠️ **And do not race it**: editing source under a running suite, or `pkill`ing workerd mid-run, produces output belonging to no tree at all.
