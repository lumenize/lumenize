...
- **ADR-007** — every Lumenize node type shares ONE mesh + security core (`executeEnvelope` behind the `EnvelopeExecutorNode` seam; JWT at the Gateway, `onBeforeCall` guard per node), reused by **composition** — never reimplemented. New node types compose the core regardless of base class (this is what lets a `@cloudflare/containers` `Container`-based node be a full node); per-type divergence needs a documented reason, unjustified divergence is a defect to consolidate. The mechanism is the seam now; the principle is the commitment.
