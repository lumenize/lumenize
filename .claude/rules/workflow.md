...
**Stray non-breaking spaces (U+00A0):** some WYSIWYG markdown editors (e.g. Typora) silently insert them on edit — they break exact-match string edits (the `Edit` tool can't match an "identical" line) and can break rendering. After editing any file a human also has open in such an editor, grep should print nothing — `grep -nP '\xc2\xa0' <file>` — and strip any with `perl -i -pe 's/\xc2\xa0/ /g' <file>`.
