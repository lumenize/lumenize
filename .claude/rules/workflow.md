...
- **ADR-007 (Proposed)** — every Lumenize node type shares one **narrow comms + guards core** — `lmz.call` (receive), `lmz.ctn` (outgoing), `onBeforeCall`, `@mesh()` — by **composition**, never reimplemented. Storage (`lmz.sql`)/alarms/`onStart`/`fetch` are **per-node-type capabilities, NOT the invariant** (a node takes only what it needs — Worker and the container node take neither alarms nor `onStart`). New types compose it regardless of base class (lets the 4th type, a `@cloudflare/containers` `Container`-based node, be a full node without inheriting `LumenizeDO`). Mechanism = `executeEnvelope`/`EnvelopeExecutorNode` (DO+Worker; the Client hand-rolls a parallel path — a divergence to classify). Held **Proposed** until the container pivot proves out (Q5).
