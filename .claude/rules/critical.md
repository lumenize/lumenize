...
- **Never hand-write `Env`** — run `npm run types` (`wrangler types`) and use the generated global `Env` from `worker-configuration.d.ts`. No `interface Env`, no `MyEnv`/`AuthEnv`.
