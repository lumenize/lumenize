...
- **Timestamps**: persisted/wire timestamps are **ISO 8601 UTC ("Zulu") strings** (`new Date().toISOString()` → `2026-07-11T00:00:00.000Z`), stored as SQLite `TEXT`, compared as strings (lexical = chronological); end-of-time sentinel `9999-01-01T00:00:00.000Z`. **Never epoch `number`.** Protocol fields keep their spec (JWT `exp`/`iat` = NumericDate epoch seconds; HTTP = RFC-1123). Background: [ADR-011](../../docs/adr/011-iso-timestamps.md); the clock-pinned-within-invocation caveat above applies regardless of encoding.
