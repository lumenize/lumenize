...
## `callRaw` is mesh — use it for single-hop result locality
`this.lmz.callRaw(binding, instance, this.ctn<T>().method(args))` is a **full mesh call**, NOT raw Workers RPC: it builds an envelope, propagates `callContext` (identity/`callChain`/`state`), and pre/post-processes errors — same as `call()`. The "raw" is only that it *also* accepts a raw operation chain. It is the **awaited request/response** form: `await` it and the value comes back (`call()` never returns a value to the left of `=` — it delivers to a handler).
- **Default to `callRaw` (continuation form) when you need the result *here*** to keep going in the same method — one method, `await`-native. This is the idiomatic single-hop request/response, and it does not violate the never-raw-RPC rule above.
- **Keep multi-hop on `call()` + continuations / direct delivery.** ADR-003: one awaited hop is *transport* (fine); awaiting `callRaw` hop-after-hop across a chain is the request/response-across-hops anti-pattern.
- **⚠️ NEVER an awaited `callRaw` whose result must cross a *client connection*, nor for *long-running* work — use fire-and-forget + direct delivery.** This property, not hop count, is the tell. The client WS hibernates and drops; an awaited result is bound to the originating socket, so a drop strands it and the caller's pending Promise hangs forever (the Studio chat "thinking… forever" bug). Instead the client *fires* the call (3-arg, passing its own `this.lmz.instanceName` — **not** `callChain[0]`, which is only the origin if no hop reset the chain) and the DO delivers the result to a `@mesh()` handler on the client addressed by that `instanceName`; the Gateway routes to whatever socket is current, so a reconnect just lands it on the new one. "Reliable + short" (DO↔DO, DO↔Worker) keeps the awaited `callRaw`; "client-facing or long" does not. Use `{ newChain: true }` when the replying node should be a fresh origin. See [calls.mdx](../../website/docs/mesh/calls.mdx) § Multi-Hop / Direct Delivery and [[client-calls-use-direct-delivery]].
- **The raw operation-chain form of `callRaw` is rare** — reach for it only when you must rewrite the chain (low-level, e.g. `LumenizeClientGateway` DMZ proxying), never for ordinary calls.
- **Cost:** for a fast single-hop call, awaiting `callRaw` costs less wall-clock than a single DO storage write — **storage writes dominate DO bills, not wall-clock** ($1/M rows vs. negligible sub-second duration). Reserve fire-and-forget / two-one-way / alarm patterns for genuinely long (multi-second) calls or true multi-hop direct delivery, not as a default.
...
- **4-arg** (`lmz.call(binding, instance, remote, this.ctn().onDelivered(remote))`) — pairs the call with a **local** result handler. When the remote call settles, the framework invokes `onDelivered(result)` on this DO with the success value, or with the Error object on failure (including structured errors like `ClientDisconnectedError`). Add `{ onErrorOnly: true }` as a 5th arg to skip the success-path dispatch when you only care about failures. **What the error path receives depends on the target:** for a **broadcast to clients** (the canonical `onErrorOnly` use) it's only *delivery* failures — the Gateway's `__executeOperation` returns `ClientDisconnectedError`, never the client's own handler error (delivery to the client is one-way) — so `onErrorOnly` is for delivery reactions (drop a dead subscriber, retry a spurious network hiccup), not for catching the callee's app errors. For a plain **DO→DO** call a remote `@mesh` throw *does* round-trip (`executeEnvelope` returns `{ $error }`, rethrown in `callRaw`).

Use the 4-arg form for reactive cleanup, retry, and observability — anything that reacts to "did the call land?" without `await`ing. The handler runs locally; it needs `@mesh()` **only** if the same handler can also be dispatched *remotely* (e.g. `svc.broadcast`'s tree path forwards it from a tier Worker — which is why `onBroadcastResult` below carries `@mesh()`). A `this.ctn()` handler must be **`public`** (not `private`/`protected`): TypeScript only surfaces `public` members on `Continuation<this>`, and the modifier is erased at runtime so a non-public handler buys nothing while forcing an untyped `(this.ctn() as any)` cast. The "not remotely callable" boundary is the **absence of `@mesh`**, never the visibility. Canonical local-only public handlers: `Star.doTransaction`/`doRead`/`doSubscribe`/`applyFetchedState`. (User docs: [continuations.mdx](../../website/docs/mesh/continuations.mdx).)
