...
class MyDO extends DurableObject {
  ...
  // Required boilerplate. Delegates standard `alarm()` handler to Lumenize Alarms
  async alarm() {
...
class MyDO extends LumenizeBase<any> {
  // No boilerplate required. LumenizeBase delegates `alarm()` handler to Lumenize Alarms
...
1. **Persistence**: All schedules are stored in a SQL table `__lmz_alarms`
...
The alarms table (`__lmz_alarms`) is automatically created with columns for id, callback, payload, type, time, and optional delayInSeconds/cron fields.
