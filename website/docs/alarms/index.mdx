...
De✨light✨ful alarm scheduling for Cloudflare Durable Objects using [continuations](/docs/mesh/continuations). Multiplexes single native alarm to give you unlimited scheduled tasks.

## Features

- **[Continuations](/docs/mesh/continuations)** - Type-safe, serializable task handlers
...
Alarms is built into `@lumenize/mesh` - no separate installation needed:
...
Alarms are **built-in** to `LumenizeDO` - no import or configuration needed:
...
class MyDO extends LumenizeDO<Env> {
  ...
  // No @mesh decorator needed - alarms are internal to this DO
...
| **Setup** | Manual | Built-in to LumenizeDO |
...
- **[Continuations](/docs/mesh/continuations)** - Understanding the continuation pattern
...
- **Built-in to LumenizeDO** - No separate import or setup needed
