...
A de✨light✨ful alarm scheduling package for Cloudflare Durable Objects. Schedule one-time tasks, delayed tasks, and recurring cron jobs - all persisted to SQL storage.

## Features

- **One-time alarms**: Schedule tasks at a specific timestamp
- **Delayed alarms**: Schedule tasks N seconds in the future
...
Cloudflare provides only **one native alarm** per Durable Object. `@lumenize/alarms` multiplexes this single alarm to manage unlimited scheduled tasks with different timing requirements.
