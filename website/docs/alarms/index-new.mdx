...
- **Immediate execution** - Support for `delay <= 0` to trigger alarms ASAP
...
class MyDO extends DurableObject {
  ...
  // Required: Delegate to alarms.alarm()
  async alarm() {
    await this.#alarms.alarm();
  }
  
  async scheduleTask(userId: string, data: any) {
    // Use newContinuation() if not using LumenizeBase
    const continuation = newContinuation<MyDO>();
    
    await this.#alarms.schedule(
      60,
      continuation.processUserData(userId, data)
    );
  }
...
Use `triggerAlarms()` to manually execute alarms in tests without waiting for real time:
...
describe('Alarm Tests', () => {
  test('scheduled task executes', async () => {
    ...
    // Manually trigger alarms (executes overdue tasks)
...
1. **Persistence**: All schedules stored in SQL table `_lumenize_alarms`
...
The alarms table (`_lumenize_alarms`) is automatically created with:
