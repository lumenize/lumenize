...
```typescript @check-example('packages/routing/test/unit/route-do-request.test.ts')
...
## Reusing the policy in your own router

`@lumenize/routing` exports `applyCorsPolicy` and `addCorsHeaders` so any
router that needs the same semantics (preflight handling, allowlist check,
server-side reject) can compose them without re-implementing. `routeDORequest`
uses these internally; `@lumenize/nebula-auth`'s `routeNebulaAuthRequest`
shares the same `CorsOptions` shape and uses the same helpers, so a single
`cors` config governs both auth and gateway routes in the Nebula entrypoint.

The pattern (as used in `routeNebulaAuthRequest`):

```typescript @check-example('packages/nebula-auth/src/router.ts')
...
See: [routeDORequest](/docs/routing/route-do-request) | 
[getDOStub API](/docs/routing/api/functions/getDOStub)
