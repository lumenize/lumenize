...
```bash npm2yarn @skip-check-approved('bash')
...
```typescript @check-example('packages/routing/test/integration/test-worker-and-dos.ts')
...
```
https://example.com[/{prefix}]/{doBindingName}/{doInstanceNameOrId}[/path...]
                                └─┬─────────┘   └─────────┬──────┘
                                  DO class binding name   Instance name or hex id
                                  (kebab → smart match,   
                                  otherwise exact)
```
...
`@lumenize/routing` also exports our own `routeAgentRequest`. It defaults the prefix to 
...
[CORS Support](/docs/routing/cors-support).
...
```typescript @check-example('packages/routing/test/integration/test-worker-and-dos.ts')
import { routeDORequest, routeAgentRequest } from '@lumenize/routing';
