...
- `@mesh()` without a guard exposes methods with no additional checks (auth is already guaranteed)
...
| `@mesh(guard)` not shown | Fine-grained permissions per method. See [Security](./security#method-level-meshguard). |
...
- `instanceName` uniquely identifies the client (typically `sub.tabId` where `sub` is the JWT subject)
