...
`SpellCheckWorker` and `AuthEmailSender` are [named entrypoints](https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/rpc/#named-entrypoints) in the same Worker, bound via `services`. The `AUTH_EMAIL_SENDER` binding is how the Auth DO delivers email — Step 8 below walks you through creating it.
:::
...
[Resend](https://resend.com) is the recommended default — it uses standard `fetch` (no SDK), works natively on Cloudflare Workers, and has a generous free tier (100 emails/day).

1. Sign up at [resend.com](https://resend.com)
2. [Verify your sending domain](https://resend.com/docs/dashboard/domains/introduction) — Resend's dashboard walks you through adding DNS records (SPF, DKIM, DMARC)
...
:::tip Customizing templates or using a different provider
See [@lumenize/auth: Email Provider](/docs/auth/getting-started#email-provider) for template customization, composing with defaults, and bringing your own email provider (Postmark, SES, SendGrid, etc.).
:::
