...
import { LumenizeClient, mesh, type LumenizeClientConfig } from '@lumenize/mesh/client';
...
:::note Service bindings and Email Sending
`SpellCheckWorker` and `AuthEmailSender` are [named entrypoints](https://developers.cloudflare.com/workers/runtime-apis/bindings/service-bindings/rpc/#named-entrypoints) in the same Worker, bound via `services`. The `AUTH_EMAIL_SENDER` binding is how the Auth DO delivers email. The `send_email` binding (`EMAIL`) is how Cloudflare actually dispatches those emails — Step 8 walks you through onboarding the domain.
:::

## Step 8: Set Up Email Delivery {#email-setup}
...
The default email path uses [Cloudflare Email Sending](https://developers.cloudflare.com/email-service/), which is available on the Workers Paid plan (entry tier $5/month). If you'd rather stay on the free tier, see [Using Resend instead](/docs/auth/using-resend-instead) — Resend has a 100-email-per-day free tier.
:::

### 8a. Onboard your domain to Cloudflare Email Sending

In the Cloudflare dashboard: **Email Services → Email Sending → Onboard Domain**. Pick a domain you already host on Cloudflare DNS; Cloudflare adds the required SPF, DKIM, DMARC, and bounce-handling records automatically. See the [Cloudflare Email Sending getting-started docs](https://developers.cloudflare.com/email-service/get-started/send-emails/) for details.

### 8b. Create your AuthEmailSender class

Create a tiny `auth-email-sender.ts` alongside your other source files:

```typescript @check-example('packages/mesh/test/for-docs/getting-started/auth-email-sender.ts')
import { AuthEmailSenderBase } from '@lumenize/auth';

export class AuthEmailSender extends AuthEmailSenderBase {
  from = 'auth@example.com';
  // Override templates, subjects, replyTo, or appName here — see @lumenize/auth docs
}
```

Change `auth@example.com` to an address on the domain you onboarded in 8a. That's it — with the `EMAIL` binding present, your sender delivers via Cloudflare Email Sending; `AuthEmailSenderBase` provides the default HTML templates and subject lines. No API key to manage.

This class is already exported from your Worker entry point (Step 6) and bound via `AUTH_EMAIL_SENDER` and `send_email` in wrangler.jsonc (Step 7). No additional wiring needed.

:::tip Customizing templates or using Resend
See [@lumenize/auth: Email Provider](/docs/auth/getting-started#email-provider) for template customization and composing with defaults. Prefer Resend? See [Using Resend instead](/docs/auth/using-resend-instead).
:::

## (Optional) Step 9: Configure Turnstile and Rate Limiting {#production-hardening}
