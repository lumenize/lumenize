...
While Cloudflare's Workers and Durable Objects with Workers RPC is actor-like, **Lumenize Mesh** extends the model all the way to clients running outside Cloudflare (browsers, node.js, bun, etc.) while adding critical security, functionality, and de✨light✨fulness.
...
Additionally, it's common to use `@lumenize/auth`, `@lumenize/utils` (particularly routeDORequest), `@lumenize/testing`, and `@lumenize/debug` with Lumenize Mesh.
