...
| **[LumenizeContainer](./lumenize-container)** | Cloudflare Container | SQL/KV | A node that *runs a container* (niche) |

All mesh node types have access to the same APIs/utilities, are coded in the same way, and act as full peers, able to both make and receive calls. Yes, even LumenizeClients can receive calls from any other mesh node.
...
Workers RPC requires the caller to know the callee's binding and instance name to make a call — but the callee can't independently learn either. Cloudflare's recent `ctx.id.name` change populates instance name for DOs accessed via `getByName()`/`idFromName()`, and persists it for alarms scheduled after 2026-03-15. But instance name remains undefined when the access used `idFromString()`, for `newUniqueId()` DOs, and for names longer than 1024 bytes. Binding name is never available from inside. Lumenize Mesh sidesteps all of this by storing identity in DO storage on first contact and reusing it across every subsequent invocation regardless of entry path — enabling return addresses for callbacks, tracing, etc.
...
updateDocument(changes: DocumentChange) {
  const { callChain, originAuth } = this.lmz.callContext;
  const sub = originAuth?.sub;           // Who authenticated (e.g., 'alice')
  const origin = callChain[0];           // Who started the chain
  const caller = callChain.at(-1)!;      // Immediate caller
  // Make decisions based on who's calling
}
```

**Ejection/Hibernation-safe** (for DOs): See [Managing Context](./managing-context) for the full guide.
...
Continuations:
- Are **serializable** — can be stored/restored or sent over the wire
- Are **type-safe** — TypeScript checks method names and signatures
- **Carry context** — pass what your handlers will need as additional parameters
...
See **[Making Calls](./calls)** for all patterns including optimizations and error handling.
...
- **[LumenizeContainer](./lumenize-container)** — A node that runs a Cloudflare Container (niche)
