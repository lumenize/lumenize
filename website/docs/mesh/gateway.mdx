...
- **Latency**: This design adds one hop inside of Cloudflare, but the primary driver for latency is the cumulative physical distance of the hops from caller to callee which would only be significantly increased in rare cases. A single hop between regions can be on the order of a hundred milliseconds (we measured ~101 ms RTT IAD↔WAW), but a hop in the same data center is typically under ten milliseconds (we measured ~8 ms RTT IAD↔IAD).

- **Throughput**: Each connected client gets its own Gateway DO instance, so authentication and routing CPU work fans out across multiple DO input gates rather than serializing on the callee. We measured this empirically at peak per-Star load: forcing the same total in-flight calls through a single Gateway DO (no fanout) costs ~14% peak throughput vs the natural one-Gateway-per-client architecture. The fanout is mildly load-bearing — at lower loads the storage commit rate of the Star DO dominates throughput regardless, but at saturation the Gateway-DO fanout is what keeps the Gateway itself off the critical path.
