...
|------|-----------------|------------------|
| `onBeforeAccept` | WebSocket upgrade | Validates `{sub}.{tabId}` format, extracts standard claims |
...
- **`onBeforeAccept`** — validates *who is connecting*. The default confirms the JWT subject matches the instance name so a stolen token can't hijack someone else's Gateway. A multi-tenant app might extend this to verify the tenant ID embedded in the instance name (e.g., `{sub}.{tenantId}.{tabId}`).
...
Called during WebSocket upgrade after the base class decodes the JWT. The `instanceName` parameter is the [client's configured `instanceName`](./lumenize-client#configuration) — the same value that identifies the Gateway DO instance (see [1:1 Gateway-Client Relationship](#11-gateway-client-relationship)). The base class always extracts `sub` and `tokenExp` (protocol-level concerns) before calling this hook.
...
- **`Record<string, unknown>`** — proceed; these key-value pairs are stored as `claims` in the WebSocket attachment and flow into `callContext.originAuth.claims`
- **`undefined`** — proceed with no claims

The default implementation validates that `instanceName` follows `{sub}.{tabId}` format and extracts `emailVerified`, `adminApproved`, `isAdmin`, and `act` claims.
...
`GatewayConnectionInfo` exposes only the identity portion of the connection — not protocol internals:
...
The binding name used in `verifiedOrigin` and caller metadata. Override when your Gateway registers under a different binding.

```typescript @skip-check
protected gatewayBindingName = 'LUMENIZE_CLIENT_GATEWAY';  // default
```
...
export class MyGateway extends LumenizeClientGateway {
  protected override gatewayBindingName = 'MY_GATEWAY';
  ...
  override onBeforeAccept(
    ...
    return { tenantId: segments[1], role: jwtPayload.role };
  }
