...
Every mesh node communicates via `this.lmz.call()`. Any node type can call any other — DOs, Workers, and clients are true peers in the mesh.
...
|------------|-------------|---------------|
| **Identity propogation** | None — must use an `init()` pattern | Automatic 
| **Call context** | None — must pass manually | Automatic `callContext` with `origin`, `originAuth`, `caller`, `state` |
| **Consistent Robust Type Support** | Cap'n Web is subset | Date, Set, Map, cycles, aliases, etc. |
| **Browser clients** | Not supported | Full peer with same type support |
| **Access control** | Manual per-method | `@mesh` guards + `onBeforeCall` hooks |
| **Uniform API** | Different patterns for DO vs Cap'n Web | Same `this.lmz.call()` everywhere |
...
generateSummary() {
  ...
  const { callChain, origin } = this.lmz.callContext;
  const clientId = (callChain.at(-1) ?? origin).instanceName;  // Capture requesting client
  this.lmz.call(
    'LLM_WORKER',
    undefined,
...
async summarize(content: string, clientId: string) {
  ...
  const { callChain, origin } = this.lmz.callContext;
  const caller = callChain.at(-1) ?? origin;
  this.lmz.call(
    caller.bindingName,
    caller.instanceName,
...
generateSummary() {
  const content = this.ctx.storage.kv.get('content');
  const { callChain, origin } = this.lmz.callContext;
  const clientId = (callChain.at(-1) ?? origin).instanceName;
  this.lmz.call(
    'LLM_WORKER',
    undefined,
    this.ctn<LlmWorker>().summarize(content, clientId)
...
| Error | Cause |
|-------|-------|
| Method not found | Method doesn't exist or lacks `@mesh` decorator |
...
Errors thrown by `@mesh` guards or `onBeforeCall` are **not wrapped** — they pass through unchanged. This preserves your domain-specific error types (e.g., `PermissionDeniedError`, `QuotaExceededError`) so you can handle them appropriately.
:::
...
- **[Manual persistence](./managing-context#manual-persistence-power-users)** — store/restore the continuation yourself using `getOperationChain()` and storage
- **[Two One-Way Calls](#cost-optimization-two-one-way-calls)** — pass the handler context (or full continuation) along with the call and return it when called back
- **[@lumenize/alarms](/docs/alarms)** — automatically persists continuations to storage with context capture and restoration
:::
...
Sometimes, like in fan-out scenarios, you want to explicitly start a fresh call context rather than continue it across a multi-hop call chain. Use the `{ newChain: true }` option:

```typescript @skip-check
// Fan-out to subscribers — each gets a fresh context
for (const subscriberId of subscribers) {
  this.lmz.call(
    ...
    undefined,
    { newChain: true }  // Subscribers don't receive caller's context
  );
}
```
