...
- **Direct client-to-client calls**: Disabled by default (DO/Worker-mediated pushes are allowed). See [Opting In to Direct Client-to-Client Calls](./lumenize-client#opting-in-to-direct-client-to-client-calls) to enable.
...
| **Identity propagation** | Binding name unavailable from inside; instance name populated for `getByName`/`idFromName` access (and post-2026-03-15 alarms) but undefined for `idFromString`, `newUniqueId`, and > 1024-byte names | Both populated on first mesh entry, persisted in storage, and reused across every subsequent invocation regardless of entry path |
...
**We strongly recommend that you use `this.lmz.call()` for all in-mesh calls**. Only when you are calling a non-mesh node (raw DO, Worker, or RpcTarget) should you drop down to Workers RPC.
...
The target binding is validated **synchronously, at the call site**: a misrouted call — an unknown binding, a Worker/service binding given an instance name, or a DO binding missing one — throws immediately from `this.lmz.call(...)`, even here where there's no handler to receive it (previously such mistakes were silently dropped). A failure *after* dispatch (the callee rejecting at its early ack, or throwing while it works) has no handler on a fire-and-forget call, so it is **logged**, never thrown asynchronously.
...
**Early ack under the covers:** There's no `await` in your code, and the caller doesn't hold one across the work either. The callee **acks early** — the instant it's admitted (binding resolved, guards passed), *before* it runs the work — so the caller is freed at that brief ack (which does open input gates), not held for the operation. The result arrives later as a one-way **fire-back** into your handler, addressed so it survives a DO hibernation or a client WS reconnect rather than stranding on a held channel. A long-running 4-arg call therefore no longer keeps the caller in wall-clock billing — the hazard the [two one-way calls pattern](#two-one-way-calls) below used to exist to avoid. That pattern is now a cost lever for moving the *work* to a CPU-billed Worker, not a fix for a held await.
...
Every failure resolves to one of three places, divided by the **early-ack boundary** (the point where the callee admits the call, before it runs the work):

- **Validation / developer error** — a bad binding or an invalid continuation throws **synchronously** at the `this.lmz.call(...)` site, before anything is dispatched. Fail-fast; never routed to a handler.
- **Admission / guard reject** — a scope/permission failure (`onBeforeCall`), a non-`@mesh` target, or overload comes back on the callee's **early ack** (before the work runs) and is delivered to your handler as the `Error` in its `$result`. On a fire-and-forget (3-arg) call there's no handler, so it's logged.
- **The `@mesh` method threw** — a post-ack error in the callee's own code rides the **fire-back** into your handler's `$result`.

So a response handler receives **every** outcome — value *or* `Error` — which is exactly why you always check `result instanceof Error` before using it. The lone gap is a callee that acks and then vanishes mid-work (crash or eviction *after* the ack): no signal comes back. That's rare, is already true of fire-and-forget today, and is the job of the deferred guaranteed-delivery tier (`callDurable`), not the default `call`.
