...
Note how we extract the originating client's ID from `callChain` — an array that grows with each hop, tracking every node in the current call chain (`[origin, hop1, hop2, ...]`):
