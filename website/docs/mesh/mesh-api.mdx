...
- `originRequest` — HTTP facts of the WebSocket upgrade that started the chain: the origin's IP address, `User-Agent`, `Accept-Language`, and Cloudflare's `cf` location fields. **Immutable**. Only set for LumenizeClient origins, and **server-side only**: the Gateway never forwards it to a client, so inside a LumenizeClient it is always `undefined`.
