...
5. **Before each call**, the client checks its own token: within 30 s of expiry (or already past it), the call is **queued**, the token refreshed, and the socket **rotated** — a new connection authenticated with the fresh token, on which the queued call is then delivered. A call is never sent on a socket whose token is due.
6. **On each inbound message**, Gateway checks the expiration stored at upgrade — if expired, it closes with `4401` and that message is dropped. This is the backstop, not the normal path: with step 5 it fires only for revocation, key rotation, or clock skew.
7. **On 4401**, client refreshes via HTTP and reconnects automatically. Anything queued while disconnected is delivered once the new connection reports ready; a message the Gateway already dropped is not replayed, which is why step 5 keeps calls off a stale socket in the first place.
...
The same payload is exposed on the client side as [`client.claims`](./lumenize-client#client-identity-clientclaims) — server-side code reads `originAuth.claims`, client-side code reads `client.claims`. Both are frozen and re-derived from the verified JWT.
...
- **Every other claim the payload carries** — becomes `originAuth.claims`. This is a derivation, not
  a fixed list: whatever the verified JWT holds at its top level shows up on `originAuth.claims`
  under the same name, so an auth provider that mints a new claim needs no Gateway change. With
  `@lumenize/auth` that means its [`AuthClaims`](../auth/#jwt-claims) (`emailVerified`,
  `adminApproved`, `isAdmin`) plus `act`, because `createJwtPayload` spreads custom claims **flat**
  onto the token rather than nesting them.

If the `Authorization` header is missing or the JWT lacks a `sub` claim, the Gateway rejects the WebSocket upgrade with 401.

**Instance name format requirement**: The default Gateway validates that the instance name follows `{sub}.{tabId}` format, where `sub` matches the JWT subject claim. If the prefix doesn't match, the connection is rejected with 403. This prevents connection hijacking where an attacker with a valid token tries to connect to someone else's Gateway instance.

:::tip Customizing connection validation and claims
If your application uses a different instance name format or extracts different JWT claims, override [`onBeforeAccept`](./gateway#onbeforeacceptinstancename-sub-jwtpayload) in a Gateway subclass. You can also use [`onBeforeCallToMesh`](./gateway#onbeforecalltomeshbasecontext-connectioninfo) to enrich `CallContext` with security-critical fields — add them as top-level properties (not in `state`) so they're immutable by convention. See [Gateway: Extensibility](./gateway#extensibility) for the full API and examples.
:::
