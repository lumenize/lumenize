...
| `instanceName` | `string` | — | **Required.** Format: `{userId}.{tabId}`. Gateway validates userId matches auth. |
...
For method-level guards, see [Security](./security#method-level-guards).
...
For call errors, see [Making Calls: Error Handling](./calls#error-handling). For auth close codes, see [Security: Auth Close Codes](./security#auth-close-codes).
...
See [Mesh API](./mesh-api#api-reference) for the full `LumenizeClient` class reference including `connectionState`, `lmz`, `connect()`, `disconnect()`, `onBeforeCall()`, and `ctn<T>()`.
