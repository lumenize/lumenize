...
// Minimal config — instanceName auto-generated, baseUrl defaults to origin,
// refresh defaults to /auth/refresh-token
...
| `refresh` | `string \| () => Promise<{ access_token, sub }>` | `/auth/refresh-token` | Token refresh endpoint or custom function. Both must return `sub`. |

Both forms return `{ access_token, sub }`. The client uses `sub` to auto-construct `instanceName` as `${sub}.${tabId}`.
...
No manual handling needed. `LumenizeClient` automatically detects tab wake-up, reconnects, and triggers `onSubscriptionRequired` to re-establish subscriptions.
...
interface LumenizeClientConfig {
  baseUrl?: string;
  instanceName?: string;               // Auto-generated from refresh sub + tabId
  ...
  sessionStorage?: Storage;             // For testing (tab ID)
  BroadcastChannel?: typeof BroadcastChannel;  // For testing (duplicate-tab detection)
}
