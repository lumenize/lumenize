...
- **`instanceName`**: `string` — Unique client identifier. **Required format: `{userId}.{tabId}`** — The Gateway validates that the `userId` prefix matches the authenticated user's ID to prevent session hijacking.
...
- **`onLoginRequired`**: `(error: LoginRequiredError) => void` — Called when re-login required (not for routine token refresh)
...
LumenizeClient has almost the exact same **secure-by-default** model as LumenizeDO and LumenizeWorker. The exception is that calls originating from other clients are rejected by default:

With this default:
- ✅ Calls from LumenizeDOs and LumenizeWorkers are allowed
- ✅ Calls that originated from this same client instance (responses to your own requests that flow back through the mesh)
- ❌ Calls originating from other clients are rejected
...
Token expiration (4401) is handled automatically — refresh and reconnect happen transparently. `onLoginRequired` only fires when the user must re-login:

```typescript @skip-check
using client = new EditorClient({
  accessToken,
  onLoginRequired: (error) => {
    // Only fires when re-login is required (refresh token expired)
...
`LumenizeClient` allows the injection of `WebSocket` and `fetch` via options.
@lumenize/utils Browser implements simulated cookie, cors, etc. behavior to
...
it('tests client-DO communication', async () => {
  ...
  using client = new EditorClient({
    ...
    WebSocket: browser.WebSocket,  // Cookie-aware WebSocket
    fetch: browser.fetch,  // Cookie-aware fetch for token refresh
  });
...
interface LumenizeClientConfig {
  ...
  WebSocket?: typeof WebSocket;  // default: globalThis.WebSocket
...
interface LoginRequiredError extends Error {
  reason: string; // Reason for login requirement
}
```
