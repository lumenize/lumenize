...
### Using `state` for Tracing

The mutable `callContext.state` property is ideal for distributed tracing. Populate it in `onBeforeCall` and accumulate data as calls flow through the mesh:

```typescript @skip-check
// In your base class or each node's onBeforeCall
onBeforeCall() {
  super.onBeforeCall();

  // Start a trace if this is a new chain
  if (!this.lmz.callContext.state.traceId) {
    this.lmz.callContext.state.traceId = crypto.randomUUID();
  }

  // Add this node as a span — callChain already tracks the path!
  this.lmz.callContext.state.spans ??= [];
  this.lmz.callContext.state.spans.push({
    node: `${this.lmz.bindingName}:${this.lmz.instanceName}`
  });
}
```

Since `state` propagates across the entire call chain, you get a complete trace without external infrastructure. At chain end, log or store the accumulated spans.

:::tip
The `callChain` array already captures the call path (`[origin, hop1, hop2, ...]`). For simple tracing, you might not even need custom spans — just use `callChain.map(n => n.bindingName).join(' → ')`.
:::
...
- Use **[Two One-Way Calls](./calls#two-one-way-calls)** — both kinds of contexts are fully contained in the outgoing call. When the call back is made, it'll reawaken the instance. The `lmz.callContext` is automatically restored, and you can include whatever context you want in the callback.
- Use **[Alarms](./alarms)** — persists alarm handler continuation including `lmz.callContext` and your continuation parameters to storage so they are available when the alarm triggers.
