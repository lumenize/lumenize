...
1. **Call Context (`this.lmz.callContext`)**: For information about the caller/callee including mesh node type and auth as well as custom annotations (`callContext.state`) added by middleware, like the `onBeforeCall` auth hook, to assist with later access control decisions, like made in the `@mesh()` handler.
...
For the canonical example of using `state` to cache session data for access control, see [Security: State-Based Access](./security#state-based-access).
