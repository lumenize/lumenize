...
it('collaborative document editing with multiple clients', async () => {
  ...
  await testLoginWithMagicLink(aliceBrowser, 'alice@example.com');
  ...
  await testLoginWithMagicLink(bobBrowser, 'bob@example.com');
...
// Requires LUMENIZE_AUTH_TEST_MODE=true in your vitest.config miniflare bindings
const { sub } = await testLoginWithMagicLink(browser, 'alice@example.com');
...
`LumenizeClient` accepts several dependencies in its constructor for testing. Use `Browser` for cookie-aware HTTP/WebSocket, and `Browser.context()` for per-tab sessionStorage and BroadcastChannel:
...
const client = new EditorClient({
  baseUrl: 'https://localhost',
  refresh: 'https://localhost/auth/refresh-token',
  // Inject browser-level implementations (shared across tabs)
  fetch: browser.fetch,
  WebSocket: browser.WebSocket,
  // Inject context-level implementations (per-tab)
  sessionStorage: ctx.sessionStorage,
  BroadcastChannel: ctx.BroadcastChannel,
});
```

For simpler tests that don't need tab ID isolation, `browser.fetch` and `browser.WebSocket` are sufficient — the client falls back to `globalThis` for sessionStorage and BroadcastChannel.
...
Enable test mode for magic link authentication in your vitest config's miniflare bindings (never in wrangler.jsonc):

```javascript @skip-check
// In vitest.config.js poolOptions.workers.miniflare
miniflare: {
