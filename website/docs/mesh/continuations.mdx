...
You own classes that extend `LumenizeDO`, `LumenizeWorker`, or `LumenizeClient`, have access to `this.ctn()`:
...
You can use `$result` anywhere in the arguments — it's just a marker that gets substituted at execution time. If the remote method throws, `$result` becomes that `Error` instance, so your handler should always check `if (result instanceof Error)` first.
