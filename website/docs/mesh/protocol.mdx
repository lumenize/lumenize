...
- **Exception**: a thrown error arrives as a plain `Error` without the callee's stack, and before compatibility date 2026-04-21 it also loses its `name` and custom properties
...
interface CallContext {
  ...
  /**
   * HTTP facts of the originating upgrade, stamped by the Gateway — client-originated chains only
   * (immutable). Server-side only: the Gateway never forwards it to a client.
   */
  originRequest?: { cf?: OriginCf; ip?: string; origin?: string; userAgent?: string; acceptLanguage?: string };
...
| `callContext.originRequest` | Native | Never sent |
...
**Note:** `callContext.originRequest` never crosses a client's WebSocket, in either direction. The Gateway builds it from the upgrade request on the way in, and leaves it out of every call it forwards to a client. It holds the origin's IP address, location and browser, and a push that inherits the writer's call chain would otherwise deliver them to every subscriber. `callContext.originAuth` does go down to the client.
...
Workers RPC loses a thrown error's class and stack, and before compatibility date 2026-04-21 its `name` and custom properties too. To preserve full error fidelity on every compatibility date, `__executeOperation` returns wrapped results instead of throwing.
:::
...
The framework's private single-hop transport unwraps the awaited RPC response:
...
```typescript @skip-check
{
  ...
  callContext: {
    ...
    // No originRequest: it stays server-side
