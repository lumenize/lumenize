...
For long-running calls like the two above, use a "two one-way call" pattern: DO fires-and-forgets to Worker, Worker calls back later with result. See [Making Calls: Two One-Way Calls](./calls#two-one-way-calls) for the full pattern.

## API Reference

See [Mesh API](./mesh-api) for the full `LumenizeWorker` class reference including `env`, `lmz`, `onBeforeCall()`, and `ctn<T>()`.
