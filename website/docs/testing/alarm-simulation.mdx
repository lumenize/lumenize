...
:::danger[Deprecated - Not Maintained]
Alarm simulation is **deprecated and will not be maintained going forward**. 

**For `@lumenize/alarms`**: Use [`triggerAlarms()`](/docs/alarms#testing-alarms) instead - it's more reliable, deterministic, and tests the actual production code path.

**For 3rd party alarm systems** (like `@cloudflare/actors` Alarms): This feature remains available for testing external alarm multiplexing systems that you cannot control, but it is unreliable due to timing simulation limitations and may not work correctly.

**For plain Cloudflare alarms**: Use Cloudflare's `runDurableObjectAlarm()` from `cloudflare:test` or test alarm handlers directly via RPC.
:::

## Overview

**Why deprecated**: Alarm simulation attempts to fake time passing using `setTimeout()`, which is fundamentally unreliable in test environments. It adds complexity and timing issues without providing value over direct execution methods like `triggerAlarms()`.

**Current purpose**: Testing 3rd party alarm multiplexing systems (like Actor Alarms) where you don't control the alarm execution logic.

**Original features** (not recommended):
- Automatic alarm execution based on scheduled time
- Transparent mocking of `ctx.storage.setAlarm()` API
- Configurable speed (100x faster)
- Full retry simulation with exponential backoff
- Works for simple single-alarm cases
...
**Why this is needed**: `schedule()` is synchronous and only stores to SQL - it never executes inline. Even 0-second delays go through Cloudflare's native alarm queue. Tests need to call `triggerAlarms()` to bypass the native timing and execute alarms immediately.
...
describe('Alarm Execution', () => {
  it('executes scheduled tasks', async () => {
    ...
    // Schedule alarms (synchronous, stores to SQL only)
...
**What `triggerAlarms()` does**:
- Core execution logic used internally by production `alarm()` handler
- Production: `alarm()` fires → calls `triggerAlarms(overdueCount)` → executes only overdue alarms
- Testing: Call manually to fast-forward through alarms without waiting

**Benefits:**
- ✅ No timing issues - deterministic execution
