...
### Option 1: Test Alarm Handlers Directly via RPC

Instead of relying on alarm timing, test your alarm callback handlers directly using the [Testing Client](/docs/testing/usage):
...
### Option 2: Use `runDurableObjectAlarm()` from `cloudflare:test`
...
We're planning to add a `triggerNextAlarm()` helper to `@lumenize/alarms` that will:
- Trigger only the next round of scheduled alarms
- Work like `runDurableObjectAlarm()` but for multiplexed alarms
- Enable full alarm testing without timing issues

See [GitHub Issue #XXX](https://github.com/lumenize/lumenize/issues) to track progress.
