...
`Browser.context().fetch` also sends preflight OPTIONS requests just 
...
1. Explicit OPTIONS testing (for server-side validation)
2. Automatic preflight behavior (POST with custom headers triggers automatic 
   OPTIONS)
...
it('shows testing CORS preflight OPTIONS requests', async () => {
  ...
  // Test 1: Explicit OPTIONS request (simulating preflight)
  // This is what browsers send before non-simple requests
  const preflightResponse = await context.fetch(
  ...
  expect(preflightResponse.status).toBe(204); // No Content
  expect(preflightResponse.headers.get('Access-Control-Allow-Origin'))
    .toBe('https://safe.com');
  
  // Test 2: POST with custom headers - automatic preflight is sent!
  // Because this POST has Content-Type: application/json and a custom header,
  // context().fetch will AUTOMATICALLY send a preflight OPTIONS request first,
  // then send the actual POST. This matches real browser behavior.
  await using client = createTestingClient<MyDOType>('MY_DO', 'preflight');
  await client.ctx.storage.put('count', 0);
  
  const postResponse = await context.fetch(
    'https://safe.com/cors/my-do/preflight/increment',
    {
      method: 'POST',
      headers: {
  ...
  // Test 3: OPTIONS from disallowed origin should fail with CORS error
  // Per CORS spec, server responds to OPTIONS (204) but without CORS headers
  // Browser's CORS validator sees missing headers and throws
  const evilContext = browser.context('https://evil.com');
  
  // Preflight OPTIONS gets 204 but without CORS headers - triggers CORS error
  await expect(async () => {
  ...
  // Test 4: The actual POST should also fail with CORS validation error
  await expect(async () => {
    await evilContext.fetch('https://safe.com/cors/my-do/preflight/increment', {
      method: 'POST',
