...
```typescript @check-example('packages/lumenize-base/test/for-docs/basic-usage.test.ts')
...
To create your own NADIS-compatible service:

1. Register a factory function in `globalThis.__lumenizeServiceRegistry`
2. Augment the `LumenizeServices` interface via TypeScript declaration merging
3. Ensure your service works standalone (without `LumenizeBase`)

See `packages/core/src/sql.ts` or `packages/alarms/src/alarms.ts` for implementation examples.
