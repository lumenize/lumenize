...
- **Full Type Support**: All primitives, special numbers (`NaN`, `Infinity`, `-Infinity`), built-in types (`Date`, `RegExp`, `Map`, `Set`), typed arrays (a subclass such as Node's `Buffer` arrives as its built-in type, as with native `structuredClone()`), and Web API objects (via RequestSync/ResponseSync)
- **Error Fidelity**: Preserves stack traces, `cause` chains, custom properties, and subclass types (including custom Error classes!)
- **Circular References**: Automatically handled — shared and cyclic subgraphs are hoisted into `meta.aliases` and referenced by `{ $ref: id }`
...
- **Objects it has no encoder for**: `CryptoKey`, `Blob`, `ReadableStream`/`WritableStream`, `WeakMap`, `Promise` and their kin throw a `DataCloneError`, the error native `structuredClone()` throws for what it can't clone. Their state lives outside their properties, so a copy would arrive broken. The message names the type and where it sat in your value, such as `at session.keys[0]`. Class instances still arrive as plain objects.

## Attribution

Inspired by:
- **SuperJSON** (nested-document + meta sidecar shape) - https://github.com/blitz-js/superjson
- **Cap'n Web** (inline-tagged special types) - https://github.com/cloudflare/capnweb
