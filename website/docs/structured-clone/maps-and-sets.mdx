...
console.log(restored.set.has(restored.aliceRef));  // ✅ true
```
...
console.log(key1 === key2 && key1 === restored.theKey);  // ✅ true
```
...
The warning symbols (⚠️) in the [type support table](/docs/structured-clone/type-support#support-table) reflect these different boundaries:

| System | Boundary | Object Identity |
|--------|----------|----------------|
| `@lumenize/structured-clone` | Per `stringify()`/`parse()` | ✅ Preserved |
| `@lumenize/rpc` | Per RPC method call | ✅ Preserved |
| Workers RPC | Per RPC method call | ✅ Preserved |
| DO Storage | Per `put()`/`get()` operation | ❌ Lost |
