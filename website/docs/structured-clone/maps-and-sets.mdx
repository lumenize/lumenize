...
- ⚠️ **Object keys/values** are fully reconstructed but have **new identity**
- 🔍 To find object keys after deserialization, **search through the reconstructed keys**
...
console.log(restored.get("user123"));  // ✅ { name: "Alice" }
console.log(restored.get(42));         // ✅ { count: 100 }
console.log(restored.get(true));       // ✅ { active: true }
```
...
console.log(keys[0]);  // { userId: 123, role: "admin" } ✅

// All properties are preserved...
console.log(keys[0].userId);  // 123 ✅
console.log(keys[0].role);    // "admin" ✅

// BUT it's a new object with different identity...
const newKey = { userId: 123, role: "admin" };
console.log(restored.get(newKey));  // ❌ undefined - different reference!

// You must use the reconstructed key object:
console.log(restored.get(keys[0]));  // ✅ "user data"
```
...
if (targetKey) {
  ...
  console.log(value);  // ✅ { name: "Bob", email: "bob@example.com" }
}
```
...
// ✅ true - same reconstructed object!
```
...
console.log(restored.has("apple"));  // ✅ true
...
console.log(restored.has({ id: 1, name: "Alice" }));  // ❌ false
...
console.log(restoredData.set.has(restoredData.aliceRef));  // ✅ true
```
...
console.log(restored.map1.get(restored.theKey));  // ✅ "data1"
console.log(restored.map2.get(restored.theKey));  // ✅ "data2"

// Verify they're the same object
const key1 = Array.from(restored.map1.keys())[0];
const key2 = Array.from(restored.map2.keys())[0];
console.log(key1 === key2);           // ✅ true
console.log(key1 === restored.theKey); // ✅ true
```
...
console.log(restored.set1.has(restored.theObj));  // ✅ true
console.log(restored.set2.has(restored.theObj));  // ✅ true
...
### ✅ DO: Use Primitive Keys When Possible
...
### ✅ DO: Store Key References Separately
...
const data = {
  ...
  adminKey: keyObj  // ✅ Can use this to look up later
};
```

### ✅ DO: Search Keys by Properties
...
### ❌ DON'T: Expect New Objects to Match

```typescript
// Bad: This won't work
const restored = await parse(serialized);
const value = restored.get({ userId: 123 });  // ❌ undefined!
```

### ❌ DON'T: Use Complex Objects as Keys Without a Lookup Strategy

```typescript
// Bad: No way to find the right key later
const map = new Map();
map.set({ complexStructure: { nested: { deep: true } } }, "value");
// How will you find this key after deserialization? 🤔
```
