...
:::tip Core Utility
This utility is used internally by Lumenize and is also available for your own logging needs. It's part of `@lumenize/core` and automatically available via NADIS when using LumenizeDO.
:::
...
- **NADIS integration**: Use standalone or auto-inject into `LumenizeDO` via `this.svc.debug`
...
// Create logger
const log = this.svc.debug('MyDO.myMethod');  // LumenizeDO
...
- **NADIS integration**: Auto-injection in LumenizeDO
