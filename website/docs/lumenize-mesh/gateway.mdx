...
The Gateway is the **trust boundary** between untrusted clients and the trusted mesh. Clients cannot fake their identity because the Gateway controls what goes into `callContext`.
...
When forwarding a client's call to the mesh, the Gateway builds `callContext` from **verified sources only**:
...
{
  "type": "__call",
  "originAuth": { "userId": "admin", "claims": { "isAdmin": true } },
  "fakeIdentity": "someone-else",
  "chain": [...]
}
```

The Gateway ignores all identity fields and uses only:
1. The continuation (`chain`) — what to call
2. Verified identity from WebSocket attachment — who's calling
