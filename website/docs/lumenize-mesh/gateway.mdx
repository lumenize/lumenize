...
The Gateway and Client communicate over WebSocket using JSON messages. Each message has a `type` field that determines how it's processed.

:::note Serialization Format
The `chain` and `result` fields use [@lumenize/structured-clone](/docs/structured-clone)'s `stringify()` format, which preprocesses values before JSON encoding. This preserves rich types (Dates, Maps, Sets, circular references, custom classes) but is not human-readable. See [Structured Clone: Serialization](/docs/structured-clone#serialization) for details.
:::
...
|------|-----------|---------|
| `call` | Client→Gateway | Client initiating a mesh call |
| `call_response` | Gateway→Client | Response to a client-initiated call |
| `incoming_call` | Gateway→Client | Mesh node calling the client |
| `incoming_call_response` | Client→Gateway | Client's response to an incoming call |
| `connection_status` | Gateway→Client | Post-handshake status (sent immediately after connection) |
| `token-update` | Client→Gateway | Refresh access token without reconnecting |

### Message Formats

**`call`** — Client initiating a call to a mesh node:
```json @skip-check
{
  "type": "call",
  "callId": "uuid",
  "binding": "DOCUMENT_DO",
  "instance": "doc-123",
  "chain": { /* serialized operation chain */ }
}
```

**`call_response`** — Gateway returning the result:
```json @skip-check
{
  "type": "call_response",
  "callId": "uuid",
  "success": true,
  "result": { /* or error if success: false */ }
}
```

**`incoming_call`** — Mesh node calling the client:
```json @skip-check
{
  "type": "incoming_call",
  "callId": "uuid",
...
**`incoming_call_response`** — Client responding to incoming call:
```json @skip-check
{
...
**`connection_status`** — Sent by Gateway immediately after WebSocket handshake:
```json @skip-check
{
...
When `subscriptionsLost` is `true`, the client's `onSubscriptionsLost` callback fires after the connection is established, prompting resubscription to any active data streams.

**`token-update`** — Client sending a refreshed access token:
```json @skip-check
{
  "type": "token-update",
...
Gateway validates the token and updates the WebSocket attachment. If validation fails, Gateway closes the connection with `4403` (invalid token).
