...
Errors are serialized/deserialized across the wire using `@lumenize/structured-clone`. The `message`, `name`, and `stack` properties are preserved. Custom error subclasses become plain `Error` objects but retain their message.
:::
