...
See [Security](./security#access-control) for the complete security model including reusable guards and capability-based trust.
...
| `@lumenize/mesh` | `debug` | Scoped, filterable logging |
...
- `state` — Mutable object for passing computed data through the call chain. Starts as `{}` for each incoming call; typically populated in `onBeforeCall()` (e.g., to attach user records or permissions) but could be added by anything. Isolated per-request via AsyncLocalStorage.
