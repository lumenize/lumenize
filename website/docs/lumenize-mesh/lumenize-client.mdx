...
| `gatewayBinding` | `string?` | Gateway DO binding name. Default: `LUMENIZE_CLIENT_GATEWAY` |
...
| `onConnectionError` | `(error) => void` | Called when connection-level error occurs |
...
@mesh((callContext, instance: EditorClient) => {
  if (callContext.caller.instanceNameOrId !== instance.documentId) {
...
See [Auth Integration](./auth-integration#access-control) for the complete security model including reusable guards.
...
- **[Auth Integration](./auth-integration)** — Token handling and security
