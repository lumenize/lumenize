...
DO and Worker instances don't know their own binding names. DO instances don't know their instance name (i.e. `getByName('my-instance')`) while they do know their id. This is problematic, because it makes it impossible for a caller to say, "Hey, do this thing for me and when you are done, call me back at this address." However, while the callee doesn't know its own identity, anything calling it, by definition, must know its identity. So we leverage that necessity to automatically propogate identity throughout the mesh. Every call into the mesh as well as every call within the mesh includes the callee identity information and every node stores that for use in making its own calls.
...
LumenizeClient integrates with `@lumenize/auth`. Tokens are passed via WebSocket subprotocol — no URL exposure, no header limitations. See [Auth Integration](./auth-integration) for token refresh patterns and call context propagation.
...
Lumenize provides **two levels** of access control:

| Level | Mechanism | Best For |
|-------|-----------|----------|
| **Class-wide** | `onBeforeExecuteCall()` hook | Universal policies (auth required, reject peer calls) |
| **Method-specific** | `this.lmz.callContext` in method body | Fine-grained permission checks |

```typescript
class DocumentDO extends LumenizeDO {
  // Class-wide: all callers must be authenticated
  ...
  // Method-specific: check document permissions
...
this.lmz.call(
  ...
  documentId,                                       // Instance name (undefined for Workers)
...
**Passing context to handlers** — include additional data alongside `$result`:
...
this.lmz.call(
  ...
  this.ctn().handleContent(this.ctn().$result, documentId, userId)
);

// Handler receives result + context
handleContent(content: string | Error, documentId: string, userId: string) {
  // ...
}
```
...
- **Call proxying**: Calls pass through without the gateway to another mesh destination
...
- **Presence tracking**: After grace period, responds to callers with `ClientDisconnectedError` so they can clean up subscriptions
...
**Peer-to-peer calls are rejected by default.** `LumenizeClient` implements `onBeforeExecuteCall` to reject calls from other clients. This minimizes attack surface for applications that don't need client-to-client communication.

To enable peer calls for collaborative features, override `onBeforeExecuteCall` in your client class. See [Client API: Access Control](./client-api#access-control) for details.
:::
