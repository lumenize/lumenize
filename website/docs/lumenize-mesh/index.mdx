...
We propogate that authentication information, using the same metadata sub-object in the call envelope that we use to auto-propogate node identity, with every call from a LumenizeClient. It's up to the LumenizeDO instances to include the appropriate authentication information on sub-calls but we provide those mechanisms.

#### Access Control

However, authentication is only part of the story. It's useless unless it's used to control access. We provide easy-to-use mechanisms and examples on how to do this. For instance, for every subscription, the client provides a subscription-specific access token. Only updates that include that token will be accepted.

[TODO: Example of using a subscription-specific token]
[TODO: Example showing LumenizeOrg style access control using the automatic authentication information metadata in calls (MAYBE NOT UNTIL WE BUILD LUMENIZE_BAAS)]
