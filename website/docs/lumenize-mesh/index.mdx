...
**Hibernation-safe** (for DOs): Call context is automatically captured in continuations. For long running remote calls, your handler can be configured to store the continuation so even if a DO hibernates during, your handler receives the correct context no matter how much later it executes.
