...
// Stringify it (supports Dates, Maps, Sets, etc.)
const serialized = stringify(chain);  // Necessary for SQL. Optional for KV

// Save to storage - THIS is what makes it hibernation-safe
this.ctx.storage.kv.put('my-key', serialized);
```

The key insight: `getOperationChain()` captures `callContext` at the moment you call it. But that capture only survives hibernation if YOU store it somewhere. The framework doesn't do that automatically.
