...
If refresh fails (e.g., refresh token expired), `onTokenRefreshFailure` fires:
...
using client = new EditorClient({
  ...
  onTokenRefreshFailure: (error) => {
    ...
    // In-flight WebSocket calls can still complete
    // Redirect to login when ready
...
`onTokenRefreshFailure` is distinct from `onConnectionError`. Token refresh happens via HTTP in parallel to WebSocket messages. When refresh fails, the WebSocket stays open for in-flight calls to complete naturally. The connection will eventually close when the Gateway rejects the expired token (4401 close code).
:::
...
using client = new EditorClient({
  ...
  onConnectionError: (error) => {
    // WebSocket connection failed (network issues, etc.)
    showOfflineBanner();
  },
  onTokenRefreshFailure: (error) => {
    // HTTP token refresh failed — session expired
    window.location.href = '/login';
  }
});
```
