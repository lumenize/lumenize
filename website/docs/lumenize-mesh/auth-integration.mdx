...
Lumenize provides **two complementary levels** of access control. Use them together for defense-in-depth.

### Two Levels of Access Control

| Level | Mechanism | Best For |
|-------|-----------|----------|
| **Class-wide** | `onBeforeExecuteCall()` hook | Universal policies (auth required, rate limiting, audit logging) |
| **Method-specific** | `this.lmz.callContext` in method body | Fine-grained permission checks |

### When to Use Which

**Use class-wide (`onBeforeExecuteCall`) for:**
- Ensuring all calls are authenticated
- Rejecting calls from untrusted sources (e.g., peer clients)
- Audit logging every incoming call
- Rate limiting per-origin
- Fail-closed security (reject by default, explicitly allow)

**Use method-specific for:**
- Permission checks unique to that method (e.g., "canEditDocument")
- Role-based access for specific operations
- Business logic that varies by action

**Example: Both levels working together:**

```typescript
class DocumentDO extends LumenizeDO {
  // Class-wide: ensure all callers are authenticated
  onBeforeExecuteCall(callContext: CallContext) {
    ...
    // Log for audit trail
    console.log(`[audit] ${callContext.originAuth.userId} calling via ${callContext.caller.bindingName}`);
  }
  ...
  update(changes: DocumentChange) {
    ...
    if (!this.canEdit(ctx.originAuth.userId)) {
  ...
  // Read-only method - different permission check
  getContent(): string {
    const ctx = this.lmz.callContext;
    
    if (!this.canView(ctx.originAuth.userId)) {
...
### The `onBeforeExecuteCall` Hook

Every mesh node (LumenizeDO, LumenizeWorker, LumenizeClient) can implement this lifecycle hook:

```typescript
/**
 * Called before any incoming RPC method is executed.
 * Throw to reject the call; return normally to allow.
 * 
 * @param callContext - Full context of the incoming call
 */
onBeforeExecuteCall(callContext: CallContext): void | Promise<void>;
```

**Behavior:**
- Called before the continuation is executed
- If it throws, the call is rejected (error returned to caller)
- If it returns normally (or resolves), the call proceeds
- Has access to full `callContext` (origin, caller, originAuth, callChain)

**Example: Role-based access for entire DO:**

```typescript
class AdminDO extends LumenizeDO {
  onBeforeExecuteCall(callContext: CallContext) {
