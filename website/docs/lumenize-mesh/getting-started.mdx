...
export class DocumentDO extends LumenizeDO<Env> {
  ...
  onBeforeCall() {
    super.onBeforeCall();  // Always call super for forward compatibility
...
export class EditorClient extends LumenizeClient {
  ...
  // Response handler for #subscribe() — no @mesh needed (framework trusts your own continuations)
...
function initEditor(documentId: string) {
  ...
  using client = new EditorClient(
    {
      instanceName: `${getUserId()}.${getTabId()}`,
...
export { LumenizeAuth, LumenizeClientGateway, DocumentDO, SpellCheckWorker };

// Allowlist your app's origins
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const authRoutes = createAuthRoutes(env, {
      redirect: '/app',  // Where to redirect after magic link
      cors
    });
