...
1. **Chain Validation** (automatic) — Only single method calls allowed.
2. **Class-level `onBeforeCall`** — Override for class-wide policies.
3. **Method Exposure** (required) — Only methods decorated with `@mesh` are callable.
4. **Method-level `@mesh(handler)`** — Decorator parameter for per-method policies.
