...
**Why extend a class?** In the Lumenize Mesh, clients are full peers that can receive incoming calls. Mesh nodes call methods on your client via `this.lmz.call('LUMENIZE_CLIENT_GATEWAY', clientId, ctn<YourClient>().yourMethod(...))`. You must define those methods on your class.
...
interface LmzClientApi {
  ...
  /**
   * Gateway binding name (e.g., 'LUMENIZE_CLIENT_GATEWAY').
   */
  ...
  /**
   * Alias for instanceName (for API consistency with LumenizeDO).
   */
  readonly instanceNameOrId?: string;
  
  /**
   * Context from the current incoming call (if any).
   * Contains full call chain, callee identity, and propagated auth.
   * Only available during method execution from an incoming mesh call.
   * 
   * Convenience getters: `origin`, `caller`, `priorCaller` (from callChain)
   * Full chain: `callChain` for debugging/auditing/circuit detection
   * 
   * See [Auth Integration](./auth-integration#call-context) for details.
   */
...
LumenizeClient supports the same two-level access control as DOs and Workers. See [Auth Integration: Access Control](./auth-integration#access-control) for full details.

### Two Levels

| Level | Mechanism | Best For |
|-------|-----------|----------|
| **Class-wide** | `onBeforeExecuteCall()` hook | Universal policies (reject peer calls, audit logging) |
| **Method-specific** | `this.lmz.callContext` in method body | Fine-grained permission checks |
...
class LumenizeClient {
  ...
  onBeforeExecuteCall(callContext: CallContext) {
...
For collaborative features (e.g., direct peer-to-peer cursors, voice/video signaling), override `onBeforeExecuteCall`:
...
class CollaborativeClient extends LumenizeClient {
  ...
  onBeforeExecuteCall(callContext: CallContext) {
    if (callContext.origin.type === 'LumenizeClient') {
      ...
      // Optionally: check if peer is in allowed list
      // if (!this.allowedPeers.has(callContext.originAuth.userId)) {
      //   throw new Error('Peer not in collaborator list');
      // }
    }
  }
  
  // Now other clients can call this method
...
For fine-grained checks within individual methods:
...
class EditorClient extends LumenizeClient {
  handleDocumentChange(change: DocumentChange) {
    ...
    // Only accept changes from the document we're subscribed to
    if (ctx?.caller.instanceNameOrId !== this.documentId) {
      console.warn('Ignoring change from unexpected source');
      return;
    }
...
It's not necessary to disconnect when tabs are hidden or hibernated. The first time an call from upstream is made to this tab, the caller will be notified that the client is disconnected. For the planned subscription functionality, the subscription will be dropped and no further attempts to update it from upstream will be attempted.

There are two kinds of disconnected states. A short disconnect due to network failure/transition, and one where you've been disconnected long enough for subscriptions to have been dropped. For the latter case, the client will receive the `onSubscriptionsLost` callback (see [Subscription Loss](#subscription-loss) above). For a UI with multiple state subscriptions, we recommend that you delegate resubscribing to the active UI elements by having them listen to this event.
