...
When using `LumenizeDO`, `sql` is automatically available via NADIS:
...
See [LumenizeDO documentation](./lumenize-do) for complete NADIS examples.

### Standalone Usage (Without LumenizeDO)

For vanilla Durable Objects without the Lumenize Mesh, import `sql` directly and create a private field:
...
:::note Examples Use Standalone Style
The validated examples below use `this.#sql` (standalone pattern) for test compatibility. **LumenizeDO users**: substitute `this.svc.sql` everywhere you see `this.#sql`.
:::
...
:::tip Direct Storage Access
`this.svc.sql` (and the standalone `sql()`) returns results as an array, which loads all rows into memory. For pagination, large result sets, or streaming, use the underlying API directly:
...
for (const row of cursor) {
  // Process rows without loading all into memory
}
```
:::
