...
Access token expiration (4401) is handled automatically — the client refreshes via HTTP and reconnects transparently. `onAuthenticationError` only fires when the refresh token itself has expired or been revoked (HTTP 401 from the refresh endpoint), meaning the user must re-login:
...
The Gateway uses these WebSocket close codes for auth issues:

| Code | Meaning | Action |
|------|---------|--------|
| `4400` | No identity provided (middleware didn't set `X-Auth-User-Id`) | Redirect to login |
...
The presence of `X-Auth-User-Id` indicates successful authentication. If authentication fails, the request should never reach the LumenizeGateway, but if for some reason it does, a missing `X-Auth-User-Id` will cause it to be rejected there.
...
import type { HookContext } from '@lumenize/mesh';

async function createAuth0WebSocketMiddleware(env: Env) {
  const publicKey = await importSPKI(env.AUTH0_PUBLIC_KEY, 'RS256');

  return async (request: Request, context: HookContext): Promise<Request | Response> => {
