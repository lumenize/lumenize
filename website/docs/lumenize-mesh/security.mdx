...
Guards that need instance state receive the instance as a second parameter. This works for all node types (`LumenizeDO`, `LumenizeWorker`, `LumenizeClient`):

```typescript @skip-check
@mesh((callContext, instance: DocumentDO) => {
  if (!instance.allowedEditors.has(callContext.originAuth?.userId)) {
