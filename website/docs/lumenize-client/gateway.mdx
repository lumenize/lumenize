...
Each LumenizeClient connects to **its own** Gateway instance:

```
Client A (user1.tab1) → Gateway DO "user1.tab1"
Client B (user1.tab2) → Gateway DO "user1.tab2"
Client C (user2.tab1) → Gateway DO "user2.tab1"
```
...
```typescript
// From a DO trying to reach a disconnected client:
try {
  await this.lmz.callRaw('GATEWAY_DO', clientId, chain);
} catch (error) {
  if (error.message === 'Client not connected') {
    // Queue for later, send push notification, etc.
  }
}
```
