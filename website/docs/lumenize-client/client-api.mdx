...
interface LumenizeClientConfig {
   ...
   * Return a new token to refresh, or throw to disconnect.
   * Called ~30 seconds before expiration.
   */
...
Update the access token without reconnecting:

```typescript
updateAccessToken(token: string): void;
```

The new token will be used for subsequent messages. For token refresh, consider using `onTokenExpiring` callback instead.
