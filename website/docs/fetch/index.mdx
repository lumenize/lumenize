...
A de✨light✨ful way to make external API calls from Durable Objects without blocking or accumulating wall-clock billing.
...
- ✅ **Type-safe** - Full TypeScript support with continuations
- ✅ **Timeout monitoring** - Automatic timeout detection via @lumenize/alarms
- ✅ **Simple architecture** - Two-hop design (Origin DO → Worker → External API)

## Quick Start

```typescript @skip-check
import '@lumenize/fetch';  // Registers as NADIS service
...
class MyDO extends LumenizeDO {
  ...
  fetchUserData(userId: string) {
    ...
    const reqId = this.svc.fetch.proxy(
...
```bash @skip-check
npm install @lumenize/fetch @lumenize/alarms
```
...
- `continuation`: Continuation created with `this.ctn().handlerMethod()`
...
```typescript @skip-check
result.json()        // → any (parsed JSON, no await)
result.text()        // → string (no await)
result.arrayBuffer() // → ArrayBuffer (no await)
result.ok            // → boolean (2xx status)
...
Since `proxyFetch` is fire-and-forget (synchronous, non-blocking), launching multiple fetches in parallel is trivial—just call it multiple times:

```typescript @skip-check
class MyDO extends LumenizeBase {
  fetchMultipleUsers(userIds: string[]) {
    // Fire all fetches in parallel - each returns immediately
    userIds.forEach((userId, index) => {
      this.svc.proxyFetch(
        `https://api.example.com/users/${userId}`,
        this.ctn().handleUserResult({ userId, index })
      );
    });
    // All fetches dispatched, DO billing stops here
  }
  
  handleUserResult(context: { userId: string; index: number }, result: ResponseSync | Error) {
    if (result instanceof Error) {
      console.error(`User ${context.userId} fetch failed:`, result);
      return;
    }
    const user = result.json();
    console.log(`User ${context.userId} (index ${context.index}):`, user.name);
  }
}
```

**Key benefit:** Unlike `Promise.all()` which keeps your DO billing while waiting, each `proxyFetch` returns immediately. Your DO stops billing after dispatching all requests, regardless of how long the external APIs take.

#### Ordering Results

Responses arrive in completion order, not dispatch order. If you need original ordering, use the context to track sequence and store results to reassemble later:

```typescript @skip-check
class MyDO extends LumenizeBase {
  fetchInOrder(urls: string[]) {
    // Clear any previous pending results
    this.ctx.storage.kv.delete('pendingResults');
    
    urls.forEach((url, index) => {
      this.svc.proxyFetch(
        url,
        this.ctn().collectResult({ index, total: urls.length })
      );
    });
  }
  
  collectResult(context: { index: number; total: number }, result: ResponseSync | Error) {
    // Get or create the pending results map from storage
    const pending: Map<number, any> = 
      this.ctx.storage.kv.get('pendingResults') ?? new Map();
    
    // Store result at its original position
    pending.set(context.index, result);
    this.ctx.storage.kv.set('pendingResults', pending);
    
    // Check if all results are in
    if (pending.size === context.total) {
      // Reassemble in order
      const ordered = Array.from({ length: context.total }, (_, i) => 
        pending.get(i)
      );
      this.ctx.storage.kv.delete('pendingResults');
      this.processAllResults(ordered);
    }
  }
  
  processAllResults(results: (ResponseSync | Error)[]) {
    console.log('All results in order:', results.length);
    // Process ordered results...
  }
}
```

This pattern works even when slower endpoints respond before faster ones—the context preserves the original sequence, and storage ensures results survive DO eviction.

## Retry Logic

The **Origin DO decides retry strategy**. The alarm system only monitors timeouts - it never retries.

**Race condition safety**: Handlers with early returns on errors are naturally safe from duplicate calls. Only add explicit deduplication if you perform destructive actions on both success AND error. See [Race Conditions](/docs/fetch/architecture-and-failure-modes#race-conditions) for details.
  ...
  handlePaymentResult(paymentId: string, result: ResponseSync | Error) {
    if (result instanceof Error) {
      if (result.message.includes('delivery timeout')) {
        // ⚠️ CRITICAL: Timeout is ambiguous!
...
| **Alarm** | **`Error` (timeout)** | ⚠️ **Ambiguous - may have succeeded!** |
...
👉 **[Architecture & Failure Modes](/docs/fetch/architecture-and-failure-modes)**

**High-level flow:**
1. Origin DO → Schedule alarm with embedded continuation
2. Origin DO → Executor: Dispatch fetch via RPC (returns immediately)
3. Executor → External API: Perform fetch (CPU billing)
4. Executor → Origin DO: Deliver result via `__handleProxyFetchResult()`
5. Origin DO: Cancel alarm atomically, execute user handler synchronously

**Key design:**
- ✅ Origin DO stops billing after quick RPC dispatch
- ✅ Executor uses CPU billing for external fetch
- ✅ Timeout monitoring via @lumenize/alarms (native DO alarm system)
- ✅ Direct delivery via continuation chains (no result storage)
- ✅ Atomic alarm cancellation on result delivery
...
handleResult(result: ResponseSync | Error) {
  ...
  const data = result.json();
  this.saveToDatabase(data); // ✅ Only happens once
}
```
...
See [Race Conditions](/docs/fetch/architecture-and-failure-modes#race-conditions) for detailed examples.
