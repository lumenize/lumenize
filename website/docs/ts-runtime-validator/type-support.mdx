...
```typescript @check-example('packages/ts-runtime-validator/test/for-docs/type-support.test.ts')
...
```typescript @check-example('packages/ts-runtime-validator/test/for-docs/type-support-types.ts')
...
Functions throw `TypeError` — they can't be represented in a TypeScript program:
