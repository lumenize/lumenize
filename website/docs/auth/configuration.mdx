...
| `RESEND_API_KEY` | Secret | No³ | `ResendEmailSender` | [Resend](https://resend.com) API key. Only needed if you chose Resend over the default `CloudflareEmailSender`. See [Using Resend instead](/docs/auth/using-resend-instead) |

¹ At least one key pair (BLUE or GREEN) is required. ² Second slot needed only during [key rotation](/docs/auth/getting-started#key-rotation). ³ The default `CloudflareEmailSender` uses a Worker binding and needs no secret.
...
| `EMAIL` | [Email Sending binding](https://developers.cloudflare.com/email-service/api/send-emails/workers-api/) (`send_email`) | With `CloudflareEmailSender` | `CloudflareEmailSender` | Cloudflare Email Sending binding used to deliver mail. Not needed for `ResendEmailSender` or bring-your-own-provider. |
...
WorkerEntrypoint                     (Cloudflare)
  └─ AuthEmailSenderBase             (@lumenize/auth — templates, subjects, from/replyTo/appName)
       ├─ CloudflareEmailSender      (@lumenize/auth — sendEmail() via env.EMAIL.send — DEFAULT)
       │    └─ AuthEmailSender       (you — sets from, optional overrides)
       └─ ResendEmailSender          (@lumenize/auth — sendEmail() via Resend API — alternative)
            └─ AuthEmailSender       (you — only if using Resend)
```
...
| `CloudflareEmailSender` | `@lumenize/auth` | Extends `AuthEmailSenderBase`. Implements `sendEmail()` via `env.EMAIL.send(...)` (the `send_email` Worker binding). The default — no API key needed. Requires the Workers Paid plan. |
| `ResendEmailSender` | `@lumenize/auth` | Extends `AuthEmailSenderBase`. Implements `sendEmail()` via `fetch` to `https://api.resend.com/emails` using `RESEND_API_KEY`. Alternative — see [Using Resend instead](/docs/auth/using-resend-instead). |
