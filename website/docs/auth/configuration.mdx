...
| Binding | Type | Read By | Description |
|---------|------|---------|-------------|
| `LUMENIZE_AUTH` | Durable Object namespace | `createAuthRoutes` | Namespace binding for the [LumenizeAuth](/docs/auth/) DO |
| `LUMENIZE_AUTH_RATE_LIMITER` | [Rate Limiting binding](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/) | `createRouteDORequestAuthHooks` | Per-subject rate limiting keyed on `sub` from the decoded JWT |
