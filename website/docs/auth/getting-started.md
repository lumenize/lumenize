...
Use `createAuthRoutes` to expose auth endpoints and `createRouteDORequestAuthHooks` to protect your DOs over HTTP (`onBeforeRequest`) or WebSockets (`onBeforeConnect`). Both integrate with [`routeDORequest`](/docs/routing/route-do-request). All auth configuration — including the URL prefix (default `/auth`), redirect URL, token TTLs, and issuer/audience — is read from [environment variables](/docs/auth/configuration#what-mustmight-be-in-env):
...
The only option is `cors` — see [CORS Support](/docs/routing/cors-support) for `CorsOptions`.

### createRouteDORequestAuthHooks

Creates hooks for [`routeDORequest`](/docs/routing/route-do-request) that verify JWTs and enforce [two-phase access control](/docs/auth/#access-flows). Reads `issuer`, `audience`, public keys, and rate limiter binding from [environment variables](/docs/auth/configuration#what-mustmight-be-in-env).
...
:::tip Using Hono?
The example above uses vanilla Workers. If you're building on [Hono](https://hono.dev), we also provide direct [Hono support](/docs/auth/hono).
:::
