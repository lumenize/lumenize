...
Delegation allows one authenticated subject to act on behalf of another (e.g., an admin or support engineer acting for a user). The principal's permissions apply; the actor's identity is recorded for audit. See [JWT Claims: Delegation](/docs/auth/#jwt-claims) for the `act` claim format.
