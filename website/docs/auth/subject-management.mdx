...
Creates subject records with `adminApproved: true`, `emailVerified: false` and sends invite emails. When subjects click the invite link ([Accept Invite](/docs/auth/endpoints#accept-invite)), `emailVerified` is set to `true` and they gain immediate access.
