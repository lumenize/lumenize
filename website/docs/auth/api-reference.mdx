...
// Throws if env.TURNSTILE_SECRET_KEY is not set (skipped in test mode)
```
...
const { onBeforeRequest, onBeforeConnect } = createRouteDORequestAuthHooks(env, {
  rateLimiterBindingName?: string,   // Default: 'LUMENIZE_AUTH_RATE_LIMITER'
});
...
// Throws if rate limiter binding not available (env[rateLimiterBindingName])
...
In [test mode](/docs/auth/#test-mode), Turnstile validation is skipped entirely. Append `?_test=true` to get the magic link directly without email delivery:
