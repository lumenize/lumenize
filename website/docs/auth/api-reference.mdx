...
| [`{prefix}/refresh-token`](#refresh-token) | POST | Auth | Exchange refresh token for access token |
| [`{prefix}/logout`](#logout) | POST | Auth | Clear refresh token cookie |
...
| `LUMENIZE_AUTH_PREFIX` | Variable | No | `'/auth'` | Auth DO, Worker | URL prefix for all auth endpoints. Used to compose links in emails |
...
// Throws if env.LUMENIZE_AUTH_REDIRECT is not set
```
...
// Public keys read from env.JWT_PUBLIC_KEY_BLUE / env.JWT_PUBLIC_KEY_GREEN
```
...
The same operations are available as methods on the Auth DO, for server-to-server calls or mesh-internal use. Note the naming convention: HTTP endpoints use kebab-case (`/delegated-token`) while RPC methods use camelCase (`delegatedToken`).
