...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const authRoutes = createAuthRoutes(env, {
      ...
      // All config options are passed to Auth DO:
      // issuer, audience, accessTokenTtl, refreshTokenTtl, magicLinkTtl, rateLimitPerHour
    });
...
- `configure(options)` - Set auth configuration (called automatically by `createAuthRoutes`)
...
When using `createAuthRoutes`, configuration is handled automatically — the options you pass are forwarded to the Auth DO on first request. If you aren't using `createAuthRoutes`, you can configure the Auth DO directly:
