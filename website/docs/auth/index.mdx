...
const { accessToken, claims, sub } = await testLoginWithMagicLink(browser, 'alice@test.com', {
...
// Login with delegation — alice is the principal, actorSubId acts on her behalf
const { accessToken: delegatedToken } = await testLoginWithMagicLink(browser, 'alice@test.com', {
  subjectData: { adminApproved: true },
  actorSub: actorSubId  // this subject is acting on behalf of alice
});
```
...
// Auth config (redirect, issuer, audience, TTLs) comes from environment variables
