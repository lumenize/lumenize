...
description: Using handleRpcRequest and handleRpcMessage for custom routing
testable: true
---

# Manual Instrumentation

When you need custom routing alongside RPC (health checks, webhooks, custom WebSocket messages), use manual instrumentation with `handleRpcRequest()` and `handleRpcMessage()`.
...
import { handleRpcRequest, handleRpcMessage } from '@lumenize/rpc';
...
export class MyDO extends DurableObject {
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    await handleRpcMessage(ws, message, this);
  }
...
1. **Check tags first**: In `webSocketMessage()`, handle custom protocols before calling `handleRpcMessage()`
