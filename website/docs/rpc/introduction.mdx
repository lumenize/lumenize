...
:::warning Deprecated
`@lumenize/rpc` is **deprecated for production use**. Use [@lumenize/mesh](/docs/mesh) instead, which provides secure mesh networking with built-in authentication, access control, and browser clients as full mesh peers.

`@lumenize/rpc` remains available as the foundation for [@lumenize/testing](/docs/testing/usage) — you don't need to change anything if you're using it for testing.
:::
