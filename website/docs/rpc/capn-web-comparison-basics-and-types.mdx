...
**Lumenize RPC**: ✅ Web API types work via `RequestSync`/`ResponseSync` (synchronous body access)  
...
it('demonstrates Web API RequestSync support', async () => {
  ...
  expect(lumenizeResult).toBeInstanceOf(RequestSync); // ✅
  expect(lumenizeResult.url).toBe('https://example.com/test'); // ✅
  expect(lumenizeResult.method).toBe('POST'); // ✅
  expect(lumenizeResult.json()).toEqual({ data: 'test payload' }); // ✅ No await needed!
