...
- Cap'n Web's server side is designed to run anywhere JavaScript is run, while Lumenize
  RPC server-side only runs in Cloudflare (although we will release a SharedWorker 
  "server" when we relase LumenizeBase)
...
- Lumenize RPC requires no resources be maintained on the server-side. Even
  the hibernating WebSocket support is lazily re-connected the first time it's needed
