...
sequenceDiagram
    ...
    Note over User,Handler: Your DO wall clock billing continues ⚡
...
**Imortant:** `PROXY_FETCH_DO` binding name is required (this is what `proxyFetch()` 
looks for)
...
```typescript title="src/my-do.ts" @skip-check
