...
`@lumenize/proxy-fetch` offloads HTTP fetches to Workers (CPU billing) or a shared DO via 
Cloudflare Queues, letting your DOs hibernate while waiting for responses.
...
over 30 days => 600,000 seconds => 170 hours/month wall clock time per instance
1,000 instances => 170,000 hours/month => 230 months wall clock time per month
@ $4/month
======================================================================
$720/month waiting on external APIs
```
...
Similarly, running one DO to serve the fetch needs of 1,000 DOs is only $4/month!

By satisifying external fetches somewhere else (Worker or shared DO), you:
- ✅ DO wall clock billing stops as soon as fetch is queued
     DO can hibernate for requests > 10 seconds
...
sequenceDiagram
    ...
    Note over User,ProxyFetchHandler: DO wall clock billing stops (can hibernate in 10 seconds) ⚡
    ...
    Note over User,ProxyFetchHandler: DO wall clock billing restarts (wakes if hibernated)
