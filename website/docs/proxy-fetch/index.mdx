...
- ✅ **Type-safe** - Full TypeScript support with OCAN continuations
- ✅ **Timeout monitoring** - Automatic timeout detection with clear error messages
- ✅ **Simple architecture** - No result storage in Origin DO, direct continuation delivery
...
**Parameters:**
- `request`: `string | Request` - URL or Request object
- `continuation`: OCAN continuation created with `this.ctn().handlerMethod()`
- `options?`: Optional configuration

**Returns:** `Promise<string>` - Request ID (for logging/debugging)
...
The **Origin DO decides retry strategy**. The Orchestrator only monitors timeouts - it never retries.
...
- **User timeout** (`options.timeout`): Controls fetch execution (AbortController)
- **Orchestrator timeout**: Automatically adds 10s buffer (5s polling + 5s network)
- **Timeout error message**: "Fetch timeout - external fetch may have partially succeeded"
...
| **Orchestrator** | **`Error` (timeout)** | ⚠️ **Ambiguous - may have succeeded!** |
| Executor | `ResponseSync` (ok) | HTTP 2xx success |
| Executor | `ResponseSync` (!ok) | HTTP 4xx/5xx error |

**Key distinction:** Only Orchestrator timeout is ambiguous. All Executor errors mean the fetch definitely failed/completed.

## Architecture

For detailed architecture diagrams, failure scenarios, and race condition handling, see:

👉 **[Architecture & Failure Modes](/docs/proxy-fetch/architecture-and-failure-modes)**

**High-level flow:**
1. Origin DO → Orchestrator: Enqueue fetch with continuation
2. Orchestrator → Executor: Dispatch via RPC (returns immediately)
3. Executor → External API: Perform fetch (CPU billing)
4. Executor → Origin DO: Deliver result via continuation chain
5. Origin DO: Handler receives result synchronously

**Key design:**
- ✅ Orchestrator stops billing after quick RPC dispatch
- ✅ Executor uses CPU billing for external fetch
- ✅ Timeout monitoring via native alarm (5s polling)
...
**Rare race condition** (network partition): Handler may be called twice - first with `ResponseSync` (success), then with `TimeoutError`. Most handlers are naturally safe:
