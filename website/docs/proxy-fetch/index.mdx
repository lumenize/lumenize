...
- ✅ **Idempotent** - Race conditions handled automatically
...
**High-level flow:**
1. Origin DO → Orchestrator: Enqueue fetch + store continuation
2. Orchestrator → Executor: Dispatch via RPC (returns immediately)
3. Executor → External API: Perform fetch (CPU billing)
4. Executor → Origin DO: Deliver result via `__receiveResult()`
5. Origin DO: Execute continuation with result
...
- ✅ Idempotent result handling (first wins, duplicates logged)
