...
This implementation provides **server-side rejection** to prevent unwanted requests from reaching your Durable Objects when using a whitelist or custom validation function:
...
- Returns `403` for disallowed origins BEFORE upgrading when using a whitelist or custom 
  validation function
- Only upgrades when origin allowed when using a whitelist or custom validation function
...
| **whitelist** | ✅ If allowed | ✅ Only allowed | ✅ Protected |
| **validation fn** | ✅ If allowed | ✅ Only allowed | ✅ Protected |

:::warning
With `false` or `true` modes, **all requests reach your Durable Object**, including from disallowed origins. Use whitelist or validation function for protection.
:::
