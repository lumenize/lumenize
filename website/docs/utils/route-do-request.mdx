...
The most common cross-origin resource sharing (CORS) configuration and our recommendation 90% of the time is a simple whitelist. This is not supported by `agents`' 
`routeAgentRequest`.
...
CORS support goes beyond `true`, `false`, and a whitelist. For a detailed discussion 
...
- **Robust CORS support** - Whitelist and custom function in addition to true/false
