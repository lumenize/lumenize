...
The following examples behave as you would expect and match the behavior of `routeAgentRequest` using the same configuration.

```typescript
await routeDORequest(request, env, { cors: true }); // Allow all origins (permissive)
await routeDORequest(request, env ); // Defaults to false, allowing on this origin
```

However, `routeDORequest` CORS support goes way beyond what is supported by `routeAgentRequest`. It fully documented in [CORS Support](/docs/utils/cors-support) but the most common configuration is a simple whitelist.
