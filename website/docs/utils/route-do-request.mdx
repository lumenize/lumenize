...
```typescript @check-example('packages/utils/test/integration/test-worker-and-dos.ts')
// Try routing to DOs with /do prefix
const doResponse = await routeDORequest(request, env, { prefix: '/do' });
