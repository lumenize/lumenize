...
## Recursive Validation (Lumenize Mesh)

Under the [Lumenize Mesh security model](/docs/lumenize-mesh/auth-integration#access-control), every step in a continuation chain is validated independently when sent over the wire. This enables secure "Sub-Target" patterns:

```typescript @skip-check
class DocumentDO extends LumenizeDO {
  @mesh
  getAdminPanel() {
    // Returns a sub-object with its own security
    return new AdminPanel(this);
  }
}

class AdminPanel extends LumenizeSubTarget {
...
// Chaining across targets:
// 1. getAdminPanel() is checked against DocumentDO (@mesh check)
// 2. Resulting AdminPanel is checked (onBeforeCall + @mesh check)
this.ctn<DocumentDO>().getAdminPanel().deleteHistory()
```
