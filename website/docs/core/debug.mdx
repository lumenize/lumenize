...
A de✨light✨ful scoped debug logging system inspired by npm's `debug` package with level support and JSON output that works perfectly with Cloudflare's observability dashboard.
...
Use `.dev.vars` file in your project root:
...
Set the DEBUG env var in `wrangler.jsonc` or via command line:
...
Or deploy with the `--var` flag:

```bash @skip-check
npx wrangler deploy --var DEBUG:"my-app.*,lmz.rpc"
```

**Important**: Changing DEBUG requires redeployment. When you deploy a new Worker version:
- All Durable Object instances are automatically evicted
- Constructors re-run with the new DEBUG value
- No manual intervention needed per DO

This ensures all instances immediately pick up your new debug configuration.
