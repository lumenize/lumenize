...
```json @skip-check
{
  ...
  "level": "info",
  "namespace": "lmz.proxy-fetch.proxyFetchQueueConsumer",
...
- Query by field (e.g., `namespace:"lmz.proxy-fetch.proxyFetchQueueConsumer"`)
- Filter by level
...
- Package names: kebab-case (npm convention) - `rpc`, `proxy-fetch`, `utils`
- Identifiers: actual case from code - `RpcClient`, `routeDORequest`, `ProxyFetchDO`

Examples:
- `lmz.rpc.RpcClient`, `lmz.rpc.WebSocketRpcTransport`
- `lmz.proxy-fetch.ProxyFetchDO`, `lmz.proxy-fetch.proxyFetchQueueConsumer`
