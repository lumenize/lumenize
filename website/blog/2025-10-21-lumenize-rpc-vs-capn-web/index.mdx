...
{/* 
TODO: Add type support comparison table showing:
- Lumenize RPC: ✅ for Date, RegExp, Map, Set, ArrayBuffer, TypedArray, BigInt, Error (with name/stack), Request/Response/Headers/URL, Infinity/-Infinity/NaN
- Cap'n Web: ❌ for most of these
- Note: ReadableStream/WritableStream don't work in either system (not serializable via structured-clone)
  - This affects Request/Response objects with streaming bodies
  - Both systems must consume streams before serialization
*/}
...
On the other hand, Lumenize RPC most values making it feel like you are working locally. Rethrowing errors on the calling side complete with the original stack trace is a great example. Support for accessing `this.ctx` and `this.env` is another.
...
- Cap'n Web’s "magic `.map()` method" records a transformation and replays it server-side to avoid extra trips. If you control the server API, an equivalent “compose it server-side” method often achieves the same with less complexity; `.map()` shines more when you don't have the ability or desire to change the server side.
...
<table>
	...
	<tbody>
		<tr>
			<td>StructuredClone types</td>
			<td>Yes</td>
			<td>Limited</td>
			<td>May come to Cap'n Web later: Map, Set, ArrayBuffer</td>
		</tr>
		<tr>
...
- Runtime validation approach (TypeBox Value vs roll your own with Zod)
