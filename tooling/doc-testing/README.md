...
> **Deprecated**: Do not use for new documentation. Use hand-written `.mdx` files with `@check-example` annotations instead (see `tooling/check-examples`). Some older packages (rpc, testing, mesh/services) still have files generated by this plugin — those files are marked with `generated_by: doc-testing` frontmatter and should not be hand-edited. A backlog item exists to migrate these to the new approach.
